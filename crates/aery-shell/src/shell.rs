...
async fn source_snapshot(shell: &mut BrushShell, snapshot_path: &str) -> Result<()> {
	...
	let source_info = SourceInfo::from("aery-engine:snapshot");
...
async fn run_shell_command(
	...
	let source_info = SourceInfo::from("aery-engine:command");
	let result = session
		.shell
...
async fn run_shell_command_streams(
	...
	let source_info = SourceInfo::from("aery-shell:streams");
	let result = session
		.shell
		.run_string(options.command.clone(), &source_info, &params)
		.await;
...
impl builtins::Command for TimeoutCommand {
	...
	fn execute<SE: brush_core::ShellExtensions>(
		...
		async move {
			...
			let source_info = SourceInfo::from("aery-engine:timeout");
...
mod tests {
	...
	///
	/// Lives in `aery-engine` because the brush-core crate is excluded from the
	...
	async fn embedded_external_command_runs_in_its_own_session() {
		...
		// Build the same kind of session aery-engine uses in production.
		...
		let shell_handle = tokio::spawn(async move {
			let source_info = SourceInfo::from("aery-engine:test");
	...
	/// leave the embedded host (AERY) stopped on its next tty read. The earlier
	...
	async fn cancellation_aborts_internal_background_jobs() {
		let unique = std::time::SystemTime::now()
			...
			.as_nanos();
		let dir =
			std::env::temp_dir().join(format!("aery-shell-bg-cancel-{}-{unique}", std::process::id()));
		...
		let source_info = SourceInfo::from("aery-shell:test");
		let result = session
			.shell
			.run_string(
