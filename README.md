<p align="center">
  <img src="https://github.com/eminent337/aery/blob/main/assets/hero.png?raw=true" alt="aery">
</p>

<p align="center">
  <strong>A coding agent with the IDE wired in.</strong>
  <strong><a href="https://aery.sh">aery.sh</a></strong>
</p>

<p align="center">
  <a href="https://www.npmjs.com/package/@aryee337/aery-coding-agent"><img src="https://img.shields.io/npm/v/@aryee337/aery-coding-agent?style=flat&colorA=222222&colorB=CB3837" alt="npm version"></a>
  <a href="https://github.com/eminent337/aery/blob/main/packages/coding-agent/CHANGELOG.md"><img src="https://img.shields.io/badge/changelog-keep-E05735?style=flat&colorA=222222" alt="Changelog"></a>
  <a href="https://github.com/eminent337/aery/actions"><img src="https://img.shields.io/github/actions/workflow/status/eminent337/aery/ci.yml?style=flat&colorA=222222&colorB=3FB950" alt="CI"></a>
  <a href="https://github.com/eminent337/aery/blob/main/LICENSE"><img src="https://img.shields.io/github/license/eminent337/aery?style=flat&colorA=222222&colorB=58A6FF" alt="License"></a>
...
<p align="center">
  Built on <a href="https://github.com/eminent337">@eminent337</a>'s independent work
</p>
...
```sh
bun install -g @aryee337/aery-coding-agent
```
...
```sh
mise use -g github:eminent337/aery
```
...
Edits that land on the first attempt. Reads that summarize files instead of dumping their content. Searches that return instantly. Pick any model — aery will get it right.
...
Aery is a fully independent coding agent — every tool, every feature, every integration built from the ground up.
...
![aery TUI: a single eval session with `[1/2] pandas describe` (Python) printing a real DataFrame.describe() table, followed by `[2/2] top scorer` (JavaScript) running a reduce. Footer: 'Both kernels ran in one session.'](https://aery.sh/captures/eval.webp)

### 02 · LSP wired into every write

Ask for a rename and you get a rename. The call goes through workspace/willRenameFiles, so re-exports, barrel files, and aliased imports update before the file moves. Everything your IDE knows, the agent knows.

![aery TUI: `LSP references` returns five hits across three files for the symbol `formatBytes`, then `LSP rename` applies the change with edits to format.ts/report.ts/cli.ts, then a `Search formatBytes 0 matches` confirmation. Final line: 'Rename complete. Five edits across three files…'.](https://aery.sh/captures/lsp.webp)
...
![aery TUI: a live lldb-dap session against a native binary at /tmp/aery-native/demo. Adapter=lldb-dap, Status=stopped, Frame=xorshift32, Instruction pointer 0x10000055C, Location demo.c:6:10. Debug scopes and Debug variables cards show locals (x = 57351) and the agent confirms the math: x went from 7 → 57351 (= 7 ^ (7<<13)).](https://aery.sh/clips/dap-poster.webp)

_[Watch the capture ↗](https://aery.sh/clips/dap.mp4)_
...
![aery TUI: web_search returns 10 ranked Perplexity sources for inference-time compute scaling, the agent picks an arxiv paper, calls read https://arxiv.org/pdf/2604.10739v1, and summarizes the paper's headline result with real numbers.](https://aery.sh/clips/web-poster.webp)

_[Watch the capture ↗](https://aery.sh/clips/web.mp4)_

### 07 · Unapologetically native. Even on Windows.

Other agents shell out to rg, grep, find, and bash. On many machines those binaries don't exist, and on the ones where they do, every call costs a fork-exec round-trip. aery links the real implementations into the process. ripgrep, glob, find: in-process. brush is the bash, with sessions that survive across calls. The same aery binary runs on macOS, Linux, and Windows — no WSL bridge.
...
![aery TUI reading pr://eminent337/aery/1063 and then /diff/1, showing hunk headers, added lines, and a [MODIFIED] (+12 -0) summary.](https://aery.sh/captures/pr.webp)
...
![aery TUI: ✓ Read src/session.ts (⚠ 1 conflict), then ✓ Write conflict://1 · 1 line with content @theirs, then a confirmation 'Resolved.'](https://aery.sh/clips/conflict-poster.webp)

_[Watch the capture ↗](https://aery.sh/clips/conflict.mp4)_
...
![aery TUI: ✓ AST Edit: console.log($X) (proposed) 3 replacements · 1 file, then ✓ Accept: 3 replacements in 1 file (AST Edit), followed by 'Applied 3 replacements in src/auth.ts.'](https://aery.sh/clips/codemod-poster.webp)

_[Watch the capture ↗](https://aery.sh/clips/codemod.mp4)_
...
![aery TUI driving the browser tool against DuckDuckGo](https://aery.sh/captures/browser.webp)
...
[Full reference →](https://aery.sh/docs/tools)
...
- **Custom providers** — Declare anything that speaks `openai-completions`, `openai-responses`, `openai-codex-responses`, `azure-openai-responses`, `anthropic-messages`, `google-generative-ai`, or `google-vertex` in `~/.aery/agent/models.yml`.
...
Full provider & routing reference at [aery.sh/docs/providers](https://aery.sh/docs/providers).
...
[`web_search` reference ↗](https://aery.sh/docs/tools#web_search)
...
- Crates: `aery-engine`, `aery-shell`, `aery-ast`
...
| iso        | Workspace isolation shim · apfs · btrfs · zfs · reflink · overlayfs · projfs · rcopy | aery-iso (PAL)                              |   245 |
...
Same engine, four wrappers. `aery` runs the TUI. `aery -p` answers a single prompt and exits. The Node SDK embeds the session in your process. `aery --mode rpc` and `aery acp` hand the wheel to another program over stdio.
...
![aery TUI: the ask tool renders an option picker with three choices, a (Recommended) badge on the first, and 'up/down navigate · enter select · esc cancel' footer.](https://aery.sh/captures/ask.webp)

### SDK — embed in Node

`@aryee337/aery-coding-agent`
...
```ts
import { ModelRegistry, SessionManager, createAgentSession, discoverAuthStorage } from "@aryee337/aery-coding-agent";
...
Full reference: [aery.sh/docs/sdk](https://aery.sh/docs/sdk).

## A harness worth keeping is one you _don't_ outgrow.

Pick it up at **[aery.sh](https://aery.sh)**.

Aery is a coding-first terminal surface: sessions, subagents, slash commands, extensions — all TypeScript, all MIT, all on [GitHub](https://github.com/eminent337/aery). Shape it from config, hook it from outside, or read the source when you need to.
...
On first run aery inherits whatever is already on disk: rules, skills, and MCP servers from `.claude`, `.cursor`, `.windsurf`, `.gemini`, `.codex`, `.cline`, `.github/copilot`, and `.vscode`. No migration script.

### Extensibility

Ask aery to write the piece you're missing, then `/reload-plugins`. Keep it local, ship it in a `marketplace`, or publish it to npm.

## Philosophy

Aery is a terminal-native coding agent with a batteries-included workflow.
...
| Package                                                   | Description                                                                |
| --------------------------------------------------------- | -------------------------------------------------------------------------- |
| **[@aryee337/aery-ai](packages/ai)**                        | Multi-provider LLM client with streaming and model/provider integration    |
| **[@aryee337/aery-core](packages/agent)**             | Agent runtime with tool calling and state management                       |
| **[@aryee337/aery-coding-agent](packages/coding-agent)**    | Interactive coding agent CLI and SDK                                       |
| **[@aryee337/aery-tui](packages/tui)**                      | Terminal UI library with differential rendering                            |
| **[@aryee337/aery-engine](packages/aery-engine)**              | N-API bindings for grep, shell, image, text, syntax highlighting, and more |
| **[@aryee337/aery-stats](packages/stats)**                 | Local observability dashboard for AI usage statistics                      |
| **[@aryee337/aery-utils](packages/utils)**                  | Shared utilities (logging, streams, dirs/env/process helpers)              |
| **[@aryee337/swarm-extension](packages/swarm-extension)** | Swarm orchestration extension package                                      |

### Rust Crates

| Crate                                                         | Description                                                                                         |
| ------------------------------------------------------------- | --------------------------------------------------------------------------------------------------- |
| **[aery-engine](crates/aery-engine)**                           | Core Rust native addon (N-API `cdylib`) used by `@aryee337/aery-engine`; aggregates the crates below |
| **[aery-shell](crates/aery-shell)**                               | Embedded shell / PTY / process management split out of `aery-engine` (wraps `brush-*`)               |
| **[aery-ast](crates/aery-ast)**                                   | tree-sitter-based code summarizer and AST utilities (50+ language grammars)                         |
| **[aery-iso](crates/aery-iso)**                                   | Task isolation backend resolver: APFS clones, btrfs/zfs reflinks, overlayfs, projfs, rcopy          |
...
- [aery.sh](https://aery.sh)
- [GitHub](https://github.com/eminent337/aery)
- [Changelog](https://github.com/eminent337/aery/blob/main/packages/coding-agent/CHANGELOG.md)
- [npm](https://www.npmjs.com/package/@aryee337/aery-coding-agent)
- [Discord](https://discord.gg/4NMW9cdXZa)
- [MIT](https://github.com/eminent337/aery/blob/main/LICENSE)
