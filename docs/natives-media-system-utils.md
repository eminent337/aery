...
This document covers the media/system/conversion exports currently present in `@aryee337/aery-engine`: terminal SIXEL image encoding, HTML conversion, clipboard access, token counting, macOS appearance/power helpers, and work profiling.

## Implementation files

- `crates/aery-engine/src/sixel.rs`
- `crates/aery-engine/src/html.rs`
- `crates/aery-engine/src/clipboard.rs`
- `crates/aery-engine/src/tokens.rs`
- `crates/aery-engine/src/appearance.rs`
- `crates/aery-engine/src/power.rs`
- `crates/aery-engine/src/prof.rs`
- `crates/aery-engine/src/task.rs`
- `packages/aery-engine/native/index.d.ts`

There is no native `PhotonImage` class, `image.rs`, or ProjFS overlay helper module in the current `aery-engine` addon. General-purpose image decode/resize/encode is expected to live outside this native surface; the native image export here is only terminal SIXEL encoding.
...
There is no current `packages/aery-engine` TS wrapper that emits OSC52, handles Termux, or suppresses native clipboard failures. Any best-effort clipboard policy must live in consumers.
