# Porting From Upstream: A Practical Merge Guide

This guide is a repeatable checklist for porting changes from upstream into this repo.
...
When starting a new sync, generate patches from this commit forward in an upstream checkout or remote that contains the commit:
...
- In `packages/tui/test` and `packages/aery-engine/bench`, keep `.js` where surrounding files already use it.
...
- Examples (adjust to match the actual packages you are porting):
  - `@eminent337/aery-coding-agent` → `@aryee337/aery-coding-agent`
  - `@eminent337/aery-agent-core` → `@aryee337/aery-core`
  - `@eminent337/aery-tui` → `@aryee337/aery-tui`
  - `@eminent337/aery-ai` → `@aryee337/aery-ai`
...
- In `packages/coding-agent`, use `logger` from `@aryee337/aery-utils` for internal/runtime logging; CLI command files may use `console.*` for intentional user-facing output.
...
- Prefer existing helpers and utilities over new ad-hoc code.
  ...
  - TypeScript packages generally use source files directly; `@aryee337/aery-engine` exports generated native bindings from `packages/aery-engine/native`.
...
- Replace aery-monorepo repo links where appropriate.
...
4. **Context/capabilities shrink** - Old APIs may have exposed `{ logger, typebox, aery }` that new APIs forgot to include.
...
```
fix(coding-agent): backported aery-monorepo changes (<from>..<to>)
...
```
fix(coding-agent): backported aery-monorepo changes (9f3eef65f..52532c7c0)
...
| Upstream                                           | Our Fork                                                  | Reason                                        |
| -------------------------------------------------- | --------------------------------------------------------- | --------------------------------------------- |
| `clipboard.ts` + `clipboard-image.ts` (tool files) | `src/utils/clipboard.ts` backed by `@aryee337/aery-engine` | Native implementation with a small TS wrapper |
...
| `pkg.aery` manifest field       | `pkg.aery` preferred; fallback to `pkg.aery` remains |
...
- `clipboard-image.ts` — image clipboard support is exposed through `src/utils/clipboard.ts` backed by `@aryee337/aery-engine`
