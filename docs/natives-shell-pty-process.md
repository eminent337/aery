...
This document covers execution/process/terminal primitives in `@aryee337/aery-engine`: `shell`, `pty`, `ps`, and `keys`, using the architecture terms from `docs/natives-architecture.md`.

## Implementation files

- `crates/aery-engine/src/shell.rs`
- `crates/aery-shell/src/shell.rs`
- `crates/aery-shell/src/fixup.rs`
- `crates/aery-shell/src/windows.rs` (Windows-only PATH enrichment)
- `crates/aery-shell/src/process.rs`
- `crates/aery-engine/src/pty.rs`
- `crates/aery-engine/src/ps.rs`
- `crates/aery-engine/src/keys.rs`
- `crates/aery-engine/src/task.rs`
- `packages/aery-engine/native/index.d.ts`

## Layer ownership

- **Package entrypoint** (`packages/aery-engine/native/index.js`): loads the `.node` addon and exports generated N-API bindings.
- **Rust N-API module layer** (`crates/aery-engine/src/*`): JS-facing shell/PTY/process/key exports and callback bridging.
- **Runtime core** (`crates/aery-shell/src/*`): brush shell execution, cancellation cleanup, minimizer integration, command fixups, and cross-platform process references.
...
- `applyBashFixups(command)` strips safe trailing `| head`/`| tail` pipeline caps and redundant trailing `2>&1` according to `aery_shell::fixup` rules. It returns `{ command, stripped }` and does not execute anything.
...
- Windows-only path enrichment (`aery-shell/src/windows.rs`) appends discovered Git-for-Windows paths when present and not already included.
...
- `CancelToken` is constructed from `timeoutMs` and optional `AbortSignal`, then converted into the shared `aery_shell::cancel::CancelToken`.
...
The platform-specific implementation lives in `aery_shell::process`; `crates/aery-engine/src/ps.rs` is a N-API shim plus re-exports used by PTY termination.
