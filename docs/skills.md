...
- `description` is required for:
  - native `.aery` provider skill discovery (`requireDescription: true`)
...
1. `native` (priority 100) — `.aery` user/project skills via `src/discovery/builtin.ts`
2. `aery-plugins` (priority 90) — `skills/` bundled next to extension packages loaded through `extensions:` or `--extension`/`-e`
