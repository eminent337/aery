...
1. Capability providers (`toolCapability`), including:
   - Native AERY config (`~/.aery/agent/tools`, `.aery/tools`)
   - Claude config (`~/.claude/tools`, `.claude/tools`)
   - Codex config (`~/.codex/tools`, `.codex/tools`)
   - Claude marketplace plugin cache provider
2. Installed plugin manifests (`~/.aery/plugins/node_modules/*` via plugin loader)
...
```ts
import type { CustomToolFactory } from "@aryee337/aery-coding-agent";
...
const factory: CustomToolFactory = (aery) => ({
  ...
  parameters: aery.zod.object({
    glob: aery.zod.string().optional().default("**/*.ts"),
  }),
...
Schemas are authored with Zod (`aery.zod`) and flow through the shared validation/wire pipeline.
...
- `aery`: injected `@aryee337/aery-coding-agent` exports
...
- Forward `signal` to subprocess work (`aery.exec(..., { signal })`) for cooperative cancellation.
...
- Guard UI usage with `aery.hasUI`.
