...
This document defines the JS/TS contract between `@aryee337/aery-engine` callers and the loaded N-API addon.

Current package shape is direct-to-native: there is no `packages/aery-engine/src/<module>` TypeScript wrapper layer. The public API is the generated `packages/aery-engine/native/index.d.ts` declaration file, the ESM loader/export wrapper in `packages/aery-engine/native/index.js`, and the Rust `#[napi]` exports in `crates/aery-engine/src`.
...
- Rust modules under `crates/aery-engine/src/*.rs`
...
3. **Rust N-API exports** (`crates/aery-engine/src`)
...
`packages/aery-engine/package.json` exposes the package root only:
...
Consumers in `packages/coding-agent` and `packages/tui` import directly from `@aryee337/aery-engine`.
...
1. Rust `#[napi]` implementation in the owning `crates/aery-engine/src/<module>.rs`.
2. `crates/aery-engine/src/lib.rs` if a new module is added.
