...
If your extension/tool can run in non-interactive mode, guard with `ctx.hasUI` / `aery.hasUI`.

## Core component contract (`@aryee337/aery-tui`)
...
```ts
import { replaceTabs, truncateToWidth } from "@aryee337/aery-tui";
...
Custom tools typically use the same UI entrypoint via the factory-scoped `aery.ui` object, then return the selected value in normal tool content:
...
async execute(toolCallId, params, onUpdate, ctx, signal) {
  ...
  const picked = await aery.ui.custom<string | undefined>((tui, theme, done) => {
...
import {
  getSelectListTheme,
  type ExtensionAPI,
} from "@aryee337/aery-coding-agent";
...
export default function extension(aery: ExtensionAPI): void {
  aery.registerCommand("pick-model", {
