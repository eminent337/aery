...
`discovery.ts` header comments still mention `.aery` and do not mention `.codex`/`.gemini`. Actual runtime order is driven by `src/config.ts` and currently uses `.aery`, `.claude`, `.codex`, `.gemini`.
...
- Higher-priority source family overrides lower (`.aery` before `.claude`, etc.).
