...
- Project directory: `<cwd>/.aery/extensions`
- User directory: `~/.aery/agent/extensions`
- Native legacy/settings JSON entries: `<cwd>/.aery/settings.json#extensions` and `~/.aery/agent/settings.json#extensions`
...
- Native auto-discovery is currently `.aery` based.
- Legacy `.aery` is still accepted in package manifests (`aery.extensions`) and project override lookup, but `.aery/extensions` is not a native root here.
...
Plugin extension entries come from package `aery.extensions` / `aery.extensions` manifests, including enabled feature entries.
...
- User: `~/.aery/agent/config.yml` (or custom agent dir via `PI_CODING_AGENT_DIR`)
- Project/native settings capability: `<cwd>/.aery/config.yml` and `<cwd>/.aery/settings.json`

Native extension-module discovery also reads legacy JSON extension lists from:

- `~/.aery/agent/settings.json`
- `<cwd>/.aery/settings.json`
...
```json
{
  "extensions": ["./.aery/extensions/my-extra"]
}
```
...
1. `package.json` in that directory with `aery.extensions` (or legacy `aery.extensions`) -> use declared entries
2. `index.ts`
3. `index.js`
4. Otherwise scan one level for extension entries:
   - direct `*.ts` / `*.js`
   - subdir `index.ts` / `index.js`
   - subdir `package.json` with `aery.extensions` / `aery.extensions`
