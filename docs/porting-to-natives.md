# Porting to aery-engine (N-API) — Field Notes

This is a practical guide for moving hot paths into `crates/aery-engine` and wiring them through the generated native package entrypoint. It exists to avoid the same failures happening twice.
...
`@aryee337/aery-engine` no longer has a `packages/aery-engine/src/<module>` TypeScript wrapper layer. The package root points at generated native artifacts:

- runtime entry/export wrapper: `packages/aery-engine/native/index.js`
- types entry: `packages/aery-engine/native/index.d.ts`
- loader helpers: `packages/aery-engine/native/loader-state.js`
- embedded manifest: `packages/aery-engine/native/embedded-addon.js`

Consumers import directly from `@aryee337/aery-engine`. The generated declarations and explicit ESM exports are produced during `bun --cwd=packages/aery-engine run build`.

## Anatomy of a native export

**Rust side:**

- Implementation lives in `crates/aery-engine/src/<module>.rs`.
- If you add a new module, register it in `crates/aery-engine/src/lib.rs`.
...
- `packages/aery-engine/scripts/build-native.ts` runs napi-rs, installs the `.node` artifact, copies generated `index.d.ts`, and regenerates explicit ESM class/function exports plus enum runtime exports in the checked-in `native/index.js`.
- `packages/aery-engine/native/index.js` is the ESM entrypoint that calls the loader, exposes named exports, and rejects install/compiled `.node` files that do not expose the package-version sentinel.
- `packages/aery-engine/package.json` exposes only the package root (`@aryee337/aery-engine`) as the import surface. At publish time the binaries are split out: the core ships the loader only (no `.node`), and each platform's `.node` is published as an optional-dependency leaf package `@aryee337/aery-engine-<tag>` (`scripts/ci-release-publish.ts` + `packages/aery-engine/scripts/gen-npm-packages.ts`). This is transparent to importers — you still `import` from `@aryee337/aery-engine`.
...
- If it is a new module, add it to `crates/aery-engine/src/lib.rs`.
...
- Run `bun --cwd=packages/aery-engine run build`.
- Confirm the generated `packages/aery-engine/native/index.d.ts` includes the new export with the intended JS name/signature.
- Confirm `packages/aery-engine/native/index.js` has generated explicit ESM exports for the new class/function and enum objects when enum changes are involved.

3. **Update consumers**

- Import the new export directly from `@aryee337/aery-engine`.
...
- Put benchmarks next to the owning package (`packages/tui/bench`, `packages/aery-engine/bench`, or `packages/coding-agent/bench`).
...
- `modern`: `aery_engine.<tag>-modern.node`, then `...-baseline.node`, then `aery_engine.<tag>.node`.
- `baseline`: `aery_engine.<tag>-baseline.node`, then `aery_engine.<tag>.node`.

Non-x64 uses `aery_engine.<tag>.node`.
...
```bash
rm packages/aery-engine/native/aery_engine.<platform>-<arch>.node
rm packages/aery-engine/native/aery_engine.<platform>-<arch>-modern.node
rm packages/aery-engine/native/aery_engine.<platform>-<arch>-baseline.node
bun --cwd=packages/aery-engine run build
```

For compiled binaries or Windows staging, delete the versioned addon cache shown in the loader error (normally under `~/.aery/natives/<version>` unless `$XDG_DATA_HOME/aery` is used).
...
```bash
bun -e 'import { createRequire } from "node:module"; const require = createRequire(import.meta.url); const mod = require(process.argv[2]); console.log(Object.keys(mod).sort())' -- /path/from/loader/error/aery_engine.<tag>[-variant].node
```
