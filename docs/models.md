...
- `transport`: `aery-native` only. When set, every model under that provider is sent to an `aery auth-gateway` compatible `baseUrl` via `POST /v1/aery/stream`; `apiKey` is the gateway bearer.
...
1. Load built-in providers/models from `@aryee337/aery-ai`.
...
Extensions can register providers at runtime (`aery.registerProvider(...)`), including:
...
When `AERY_AUTH_BROKER_URL` (or `auth.broker.url`) is set, the local SQLite credential store is replaced by `RemoteAuthCredentialStore`. Layers 2 and 3 above (stored API key / OAuth in `agent.db`) are served from a broker-supplied snapshot whose `refresh` tokens are redacted; expiry triggers `POST /v1/credential/:id/refresh` on the broker rather than a local refresh.

`AuthStorage.setConfigApiKey` lets a `models.yml` `apiKey` win over a broker-resolved OAuth token without overriding a runtime `--api-key`. See [`auth-broker-gateway.md`](./auth-broker-gateway.md) for the full broker / gateway design and env surface (`AERY_AUTH_BROKER_URL`, `AERY_AUTH_BROKER_TOKEN`, `auth.broker.url`, `auth.broker.token`).
...
Role aliases like `aery/smol` expand through `settings.modelRoles`. Each role value can also append a thinking selector such as `:minimal`, `:low`, `:medium`, or `:high`.
...
Most model configuration now flows through `models.yml` via `ModelRegistry`. Explicit `.json` / `.jsonc` paths remain supported only when passed programmatically to `ModelRegistry`; the default user config is `~/.aery/agent/models.yml`.
