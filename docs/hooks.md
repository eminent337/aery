...
```ts
import type { HookAPI } from "@aryee337/aery-coding-agent/extensibility/hooks";

export default function hook(aery: HookAPI): void {
...
- register event handlers with `aery.on(...)`
- send persistent custom messages with `aery.sendMessage(...)`
- persist non-LLM state with `aery.appendEntry(...)`
- register slash commands via `aery.registerCommand(...)`
- register custom message renderers via `aery.registerMessageRenderer(...)`
- run shell commands via `aery.exec(...)`
- author schemas/helpers with injected `aery.zod`, `aery.typebox`, and package exports via `aery.aery`
...
Discovery providers for `hookCapability` still model pre/post shell-style hook files (for example `.claude/hooks/pre/*`, `.aery/.../hooks/pre/*`).
...
export default function (aery: HookAPI): void {
  aery.on("tool_call", async (event, ctx) => {
...
`src/extensibility/hooks/index.ts` and the package subpath `@aryee337/aery-coding-agent/extensibility/hooks` export:
...
The package root (`@aryee337/aery-coding-agent`) does not re-export `HookAPI`; import legacy hook types from the hooks subpath.
