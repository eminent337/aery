...
This document covers the runtime loader shipped by `@aryee337/aery-engine`: how `native/index.js` decides which `.node` file to require, how compiled-binary embedded payloads are extracted, and what startup failures report.
...
For install and compiled-binary paths, the loader verifies a release sentinel export named from `package.json#version` (for example `__aeryNativesV15_7_2`). Workspace-dev loads skip this validation so a local checkout can rebuild after a pull. The loader does not validate the full export surface; stale same-version or incomplete binaries still surface as missing members or native errors at use sites.
...
- **Package version**: from `packages/aery-engine/package.json`.
- **Core directories**:
  - `leafPackageDir`: directory of the platform leaf package, resolved via `require.resolve("@aryee337/aery-engine-<tag>/package.json")`; `null` when no leaf is installed (e.g. local dev).
  - `nativeDir`: package-local `packages/aery-engine/native`.
  - `execDir`: directory containing `process.execPath`.
  - `versionedDir`: `<getNativesDir()>/<packageVersion>`.
  - `userDataDir` fallback:
    - Windows: `%LOCALAPPDATA%/aery` or `%USERPROFILE%/AppData/Local/aery`.
    - Non-Windows: `~/.local/bin`.
- **Natives cache root** (`getNativesDir()`):
  - if `$XDG_DATA_HOME/aery` exists, `$XDG_DATA_HOME/aery/natives`;
  - otherwise `~/.aery/natives`.
...
No variant suffix is used; the filename is `aery_engine.<platform>-<arch>.node`.

### Filename construction

`loader-state.js#getAddonFilenames` returns:

- Non-x64 or no variant: `aery_engine.<tag>.node`
- x64 + `modern`:
  1. `aery_engine.<tag>-modern.node`
  2. `aery_engine.<tag>-baseline.node`
  3. `aery_engine.<tag>.node`
- x64 + `baseline`:
  1. `aery_engine.<tag>-baseline.node`
  2. `aery_engine.<tag>.node`
...
- `Failed to load aery_engine native addon for <platformTag>` or `<platformTag> (<variant>)`
...
- release sentinel mismatch details when a loadable `.node` belongs to another `@aryee337/aery-engine` version.

### Non-compiled startup failures

Normal package/runtime diagnostics include:

- reinstall hint (`bun install @aryee337/aery-engine`),
- local rebuild command (`bun --cwd=packages/aery-engine run build`),
- optional x64 variant build hint (`TARGET_VARIANT=baseline|modern bun --cwd=packages/aery-engine run build`).
