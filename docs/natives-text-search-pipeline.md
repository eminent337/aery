...
This document maps the `@aryee337/aery-engine` text/search/code surface from generated JS/TS exports to Rust N-API modules and back to JS result objects.

Terminology follows `docs/natives-architecture.md`:

- **Generated binding**: public API in `packages/aery-engine/native/index.d.ts`.
- **Rust module layer**: N-API exports in `crates/aery-engine/src/*`.
...
- `crates/aery-engine/src/grep.rs`
- `crates/aery-engine/src/glob.rs`
- `crates/aery-engine/src/glob_util.rs`
- `crates/aery-engine/src/fs_cache.rs`
- `crates/aery-engine/src/fd.rs`
- `crates/aery-engine/src/ast.rs`
- `crates/aery-engine/src/text.rs`
- `crates/aery-engine/src/highlight.rs`
...
These exports are direct native APIs used by tooling; they are not mediated by a TS wrapper in `packages/aery-engine`.
