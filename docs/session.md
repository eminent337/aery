...
```text
~/.aery/agent/sessions/--<cwd-encoded>--/<timestamp>_<sessionId>.jsonl
```
...
```text
~/.aery/agent/blobs/<sha256>
```

Terminal breadcrumb files are written under:

```text
~/.aery/agent/terminal-sessions/<terminal-id>
```
...
- `listAll()` -> sessions across all project scopes under `~/.aery/agent/sessions`
