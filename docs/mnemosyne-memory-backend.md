...
Aery can use `@aryee337/aery-mnemopi` as a local long-term memory backend.
...
| `mnemopi.llmMode`             | `smol`                 | `smol` uses the configured aery-ai smol model, `remote` uses the settings below, and `none` disables LLM calls.                                                           |
