...
┌───────────────────────────────────────┐
│ 1) ~/.aery/agent, ~/.claude, ...       │
│ 2) <cwd>/.aery, <cwd>/.claude, ...     │
└───────────────────────────────────────┘
                    │
                    ▼
        capability providers enumerate items
 (native provider scans project .aery before user .aery;
...
`CONFIG_DIR_NAME` is `.aery` (`packages/utils/src/dirs.ts`).

## Important constraint

The generic helpers in `src/config.ts` do **not** include `.aery` in source discovery order.
...
Walks parent directories upward and returns the **nearest existing directory per source base** (`.aery`, `.claude`, `.codex`, `.gemini`), then sorts results by source priority.
...
1. Global settings: `~/.aery/agent/config.yml`
...
1. Migrate from `~/.aery/agent/settings.json` (renamed to `.bak` on success)
...
## 6) Native `.aery` provider behavior (`packages/coding-agent/src/discovery/builtin.ts`)
...
- Skills scan `<ancestor>/.aery/skills` for each ancestor from the current working directory up to the repo root/home boundary, plus `~/.aery/agent/skills`, without requiring the root `.aery` directory itself to be non-empty.
- `SYSTEM.md` and `AGENTS.md` read user-level files directly and use nearest-ancestor project `.aery` lookup for project files, but the project `.aery` directory must be non-empty. See [`docs/system-prompt-customization.md`](./system-prompt-customization.md) for the full `SYSTEM.md` / `APPEND_SYSTEM.md` contract (replace vs. append, templating).

### Scope-specific loading

- Skills: `<ancestor>/.aery/skills/*/SKILL.md` and `~/.aery/agent/skills/*/SKILL.md`
...
## For `SYSTEM.md` and `AGENTS.md`, native provider uses nearest-ancestor project `.aery` directory search (walk-up) and still requires the project `.aery` dir to be non-empty.
