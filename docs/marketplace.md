...
A **plugin** is a directory containing Claude/AERY plugin content such as skills, commands, hooks, tools, MCP servers, LSP servers, rules, prompts, or extension modules. Plugins are identified by `name@marketplace` (e.g. `code-review@claude-plugins-official`).

**Scopes**: marketplace plugins can be installed at two scopes:

- **user** (default) -- available in all projects, stored in `~/.aery/plugins/installed_plugins.json`
- **project** -- available only in the active project, stored in the nearest project `.aery/plugins/installed_plugins.json`
...
```
aery plugin marketplace add <source>
aery plugin marketplace remove <name>
aery plugin marketplace update [name]
aery plugin marketplace list
aery plugin discover [marketplace]
aery plugin install [--force] [--scope user|project] name@marketplace
aery plugin uninstall [--scope user|project] name@marketplace
aery plugin upgrade [--scope user|project] [name@marketplace]
aery plugin enable [--scope user|project] name@marketplace
aery plugin disable [--scope user|project] name@marketplace
```
