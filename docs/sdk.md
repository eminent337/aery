...
The SDK is the in-process integration surface for `@aryee337/aery-coding-agent`.
...
```bash
bun add @aryee337/aery-coding-agent
```

## Entry points

`@aryee337/aery-coding-agent` exports the SDK APIs from the package root (and also via `@aryee337/aery-coding-agent/sdk`).
...
```ts
import { createAgentSession } from "@aryee337/aery-coding-agent";
...
- `cwd`: `getProjectDir()`
- `agentDir`: `~/.aery/agent` (via `getAgentDir()`)
...
```ts
import { createAgentSession, SessionManager } from "@aryee337/aery-coding-agent";
...
```ts
import { SessionManager } from "@aryee337/aery-coding-agent";
