# MCP configuration in AERY

This guide explains how to add, edit, and validate MCP servers for the AERY coding agent.
...
AERY can discover MCP servers from multiple tools (`.claude/`, `.cursor/`, `.vscode/`, `opencode.json`, and more), but for AERY-native configuration you should usually use one of these primary files:

- Project: `.aery/mcp.json`
- User: `~/.aery/agent/mcp.json`

The native provider also reads `.aery/.mcp.json` and `~/.aery/agent/.mcp.json` for compatibility, but AERY writes to the primary `mcp.json` paths above.

AERY also accepts fallback standalone files in the project root:

- `mcp.json`
- `.mcp.json`

Use `.aery/mcp.json` or `~/.aery/agent/mcp.json` when you want AERY to own the configuration. Use root `mcp.json` / `.mcp.json` only when you want a portable fallback file that other MCP clients may also read.
...
```json
{
  "$schema": "https://raw.githubusercontent.com/eminent337/aery/main/packages/coding-agent/src/config/mcp-schema.json",
  "mcpServers": {}
}
```

AERY now writes this automatically when `/mcp add`, `/mcp enable`, `/mcp disable`, `/mcp reauth`, or other config-writing flows create or update an AERY-managed MCP file.

## File shape

AERY supports this top-level structure:
...
- `disabledServers` — user-level denylist used to turn off discovered servers by name; runtime loading reads this list from `~/.aery/agent/mcp.json`
...
- `auth?: { ... }` — auth metadata used by AERY for OAuth/API-key flows
- `oauth?: { ... }` — explicit OAuth client settings used during auth/reauth

Set `AERY_MCP_TIMEOUT_MS=0` to disable the client-side timeout for every MCP server in the current process. Set it to a positive millisecond value, such as `AERY_MCP_TIMEOUT_MS=120000`, to apply one global timeout without editing each server entry.
...
AERY understands two auth-related objects.
...
Use this when AERY should remember how to rehydrate credentials for a server.
...
AERY expands `${VAR}` and `${VAR:-default}` placeholders while discovering MCP configs from AERY-native files and standalone fallback files. Expansion applies recursively to string values in `command`, `args`, `env`, `cwd`, `url`, `headers`, `auth`, and `oauth`; unresolved placeholders remain literal strings.
...
Before AERY launches a stdio server or makes an HTTP/SSE request, it resolves stdio `env` values and HTTP/SSE `headers` values like this:

1. If a value starts with `!`, AERY runs the rest as a shell command with a 10s timeout and uses trimmed stdout.
2. If the command fails, times out, or prints only whitespace, that `env`/`headers` entry is omitted.
3. Otherwise AERY checks whether the value names an environment variable.
4. If that environment variable is set to a non-empty value, AERY uses the environment value; otherwise it uses the string literally.
...
`disabledServers` is read from the user config file (`~/.aery/agent/mcp.json`) when a server is discovered from any source and you want AERY to ignore it without editing that other tool's config.
...
## Validation rules AERY enforces
...
- If you paste a remote server config and forget `"type": "http"`, AERY will treat it as `stdio` and complain that `command` is missing
- `sse` remains valid for compatibility, but new hosted servers should usually be configured as `http`

## Discovery and precedence

AERY does not merge duplicate server definitions across files. Discovery providers are prioritized, and the higher-priority definition wins. Separately, `disabledServers` from `~/.aery/agent/mcp.json` can suppress a discovered server by name.

In practice:

- prefer `.aery/mcp.json` or `~/.aery/agent/mcp.json` when you want an AERY-specific override
...
Pick one transport. AERY treats `command` as stdio and `url` as http/sse.
...
### The server exists in another tool's config but not in AERY

Run `/mcp list`. AERY discovers many third-party MCP files, but project-level loading can also be disabled via the `mcp.enableProjectConfig` setting, and a user-level `disabledServers` entry can suppress a server by name.
