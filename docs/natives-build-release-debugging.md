...
This runbook describes how `@aryee337/aery-engine` produces `.node` addons, generated declarations, and compiled-binary embedded payloads, and how to debug loader/build failures.
...
- `crates/aery-engine/Cargo.toml`

## Build pipeline overview

### 1) Build entrypoints

`packages/aery-engine/package.json` scripts:
...
Root scripts include `build:native` as `bun --cwd=packages/aery-engine run build`.
...
- `napi build`
- `--manifest-path crates/aery-engine/Cargo.toml`
- `--package-json-path packages/aery-engine/package.json`
- `--platform`
- `--no-js`
- `--dts index.d.ts`
...
`crates/aery-engine/Cargo.toml` declares `crate-type = ["cdylib"]`; napi-rs emits `.node` artifacts plus generated `index.d.ts` in an isolated temporary output directory under `packages/aery-engine/native/.build/`.
...
2. normalizes its name to `aery_engine.<platform>-<arch>(-variant).node` when needed;
3. installs the addon into `packages/aery-engine/native/` with temp-file + rename semantics;
4. copies generated `index.d.ts` into `packages/aery-engine/native/`;
...
- x64: `aery_engine.<platform>-<arch>-modern.node` or `...-baseline.node`
- non-x64: `aery_engine.<platform>-<arch>.node`
...
4. **Compile**: run napi-rs against `crates/aery-engine` into an isolated output directory.
5. **Locate artifact**: accept the canonical filename or a single napi-rs-generated `aery_engine.<platform>-<arch>*.node` candidate.
6. **Install**: copy/rename addon into `packages/aery-engine/native`.
...
3. **Validate availability**: at least one expected file must exist in `packages/aery-engine/native`.
...
1. Build addon: `bun --cwd=packages/aery-engine run build`.
...
3. Runtime candidate order includes:
   ...
   - versioned cache dir,
   - legacy compiled-binary dir (`%LOCALAPPDATA%/aery` on Windows, `~/.local/bin` elsewhere),
...
| ---------------------------------------------------------------------- | ------------------------------------------------------------------------------------------- | ----------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------- |
| `Cannot find module` or dynamic library load error for every candidate | Missing release artifact, wrong platform tag, or stale compiled cache                       | Inspect loader error list and `packages/aery-engine/native` filenames | Build correct target/variant; delete stale cache for the package version                              |
...
TARGET_VARIANT=modern bun --cwd=packages/aery-engine run build
TARGET_VARIANT=baseline bun --cwd=packages/aery-engine run build

# Generate embedded addon manifest from built native files
bun --cwd=packages/aery-engine run embed:native
# Output archive: packages/aery-engine/native/embedded-addons.<platform>-<arch>.tar.gz

# Reset embedded manifest to null stub
bun --cwd=packages/aery-engine run embed:native -- --reset
```

## Orchestrator-side content-addressed build cache (robaery)

When `aery-engine` is built inside the robaery orchestrator (`python/robaery/`), workspaces share built artifacts through a content-addressed cache instead of rebuilding from scratch in every per-issue worktree. The cache is **orchestrator-side only** — `bun --cwd=packages/aery-engine run build` itself is unchanged; the cache lives outside the build pipeline and is populated/captured around `ensure_workspace` and post-task success in `python/robaery/src/natives_cache.py`.

### What is cached

The complete set of files in `packages/aery-engine/native/` that are pure functions of the cache-key inputs:

- `aery_engine.<platform>-<arch>[-variant].node` (glob `aery_engine.*.node`)
- `index.d.ts`
- `index.js`
- `embedded-addon.js`
...
1. `crates` (whole subtree — aery-engine transitively depends on other workspace crates)
2. `Cargo.lock`
3. `Cargo.toml`
4. `rust-toolchain.toml`
5. `packages/aery-engine` (whole subtree — build script, `scripts/*`, package.json with napi config)
...
- Root: `/data/cache/aery-engine` (provisioned by `entrypoint.sh` alongside the cargo caches, owned `root:aery`, mode `02770` setgid so cached files inherit `gid=aery` and stay readable by every slot user).
...
### Configuration (settings on `robaery.config.Settings`)
...
| `ROBOMP_NATIVES_CACHE_ROOT`                 | `/data/cache/aery-engine` | Cache root directory. Must be `root:aery 02770` for cross-slot reads.                                |
...
- One key: `rm -rf /data/cache/aery-engine/<repo-slug>/<sha256>`.
- One repo: `rm -rf /data/cache/aery-engine/<repo-slug>`.
- Everything: `rm -rf /data/cache/aery-engine/*` (preserve the root so its setgid mode survives).
- Stuck lock: `rm /data/cache/aery-engine/<repo-slug>/.lock` (only when no orchestrator process is touching the repo).

Trigger an automatic miss by editing any path in the key set: a single touched byte under `crates/`, `Cargo.lock`, `Cargo.toml`, `rust-toolchain.toml`, or `packages/aery-engine/` shifts the tree hash and forces a fresh build at the next populate.
