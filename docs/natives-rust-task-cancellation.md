# Native Rust task execution and cancellation (`aery-engine`)

This document describes how `crates/aery-engine` schedules native work and how cancellation flows from JS options (`timeoutMs`, `AbortSignal`) into Rust execution.
...
| `Shell#run(options, onChunk?)`          | `Shell::run`                | `task::future(env, "shell.run", ...)`                          | JS `CancelToken` is converted into `aery_shell::cancel::CancelToken`; shell races it against command completion and descendant cleanup |
