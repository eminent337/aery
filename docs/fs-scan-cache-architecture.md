...
This document defines the current contract for the shared filesystem scan cache implemented in Rust (`crates/aery-engine/src/fs_cache.rs`) and consumed by native discovery/search APIs exposed to `packages/coding-agent`.
...
- Cache implementation and policy: `crates/aery-engine/src/fs_cache.rs`
- Native consumers:
  - `crates/aery-engine/src/glob.rs`
  - `crates/aery-engine/src/fd.rs` (`fuzzyFind`)
  - `crates/aery-engine/src/grep.rs` (cached directory mode only)
- JS binding/export:
  - `packages/aery-engine/native/index.d.ts` (`invalidateFsScanCache`)
