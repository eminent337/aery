...
Loads `.aery` rules from:

- project: `<cwd>/.aery/rules/*.{md,mdc}` when the cwd `.aery` directory exists
- user: `~/.aery/agent/rules/*.{md,mdc}`
- sticky user rule: `~/.aery/agent/RULES.md`
- sticky project rule: nearest ancestor `.aery/RULES.md` while walking from cwd toward the repository root
...
- `native` appends project `.aery/rules`, user `~/.aery/agent/rules`, user `RULES.md`, then nearest project `RULES.md`.
