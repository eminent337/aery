...
Run `/hotkeys` inside an `aery` session to see the active chords for your current build. The list reflects any remaps loaded from disk and any bindings added by extensions.

## Customize keybindings

User remaps live in `~/.aery/agent/keybindings.yml`. The file is a YAML mapping whose keys are keybinding action IDs and whose values are either one chord string or an array of chord strings. It is not read from `~/.aery/agent/config.yml`, and there is no nested `keybindings` object.
...
On Windows Terminal, `Ctrl+V` may be handled by the terminal paste command before `aery` sees it; use the `Alt+V` fallback when clipboard image paste appears to do nothing.
