...
- event handlers (`aery.on(...)`)
- LLM-callable tools (`aery.registerTool(...)`)
- slash commands (`aery.registerCommand(...)`)
...
- calling action methods like `aery.sendMessage()` during extension load throws `ExtensionRuntimeNotInitializedError`
...
export default function (aery: ExtensionAPI) {
  ...
  aery.setLabel("Safety + Utilities");
  ...
  aery.registerCommand("hello-ext", {
...
In interactive mode, `input` handlers run before the built-in first-message auto-title check. Extensions that call `await aery.setSessionName(...)` from `input` can set the persisted session name and prevent the default auto-generated title from running for that session.

Also exposed:

- `aery.logger`
- `aery.typebox` (zod-backed compatibility shim for legacy TypeBox-style schemas)
- `aery.zod` (injected `zod/v4` module — canonical for tool parameter schemas)
- `aery.aery` (package exports)

### Message delivery semantics

`aery.sendMessage(message, options)` supports:
...
`aery.sendUserMessage(content, { deliverAs })` always goes through prompt flow; while streaming it queues as steer/follow-up.
...
1. Persist with `aery.appendEntry(customType, data)`.
...
```ts
aery.registerMessageRenderer("my-type", (message, { expanded }, theme) => {
  // return aery-tui Component
});
```
