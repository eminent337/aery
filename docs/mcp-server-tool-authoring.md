...
```text
Config sources (.aery/.claude/.cursor/.vscode/mcp.json, mcp.json, etc.)
...
In practice MCP servers also come from higher-priority providers (for example native `.aery/...` and tool-specific config dirs). Authoring guidance:

- Prefer `.aery/mcp.json` (project) or `~/.aery/agent/mcp.json` (user) for explicit control.
...
AERY-native MCP config (`.aery/mcp.json`, `~/.aery/agent/mcp.json`, plus their `.mcp.json` variants) expands `${VAR}` and `${VAR:-default}` placeholders recursively before converting to runtime config. It also accepts boolean/string forms for `enabled` (`true`, `false`, `1`, `0`) and numeric strings for `timeout`.
