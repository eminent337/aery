...
3. Model selection tries, in order, `aery/vision`, `aery/default`, the active model string from the session, then `availableModels[0]`. `expandRoleAlias(...)` and `resolveModelFromString(...)` handle each lookup.
