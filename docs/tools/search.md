...
- Key collaborators:
  ...
  - `packages/aery-engine/native/index.d.ts` — native `grep()` types exposed to TS.
  - `crates/aery-engine/src/grep.rs` — native regex/file search implementation.
...
4. Internal URLs are resolved before filesystem scope resolution:
   ...
   - `aery://` expands to every embedded documentation file via URL completion;
...
9. It calls native `grep()` from `@aryee337/aery-engine` with:
   ...
   - `mode: content`.
10. Native execution happens in `crates/aery-engine/src/grep.rs`:
...
5. **Internal URL paths**
   ...
   - `aery://` expands to all embedded documentation files so it can be used as a docs search root.
