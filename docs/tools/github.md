...
- Worktree path is `path.join(getWorktreesDir(), encodeRepoPathForFilesystem(primaryRepoRoot), localBranch)`, where `getWorktreesDir()` is `~/.aery/wt`; effective path is `~/.aery/wt/<encoded-primary-repo-root>/pr-<number>`.
...
- The branch push metadata is persisted with `git config` under the repository's shared `.git/config` as:
  ...
  - `branch.pr-<number>.aeryPrHeadRef`
  - `branch.pr-<number>.aeryPrUrl`
  - `branch.pr-<number>.aeryPrIsCrossRepository`
  - `branch.pr-<number>.aeryPrMaintainerCanModify`
...
Push target resolution reads the `branch.<name>.aeryPrHeadRef`, `pushRemote`/`remote`, `aeryPrUrl`, `aeryPrMaintainerCanModify`, and `aeryPrIsCrossRepository` git-config keys written by `pr_checkout`. If the current checked-out branch matches the target branch, the source ref is `HEAD`; otherwise it pushes `refs/heads/<branch>`. The refspec is `HEAD:refs/heads/<headRef>` or `refs/heads/<branch>:refs/heads/<headRef>`.
...
- Filesystem
  ...
  - `pr_checkout` may create directories under `~/.aery/wt/<encoded-primary-repo-root>/` and add git worktrees there.
...
- Local validation errors throw `ToolError`, including:
  ...
  - `pr_push` on a branch without `aeryPrHeadRef` metadata
