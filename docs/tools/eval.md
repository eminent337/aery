...
- Top-level static `import ... from ...` and dynamic `import(...)` calls are routed through `rewriteImports()`, which sends them via `__aery_import__` so the specifier resolves against the session cwd
- Module cache is busted for **local** imports between cells so edits to source files are picked up without restarting the runtime. `__aery_import__` deletes `require.cache[absPath]` before re-importing whenever the original specifier is a filesystem path: relative (`./x`, `../x`, `.`, `..`), POSIX-absolute (`/...`), home-prefixed (`~/...`), or Windows drive-letter (`C:\...` / `C:/...`). Bare specifiers (`react`, `lodash/x`) and URL/scheme specifiers (`node:fs`, `file://...`, `https://...`) are left in cache so package identity stays stable across cells. The cache-bust only fires when the resolved target is an absolute path — unresolved bare-package fallbacks (`resolveImportSpecifier()` returning the original specifier) skip it.
...
- Kernel `display_data` / `execute_result` messages map to:
  - `application/x-aery-status` → status event
...
- `model` selects a tier (default `"default"`):
  - `"smol"` → `aery/smol` role (fast / cheap)
  - `"default"` → the session's active model, falling back to the `aery/default` role
  - `"slow"` → `aery/slow` role; requests high reasoning effort only on reasoning-capable models
