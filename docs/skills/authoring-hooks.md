...
description: Use when creating a new aery hook. Covers HookAPI, event catalog, blocking/overriding tool calls, and context modification.
---

# Authoring Hooks

Hooks are event-driven interceptors that run alongside the agent loop. They are best used for cross-cutting concerns: safety policy, secret redaction, context pruning, audit logging. A hook module registers handlers via `aery.on(event, handler)` and can block tool execution, override tool output, or rewrite the message context before each LLM call.
...
export default function myHook(aery: HookAPI): void {
...
export default function myExtension(aery: ExtensionAPI): void {
  aery.on("tool_call", async (event, ctx) => { /* ... */ });
}
```
...
export default function rmRfBlocker(aery: HookAPI): void {
  aery.on("tool_call", async (event, ctx) => {
...
export default function apiKeyRedactor(aery: HookAPI): void {
...
export default function contextFilter(aery: HookAPI): void {
