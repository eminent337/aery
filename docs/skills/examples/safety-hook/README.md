...
An `aery` extension that demonstrates `tool_call` blocking. It intercepts `bash` tool calls and returns `{ block: true, reason: "..." }` when the command contains `rm -rf /` with normal whitespace, preventing the tool from executing.

## What it demonstrates

- `aery.on("tool_call", ...)` — pre-execution interception
...
```
cp -r . ~/.aery/agent/extensions/safety-hook
```

Restart `aery`. The hook is active for all sessions.

Or load once:

```
aery --extension ./safety-hook
```
