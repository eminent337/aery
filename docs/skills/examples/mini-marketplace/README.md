...
A minimal `aery` marketplace catalog that demonstrates the `marketplace.json` format. It lists one plugin (`my-plugin`) using a relative path source.
...
```
aery plugin marketplace add ./docs/skills/examples/mini-marketplace
aery plugin install my-plugin@example-marketplace
```
...
mini-marketplace/
  ...
  README.md
  my-plugin/
    package.json          ← aery.extensions manifest
