...
description: Use when creating a new aery marketplace. Covers marketplace.json schema, source types, install commands, and publishing.
---
...
```json
{
  ...
  "metadata": {
    "description": "Official Acme plugins for aery"
  },
  "plugins": [
    {
      "name": "acme-linter",
    ...
    },
    {
      ...
      "source": {
        "source": "github",
        "repo": "acme-corp/aery-deploy-plugin",
        "ref": "main"
      }
    }
  ]
...
```
my-plugin/
  package.json          ← required: declares aery.extensions entry points
  src/
...
```
aery plugin marketplace add owner/repo
aery plugin install name@marketplace-name
```

Scope behavior:

- **user** (default) — installed in `~/.aery/plugins/installed_plugins.json`, available in all projects
- **project** — installed in `<project>/.aery/plugins/installed_plugins.json`, available only in that project
