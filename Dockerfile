...
#                     + /usr/local/bin/aery shim
#   pi-runtime      — pi-base + pi source + bun install      (DEFAULT, runnable)
#
# Build:
#     docker build -t aery/pi:dev .                          # default = pi-runtime
#     docker build --target pi-base -t aery/pi-base:dev .    # base for derived images
#
# Run:
#     docker run --rm aery/pi:dev --help
#     docker run --rm -it -v "$PWD":/work aery/pi:dev cli    # interactive aery
#
# Consume as a base in another Dockerfile (see Dockerfile.robomp):
#     ARG PI_BASE=aery/pi:dev
...
RUN --mount=type=cache,target=/root/.cargo/registry \
    ...
    mkdir -p /out; \
    cp packages/aery-engine/native/pi_natives.linux-*.node /out/

############################
# 2) wheel-builder — aery-rpc wheel
############################
...
WORKDIR /src
COPY python/aery-rpc /src
RUN python -m build --wheel --outdir /out

############################
# 3) pi-base — python + bun + rustup + natives + omp_rpc + aery shim
#
...
RUN printf '%s\n' \
    ...
    > /usr/local/bin/aery \
    && chmod +x /usr/local/bin/aery

############################
# 4) pi-runtime — pi-base + pi source + bun install (DEFAULT)
#
# A self-contained, runnable aery image. `docker run aery/pi:dev --help`
...
ENTRYPOINT ["/usr/bin/tini", "--", "/usr/local/bin/aery"]
CMD ["--help"]
