...
const rustDir = path.join(repoRoot, "crates/aery-engine");
...
async function resolveBuiltAddonPath(outputDir: string, canonicalFilename: string): Promise<string> {
	...
	const generatedCandidates = entries.filter(entry => {
		if (!entry.startsWith(`aery_engine.${targetPlatform}-${targetArch}`) || !entry.endsWith(".node")) {
			return false;
		}
		return true;
	});
...
const canonicalAddonFilename = `aery_engine.${targetPlatform}-${targetArch}${variantSuffix}.node`;
const canonicalAddonPath = path.join(nativeDir, canonicalAddonFilename);

console.log(`Building aery-engine for ${targetPlatform}-${targetArch}${variantSuffix}${profileSuffix}…`);
