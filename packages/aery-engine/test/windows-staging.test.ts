/**
 * Regression for the Windows `bun install -g` update path: when an `aery`
 * process is running, bun cannot overwrite a locked
 * `node_modules/@aryee337/aery-engine/native/aery_engine.win32-x64.node` during
 ...
 *      (per-package-version cache under `~/.aery/natives/<version>/`) so the
 ...
 *      contention.
 *
 * Both behaviors are off in workspace dev (`bun --cwd=packages/aery-engine run
...
const winNodeModulesNativeDir = "C:\\Users\\Admin\\node_modules\\@aery\\aery-engine\\native";
const winWorkspaceNativeDir = "C:\\Users\\Admin\\dev\\aery\\packages\\natives\\native";
const posixNodeModulesNativeDir = "/home/u/proj/node_modules/@aryee337/aery-engine/native";
...
describe("windows native addon staging", () => {
	it("stages only on Windows node_modules installs", () => {
		...
		// otherwise rebuilds via `bun --cwd=packages/aery-engine run build` would be
	...
	it("prepends versionedDir candidates ahead of node_modules when staging on Windows", () => {
		const versionedDir = "C:\\Users\\Admin\\.aery\\natives\\15.0.1";
		const userDataDir = "C:\\Users\\Admin\\AppData\\Local\\aery";
		...
		const versionedBaseline = path.join(versionedDir, "aery_engine.win32-x64-baseline.node");
		const versionedDefault = path.join(versionedDir, "aery_engine.win32-x64.node");
		const nodeModulesBaseline = path.join(winNodeModulesNativeDir, "aery_engine.win32-x64-baseline.node");
		...
		// addon anyway).
		const userDataBaseline = path.join(userDataDir, "aery_engine.win32-x64-baseline.node");
	...
	it("falls back to the node_modules-only candidate list when staging is off", () => {
		...
		const versionedDir = "/home/u/.aery/natives/15.0.1";
		const candidates = resolveLoaderCandidates({
		...
		const nodeModulesBaseline = path.join(posixNodeModulesNativeDir, "aery_engine.linux-x64-baseline.node");
		expect(candidates).not.toContain(versionedBaseline);
		expect(candidates).toContain(nodeModulesBaseline);
	});
});

describe("aery-engine version sentinel", () => {
	it("Rust `js_name` matches the package version", async () => {
		// The JS loader (`packages/aery-engine/native/index.js`) computes its expected
		...
		const libRs = await Bun.file(path.join(import.meta.dir, "../../../crates/aery-engine/src/lib.rs")).text();
		const sentinelMatch = libRs.match(/js_name = "(__aeryNativesV[A-Za-z0-9_]+)"/);
		expect(sentinelMatch, 'Rust sentinel `js_name = "__aeryNativesV…"` not found in lib.rs').not.toBeNull();
		const expected = `__aeryNativesV${packageJson.version.replace(/[^A-Za-z0-9]/g, "_")}`;
