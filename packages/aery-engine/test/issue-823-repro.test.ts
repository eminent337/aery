/**
 * Regression for https://github.com/eminent337/aery/issues/823.
 *
 * On WSL (and any host where the user moves the standalone binary away from the
 * build-time native artifacts), the compiled `aery` binary fails to load
 * `aery_engine.linux-x64-*.node`. Root cause: the old loader's
 ...
 * `execDir`. On WSL with `~/.local/bin/aery` and no sibling `.node` file, this
...
describe("issue 823: standalone-binary native loader path resolution", () => {
	it("detects compiled-binary mode from embedded-addon presence when env and url markers are absent", () => {
			...
			detectCompiledBinary({
				embeddedAddon: {
					...
					files: [
						{
							variant: "modern",
							filename: "aery_engine.linux-x64-modern.node",
							filePath: "/$bunfs/root/packages/aery-engine/native/aery_engine.linux-x64-modern.node",
						},
					],
				},
				env: {},
				importMetaUrl: "/home/u/build-host/packages/aery-engine/native/index.js",
			}),
		).toBe(true);
		...
		expect(
			detectCompiledBinary({
				embeddedAddon: null,
				env: {},
				importMetaUrl: "/home/u/dev/packages/aery-engine/native/index.js",
			}),
		).toBe(false);
	...
	it("places embedded-extracted candidates ahead of build-host candidates for linux-x64 standalone", () => {
		const versionedDir = "/home/u/.aery/natives/14.5.2";
		const userDataDir = "/home/u/.local/bin";
		const nativeDir = "/build-host/packages/aery-engine/native";
		...
		const versionedModern = path.join(versionedDir, "aery_engine.linux-x64-modern.node");
		const versionedBaseline = path.join(versionedDir, "aery_engine.linux-x64-baseline.node");
		const userDataModern = path.join(userDataDir, "aery_engine.linux-x64-modern.node");
		const buildHostModern = path.join(nativeDir, "aery_engine.linux-x64-modern.node");

		// Versioned cache and user-data dir candidates must exist for compiled binaries —
		// these are where the embedded-addon extraction lands (~/.aery/natives/<v>) and where
		// `aery update` writes the standalone binary on linux (~/.local/bin).
	...
	it("does not probe user-data candidates when running outside a standalone binary", () => {
		...
		const candidates = resolveLoaderCandidates({
			...
			nativeDir: "/repo/packages/aery-engine/native",
			execDir: "/usr/bin",
			versionedDir,
			userDataDir,
		});
		expect(candidates).not.toContain(path.join(versionedDir, "aery_engine.linux-x64-baseline.node"));
		expect(candidates).not.toContain(path.join(userDataDir, "aery_engine.linux-x64-baseline.node"));
	});

	it("prefers platform leaf package candidates ahead of core nativeDir candidates on npm installs", () => {
		const leafPackageDir = "/app/node_modules/@aryee337/aery-engine-linux-x64";
		const nativeDir = "/app/node_modules/@aryee337/aery-engine/native";
		const candidates = resolveLoaderCandidates({
			addonFilenames: getAddonFilenames({ tag: "linux-x64", arch: "x64", variant: "baseline" }),
			isCompiledBinary: false,
			leafPackageDir,
			...
			versionedDir: "/home/u/.aery/natives/15.5.15",
			userDataDir: "/home/u/.local/bin",
		});

		const leafBaseline = path.join(leafPackageDir, "aery_engine.linux-x64-baseline.node");
		const coreBaseline = path.join(nativeDir, "aery_engine.linux-x64-baseline.node");
	...
	it("keeps Windows staging ahead of leaf package and core nativeDir candidates", () => {
		const versionedDir = "/home/u/.aery/natives/15.5.15";
		const leafPackageDir = "/app/node_modules/@aryee337/aery-engine-win32-x64";
		const nativeDir = "/app/node_modules/@aryee337/aery-engine/native";
		const candidates = resolveLoaderCandidates({
			...
			execDir: "/app/node_modules/.bin",
			versionedDir,
			userDataDir: "/home/u/AppData/Local/aery",
		});

		const stagedBaseline = path.join(versionedDir, "aery_engine.win32-x64-baseline.node");
		const leafBaseline = path.join(leafPackageDir, "aery_engine.win32-x64-baseline.node");
		const coreBaseline = path.join(nativeDir, "aery_engine.win32-x64-baseline.node");
	...
	it("keeps the development candidate list unchanged when no leaf package is installed", () => {
		const nativeDir = "/repo/packages/aery-engine/native";
	...
	it("extracts all bundled native variants from one gzip archive and skips current files", async () => {
		...
		try {
			...
			const modernFilename = "aery_engine.linux-x64-modern.node";
			const baselineFilename = "aery_engine.linux-x64-baseline.node";
			await Bun.write(
				archivePath,
