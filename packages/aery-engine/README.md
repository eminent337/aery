...
```typescript
import { grep, find, encodeSixel } from "@aryee337/aery-engine";
...
`@aryee337/aery-engine` publishes a small core package plus generated
platform-specific optional dependency packages:

```
crates/aery-engine/       # Rust source (workspace member)
...
native/                  # Core loader files and local/CI native build outputs
  ...
  aery_engine.<platform>-<arch>-modern.node   # x64 modern ISA (local/CI artifact)
  aery_engine.<platform>-<arch>-baseline.node # x64 baseline ISA (local/CI artifact)
  aery_engine.<platform>-<arch>.node          # non-x64 build artifact
npm/<platform>-<arch>/   # Generated at publish time, not committed
  package.json           # @aryee337/aery-engine-<platform>-<arch>
