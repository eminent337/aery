...
/**
 * Native addon loader for `@aryee337/aery-engine`.
 *
 * Owns every step between "Node imports `native/index.js`" and "the right
 * `aery_engine.<platform>-<arch>*.node` is required, validated, and returned":
...
function getNativesDir() {
	...
	if (xdgDataHome && fs.existsSync(path.join(xdgDataHome, "aery"))) {
		return path.join(xdgDataHome, "aery", "natives");
	}
	return path.join(os.homedir(), ".aery", "natives");
}

function resolveLeafPackageDir(platformTag) {
	try {
		const require_ = createRequire(import.meta.url);
		return path.dirname(require_.resolve(`@aryee337/aery-engine-${platformTag}/package.json`));
	} catch {
		return null;
	}
}
...
// =========================================================================
// Pure helpers — re-exported for unit tests in `packages/aery-engine/test/`.
// =========================================================================
...
export function getAddonFilenames({ tag, arch, variant }) {
	const defaultFilename = `aery_engine.${tag}.node`;
	if (arch !== "x64" || !variant) return [defaultFilename];
	const baselineFilename = `aery_engine.${tag}-baseline.node`;
	const modernFilename = `aery_engine.${tag}-modern.node`;
	if (variant === "modern") {
 ...
 * into the per-version cache directory (`~/.aery/natives/<version>/`) before loading.
 *
 * Windows-only safety net for `bun install -g` updates: when a previous `aery`
 * process is running, bun cannot overwrite the locked `.node` inside
 * `node_modules/@aryee337/aery-engine/native/`, leaving an old binary next to a
 * newer `index.js` and producing `<sym> is not a function` crashes on the next
 * launch. Staging into the version-pinned cache:
 *   1. Gives every package version its own filesystem path, so concurrent aery
...
function validateLoadedBindings(ctx, bindings, candidate) {
	// In workspace dev (running out of `packages/aery-engine/native/` rather than a
	// `node_modules` install or a compiled bundle) the local `.node` only gains
	// the renamed sentinel after `bun --cwd=packages/aery-engine run build`. Skip
	...
	throw new Error(
		`Loaded ${candidate} but it does not expose the @aryee337/aery-engine@${ctx.packageVersion} ` +
...
function buildHelpMessage(ctx) {
	if (ctx.isCompiledBinary) {
		...
		const downloadHints = ctx.addonFilenames
			.map(filename => {
				const downloadUrl = `https://github.com/eminent337/aery/releases/latest/download/${filename}`;
				const targetPath = path.join(ctx.versionedDir, filename);
	...
	return (
		"If installed via npm/bun, try reinstalling: bun install @aryee337/aery-engine\n" +
		"If developing locally, build with: bun --cwd=packages/aery-engine run build\n" +
		"Optional x64 variants: TARGET_VARIANT=baseline|modern bun --cwd=packages/aery-engine run build"
	);
}
...
function initLoaderContext() {
	...
	const userDataDir =
		process.platform === "win32"
			? path.join(process.env.LOCALAPPDATA || path.join(os.homedir(), "AppData", "Local"), "aery")
	...
	// the package version (`__aeryNativesV{major}_{minor}_{patch}`).
	// `scripts/release.ts` bumps the name in `crates/aery-engine/src/lib.rs` in
	...
	const versionSentinelExport = `__aeryNativesV${packageVersion.replace(/[^A-Za-z0-9]/g, "_")}`;
...
export function loadNative() {
	...
	throw new Error(
		`Failed to load aery_engine native addon for ${ctx.addonLabel}.\n\nTried:\n${details}\n\n${buildHelpMessage(ctx)}`,
	);
}
