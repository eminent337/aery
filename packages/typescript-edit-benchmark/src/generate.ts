#!/usr/bin/env bun
/**
 * Generate edit benchmark cases from TypeScript sources in a standard repo.
 *
...
using DEFAULT_SOURCE_REPO_DIR = TempDir.createSync("@aery-source");
...
const DEFAULT_SOURCE_REPO_URL = "https://github.com/badlogic/aery-monorepo.git";
...
async function ensureSourceRepo(typescriptDir: string): Promise<void> {
	...
	console.log(`Cloning aery-monorepo repository to ${typescriptDir}…`);
	...
	if (result.exitCode !== 0) {
		...
		throw new Error(`Failed to clone aery-monorepo: ${stderr.trim()}`);
	}
