	...
	#renderToken(token: Token, width: number, nextTokenType?: string, styleContext?: InlineStyleContext): string[] {
		...
		switch (token.type) {
			case "heading": {
				...
				const osc66Start = `\x1b]66;h${headingLevel}\x07`;
				const osc66End = `\x1b]66;\x07`;

				let styledHeading: string;
				if (headingLevel === 1) {
					styledHeading = this.#theme.heading(
						this.#theme.bold(this.#theme.underline(osc66Start + headingText + osc66End)),
					);
				} else if (headingLevel === 2) {
					styledHeading = this.#theme.heading(this.#theme.bold(osc66Start + headingText + osc66End));
				} else {
					styledHeading = this.#theme.heading(
						this.#theme.bold(osc66Start + headingPrefix + headingText + osc66End),
					);
				}
