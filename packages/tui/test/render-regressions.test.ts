...
import { type Component, CURSOR_MARKER, type Focusable, TUI } from "@aryee337/aery-tui";
...
describe("TUI terminal-state regressions", () => {
	...
	describe("scrollback integrity", () => {
		...
		it("leaves the parent shell prompt directly after short content on stop", async () => {
			...
			const tui = new TUI(term);
			let stopped = false;
			tui.addChild(new MutableLinesComponent(["aery0", "aery1", "aery2"]));
			...
			try {
				...
				expect(visible(term)).toEqual(["aery0", "aery1", "aery2", "bash$", ""]);
			} finally {
				if (!stopped) tui.stop();
			}
		});
