...
import { z } from "@aryee337/aery-ai";
import { createMockModel } from "@aryee337/aery-ai/providers/mock";
import type { EventStream } from "@aryee337/aery-ai/utils/event-stream";
import { agentLoop } from "@aryee337/aery-core/agent-loop";
import {
...
} from "@aryee337/aery-core/telemetry";
import type { AgentContext, AgentEvent, AgentLoopConfig, AgentMessage, AgentTool } from "@aryee337/aery-core/types";
...
describe("agent-loop OTEL instrumentation", () => {
	...
	it("emits invoke_agent → chat hierarchy with OTEL and aery.gen_ai extension attributes", async () => {
	...
	it("emits execute_tool spans parented to invoke_agent (not chat) per semconv", async () => {
		...
		// aery.gen_ai.agent.step.count counts chat completions
	...
	it("invokes costEstimator and stamps aery.gen_ai.cost.estimated_usd", async () => {
		const mock = createMockModel({
			...MOCK_IDENT,
			responses: [
				{
	...
	it("emits aery.gen_ai.cost.unavailable_reason when the estimator declines", async () => {
		const mock = createMockModel({
			...MOCK_IDENT,
...
describe("ChatUsageEvent.headers and aery.gen_ai.gateway.* span attributes", () => {
	...
	it("auto-stamps aery.gen_ai.gateway.* on the chat span when LiteLLM headers are present", async () => {
		const mock = createMockModel({
			...MOCK_IDENT,
			responses: [
				{
