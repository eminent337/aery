...
import type { AssistantMessage, Message } from "@aryee337/aery-ai";
import { z } from "@aryee337/aery-ai";
import { createMockModel } from "@aryee337/aery-ai/providers/mock";
import { agentLoop, agentLoopDetailed } from "@aryee337/aery-core/agent-loop";
import {
...
} from "@aryee337/aery-core/run-collector";
import { EXECUTE_TOOL_STATUS_ATTR, GenAIAttr, PiGenAIAggregateAttr } from "@aryee337/aery-core/telemetry";
import type { AgentEvent, AgentLoopConfig, AgentMessage, AgentTool } from "@aryee337/aery-core/types";
import type {
	AttributeValue,
...
describe("AgentRunSummary aggregation", () => {
	...
	it("populates aggregate aery.gen_ai.agent.* attributes on the invoke_agent span", async () => {
		const tracer = new RecordingTracer();
		const tool = buildTool({ name: "alpha", behavior: "ok" });
		const mock = createMockModel({
			responses: [
