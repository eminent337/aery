...
import type { AssistantMessage, Model, Usage } from "@aryee337/aery-ai";
...
import type { AgentMessage } from "@aryee337/aery-core/types";
...
describe("handoff oneshot telemetry", () => {
	it("tags generateHandoff with aery.gen_ai.oneshot.kind = handoff and toolChoice = none", async () => {
...
describe("branch summary oneshot telemetry", () => {
	it("tags generateBranchSummary with aery.gen_ai.oneshot.kind = branch_summary", async () => {
		const spy = vi
			.spyOn(ai, "completeSimple")
