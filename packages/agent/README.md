...
Stateful agent with tool execution and event streaming. Built on `@aryee337/aery-ai`.

## Installation

```bash
npm install @aryee337/aery
```

## Quick Start

```typescript
import { Agent } from "@aryee337/aery";
...
Define tools using `AgentTool` with a Zod parameter schema (via `z` from `@aryee337/aery-ai`).
...
```typescript
import { Agent, streamProxy } from "@aryee337/aery";
...
```typescript
import { agentLoop, agentLoopContinue } from "@aryee337/aery";
...
`execute_tool` spans carry `aery.gen_ai.tool.status` ∈
