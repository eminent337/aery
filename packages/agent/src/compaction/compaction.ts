...
function extractFileOperations(
	...
	// Collect from previous compaction's details (if aery-generated)
 ...
 * (aery-native gateway) does not populate `errorStatus`, hence the legacy
...
export interface SummaryOptions {
	 ...
	 * `aery.gen_ai.oneshot.kind` (`compaction_summary`, `compaction_short_summary`,
...
export interface HandoffOptions {
	 ...
	 * wrapped in an OTEL chat span tagged with `aery.gen_ai.oneshot.kind = "handoff"`.
	 */
	telemetry?: AgentTelemetry;
	/**
	 * Active session thinking level. Threaded from `agent-session.ts` so
