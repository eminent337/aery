...
import { logger, prompt } from "@aryee337/aery-utils";
...
export const createAutoresearchExtension: ExtensionFactory = api => {
	...
	api.registerTool(createInitExperimentTool({ dashboard, getRuntime, aery: api }));
	api.registerTool(createRunExperimentTool({ dashboard, getRuntime, aery: api }));
	api.registerTool(createLogExperimentTool({ dashboard, getRuntime, aery: api }));
	api.registerTool(createUpdateNotesTool({ dashboard, getRuntime, aery: api }));
