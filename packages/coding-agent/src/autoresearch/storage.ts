...
import { getAutoresearchDbPath, getAutoresearchProjectDir, logger } from "@aryee337/aery-utils";
 ...
 *
 * Used to key per-project autoresearch state under `~/.aery/autoresearch/`.
...
async function resolveAutoresearchPaths(cwd: string): Promise<{ dbPath: string; projectDir: string }> {
	const override = process.env.AERY_AUTORESEARCH_DB_DIR;
