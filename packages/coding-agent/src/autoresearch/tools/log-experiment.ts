...
export function createLogExperimentTool(
	...
	return {
		...
		async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
			...
			const branchName = await getCurrentAutoresearchBranch(options.aery, ctx.cwd);
			...
			if (finalState.maxExperiments !== null && segmentRunCount >= finalState.maxExperiments) {
				...
				options.aery.appendEntry(
					"autoresearch-control",
					runtime.goal ? { mode: "off", goal: runtime.goal } : { mode: "off" },
				);
				await options.aery.setActiveTools(
					options.aery.getActiveTools().filter(name => !EXPERIMENT_TOOL_NAMES.includes(name)),
				);
			}
