...
import type { WorkProfile } from "@aryee337/aery-engine";
import { APP_NAME, getLogPath, getLogsDir, getReportsDir, isEnoent } from "@aryee337/aery-utils";
...
export async function createReportBundle(options: ReportBundleOptions): Promise<ReportBundleResult> {
	...
	const outputPath = path.join(reportsDir, `aery-report-${timestamp}.tar.gz`);
