...
// `__aery_import__` helper, which resolves the specifier against the active session cwd. A real
...
function buildAeryImportCall(sourceLiteral: string, optionsLiteral: string | undefined): string {
	// Route every static import through the worker-injected `__aery_import__` helper so the
	// specifier resolves against the session cwd (and `with`-attribute imports keep working).
	return optionsLiteral ? `__aery_import__(${sourceLiteral}, ${optionsLiteral})` : `__aery_import__(${sourceLiteral})`;
}
...
function buildOptionsLiteral(node: BabelImportDeclaration): string | undefined {
	...
	// Native dynamic import takes options as `{ with: { ... } }`. `__aery_import__` forwards the
...
function rewriteImportNode(node: BabelImportDeclaration): string {
	...
	const importCall = buildAeryImportCall(sourceLiteral, optionsLiteral);
...
export function rewriteImports(code: string): string {
	...
	// Top-level static `import` declarations become `await __aery_import__(...)` calls.
	...
	// Dynamic `import(...)` expressions (anywhere) get their callee swapped for `__aery_import__`
	...
	walkNodes(ast, node => {
		...
		edits.push({ start: callee.start, end: callee.end, text: "__aery_import__" });
	});
...
export function rewriteDynamicImports(code: string, callee = "__aery_import__"): string {
	if (!code.includes("import")) return code;
	const ast = parseProgram(code);
	if (!ast) return code;
...
function returnFinalExpression(code: string): { source: string; returned: boolean } {
	...
	if (last?.type === "ExpressionStatement") {
		...
		return { source: `${prefix}__aery_set_final_expr__((${trimmedStatement}));${suffix}`, returned: true };
	}
	if (last?.type === "ReturnStatement") {
		// Top-level `return value;` is otherwise swallowed: it forces the cell into an async IIFE
		// wrapper that discards the returned value. Rewrite into `__aery_set_final_expr__((expr))`
		...
		return { source: `${prefix}__aery_set_final_expr__((${expr}));${suffix}`, returned: true };
	}
