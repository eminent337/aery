...
export interface RuntimeOptions {
	...
	sessionId: string;
	/**
	 * Extra globals installed alongside `__aery_helpers__` / prelude. Use for stable, lifetime-
...
export class JsRuntime {
	...
	setCwd(cwd: string): void {
		this.#cwd = cwd;
		const session = (globalThis as { __aery_session__?: { cwd?: string } }).__aery_session__;
	...
	#install(extraGlobals: Record<string, unknown> | undefined): void {
		const injected: Record<string, unknown> = {
			__aery_session__: { cwd: this.#cwd, sessionId: this.sessionId },
			__aery_helpers__: this.helpers,
			__aery_call_tool__: async (name: string, args: unknown) => {
				const hooks = this.#activeHooks("tool");
				if (!hooks) return undefined;
				return await hooks.callTool(name, args);
			},
			__aery_import__: async (source: string, options?: ImportCallOptions) => {
			...
			__aery_import_from__: async (moduleUrl: string, source: string, options?: ImportCallOptions) => {
				const resolved = await this.#moduleLoader.resolveForModule(moduleUrl, source, this.#activeCwd());
				if (resolved.mode === "local") return resolved.value;
				const target = resolved.target;
				return options !== undefined ? await import(target, options) : await import(target);
			},
			__aery_get_require__: (moduleUrl?: string) => this.#activeRequire(moduleUrl),
			__aery_get_filename__: (moduleUrl?: string) => this.#moduleFilename(moduleUrl),
			__aery_get_dirname__: (moduleUrl?: string) => this.#moduleDirname(moduleUrl),
			__aery_emit_status__: (op: string, data: Record<string, unknown> = {}) => {
				const event: JsStatusEvent = { op, ...data };
				this.#activeHooks("emitStatus")?.onDisplay({ type: "status", event });
			},
			__aery_log__: (level: string, ...args: unknown[]) => {
				const prefix = level === "error" ? "[error] " : level === "warn" ? "[warn] " : "";
				const text = `${prefix}${formatConsoleArgs(args)}`;
				this.#activeHooks("log")?.onText(text.endsWith("\n") ? text : `${text}\n`);
			},
			__aery_table__: (...args: unknown[]) => {
				...
				((tableConsole as any).table as (...a: unknown[]) => void)(...args);
				hooks.onText(buffer.endsWith("\n") ? buffer : `${buffer}\n`);
			},
			__aery_display__: (value: unknown) => this.displayValue(value),
			__aery_set_final_expr__: (value: unknown) => {
