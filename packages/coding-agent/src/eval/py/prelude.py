...
# AERY prelude helpers (loaded once into the runner namespace)
if "__aery_prelude_loaded__" not in globals():
    __aery_prelude_loaded__ = True
    from pathlib import Path
    import os, json, math

    # __aery_display is injected by runner.py before the prelude executes; it
    # mirrors IPython's display() semantics with the same MIME bundle output.
    _aery_display = __aery_display  # type: ignore[name-defined]
    ...
    def _emit_status(op: str, **data):
        ...
        _aery_display({"application/x-aery-status": {"op": op, **data}}, raw=True)
    ...
    def _bridge_call(name: str, args: dict):
        ...
        _run_id_getter = globals().get("__aery_current_run_id__")
        _run_id = _run_id_getter() if callable(_run_id_getter) else globals().get("__aery_run_id__")
        payload = json.dumps(
    ...
    def phase(title):
        ...
        globals()["__aery_current_phase__"] = str(title)
