"""AERY Python runner — subprocess wrapper used by the coding-agent host.
...
_CURRENT_RID: contextvars.ContextVar[str | None] = contextvars.ContextVar("aery_current_rid", default=None)
...
def transform_cell(source: str) -> str:
    ...
    Rules
    -----
    * ``%name args``              -> ``__aery_magic("name", "args")``
    * ``var = %name args``        -> ``var = __aery_magic("name", "args")``
    * ``!cmd``                    -> ``__aery_shell("cmd")``
    * ``var = !cmd``              -> ``var = __aery_shell("cmd")``
    * ``%%name args\\n<body>``    -> ``__aery_magic_cell("name", "args", "<body>")``
    ...
    while i < len(lines):
        ...
        if stripped.startswith("%%"):
            ...
            body = "\n".join(body_lines)
            out.append(
                f"{indent}__aery_magic_cell({_quote_arg(name)}, {_quote_arg(args)}, {_quote_arg(body)})"
            )
            return "\n".join(out)
        ...
        if stripped.startswith("%") and not stripped.startswith("%%"):
            ...
            name, args = head
            out.append(f"{indent}__aery_magic({_quote_arg(name)}, {_quote_arg(args)})")
            i += consumed
            continue
        ...
        if stripped.startswith("!"):
            ...
            out.append(f"{indent}__aery_shell({_quote_arg(cmd)})")
            i += consumed
            continue
        ...
        if m:
            ...
            if rhs.startswith("!"):
                cmd = rhs[1:].strip()
                out.append(f"{m.group('indent')}{m.group('lhs').rstrip()} = __aery_shell({_quote_arg(cmd)})")
                i += 1
                continue
            if rhs.startswith("%") and not rhs.startswith("%%"):
                ...
                name, args = head
                out.append(
                    f"{m.group('indent')}{m.group('lhs').rstrip()} = __aery_magic({_quote_arg(name)}, {_quote_arg(args)})"
                )
                i += 1
                continue
...
def _emit_status(op: str, **data: Any) -> None:
    bundle = {"application/x-aery-status": {"op": op, **data}}
    rid = _CURRENT_RID.get()
    if rid is None:
        return
...
def __aery_magic(name: str, args: str) -> Any:
...
def __aery_magic_cell(name: str, args: str, body: str) -> Any:
...
def __aery_shell(cmd: str) -> _ShellResult:
    proc = subprocess.run(
        cmd,
        shell=True,
        stdout=subprocess.PIPE,
...
def __aery_display(value: Any, *, raw: bool = False, kind: str = "display") -> None:
    if raw:
...
def _install_builtins(ns: dict) -> None:
    ns["display"] = __aery_display
    ns["__aery_display"] = __aery_display
    ns["__aery_magic"] = __aery_magic
    ns["__aery_magic_cell"] = __aery_magic_cell
    ns["__aery_shell"] = __aery_shell
    ns["__aery_current_run_id__"] = lambda: _CURRENT_RID.get()
...
def _start_parent_watchdog() -> None:
    ...
    thread = threading.Thread(target=watch, name="aery-parent-watchdog", daemon=True)
    thread.start()
...
async def _handle_request_async(req: dict) -> None:
    ...
    _STATE.user_ns["__aery_run_id__"] = rid
...
async def _main_async() -> None:
    ...
    reader = threading.Thread(target=_read_stdin, args=(loop, queue, stdin), name="aery-stdin-reader", daemon=True)
    reader.start()
