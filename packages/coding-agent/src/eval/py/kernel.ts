...
import { $flag, isBunTestRuntime, logger, Snowflake } from "@aryee337/aery-utils";
...
const RUNNER_CACHE_DIR = path.join(os.tmpdir(), "aery-python-runner");
...
function buildInitScript(cwd: string, env?: Record<string, string | undefined>): string {
	...
	return [
		"import os, sys",
		`__aery_cwd = ${JSON.stringify(cwd)}`,
		"os.chdir(__aery_cwd)",
		`__aery_env = ${JSON.stringify(envPayload)}`,
		"for __aery_key, __aery_val in __aery_env.items():\n    os.environ[__aery_key] = __aery_val",
		"if __aery_cwd not in sys.path:\n    sys.path.insert(0, __aery_cwd)",
	].join("\n");
}
