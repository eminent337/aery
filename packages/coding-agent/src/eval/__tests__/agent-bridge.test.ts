...
describe("agent() through eval runtimes", () => {
	...
	it("exposes agent() in JavaScript and parses structured output", async () => {
		using tempDir = TempDir.createSync("@aery-eval-agent-js-");
	...
	it("bounds JavaScript parallel() by the task.maxConcurrency setting while preserving order", async () => {
		using tempDir = TempDir.createSync("@aery-eval-agent-js-parallel-");
	...
	it("propagates JavaScript parallel() rejections", async () => {
		using tempDir = TempDir.createSync("@aery-eval-agent-js-reject-");
	...
	it("exposes agent() in the Python runtime", async () => {
		using tempDir = TempDir.createSync("@aery-eval-agent-py-");
	...
	it("bounds Python parallel() by the task.maxConcurrency setting while preserving order", async () => {
		using tempDir = TempDir.createSync("@aery-eval-agent-py-parallel-");
		const settings = Settings.isolated({
			"async.enabled": false,
			"task.isolation.mode": "none",
			"task.enableLsp": true,
	...
	it("streams enriched agent progress through onStatus before the cell finishes", async () => {
		using tempDir = TempDir.createSync("@aery-eval-agent-progress-");
	...
	it("keeps the idle watchdog armed while a quiet agent() runs past the budget", async () => {
		using tempDir = TempDir.createSync("@aery-eval-agent-heartbeat-");
	...
	it("does not let agent() progress snapshots re-arm the watchdog without a heartbeat", async () => {
		using tempDir = TempDir.createSync("@aery-eval-agent-progress-no-rearm-");
