...
import type { Api, AssistantMessage, Model } from "@aryee337/aery-ai";
import * as ai from "@aryee337/aery-ai";
import { Effort } from "@aryee337/aery-ai";
...
describe("runEvalLlm", () => {
	...
	it("prefers the session active model for the default tier, falling back to aery/default", async () => {
		const spy = vi.spyOn(ai, "completeSimple").mockResolvedValue(assistant({ text: "ok" }));
...
describe("llm() through eval runtimes", () => {
	...
	it("exposes llm() in the JavaScript runtime", async () => {
		using tempDir = TempDir.createSync("@aery-eval-llm-js-");
	...
	it("parses structured llm() output in the JavaScript runtime", async () => {
		using tempDir = TempDir.createSync("@aery-eval-llm-js-struct-");
		const sessionFile = path.join(tempDir.path(), "session.jsonl");
	...
	it("exposes llm() in the Python runtime", async () => {
		using tempDir = TempDir.createSync("@aery-eval-llm-py-");
		const sessionFile = path.join(tempDir.path(), "session.jsonl");
	...
	it("parses structured llm() output in the Python runtime", async () => {
		using tempDir = TempDir.createSync("@aery-eval-llm-py-struct-");
		const sessionFile = path.join(tempDir.path(), "session.jsonl");
