import { THINKING_EFFORTS } from "@aryee337/aery-ai";
...
/**
 * Ordered layout sections per tab. Settings are grouped by their `group` field;
...
export const SETTINGS_SCHEMA = {
	...
	// Auth broker — credentials proxied through a remote `aery auth-broker serve`
	// host. Hidden from the UI; populate via env vars or hand-edited config.yml.
	// Env (`AERY_AUTH_BROKER_URL` / `AERY_AUTH_BROKER_TOKEN`) takes precedence so
	...
	"statusLine.separator": {
		type: "enum",
		values: ["powerline", "powerline-thin", "slash", "pipe", "block", "none", "ascii", "aery-bar"] as const,
	...
	"mnemopi.llmMode": {
		...
		ui: {
			...
			options: [
				...
				{ value: "smol", label: "Smol", description: "Use the configured aery-ai smol model" },
	...
	"hindsight.retainContext": { type: "string", default: "aery" },
	...
	"github.cache.enabled": {
		...
		ui: {
			...
			description: "Cache rendered issue/PR view output in ~/.aery/cache/github-cache.db so repeated reads are free",
		},
	},
	...
	"task.batch": {
		...
		ui: {
			tab: "tasks",
			group: "Subagents",
			label: "Batch Spawning",
			description: "Accept multiple parallel tasks (tasks[]) with shared context in a single call",
		},
	},
	...
	"providers.webSearch": {
		type: "enum",
		values: ["auto", "tavily", "exa", "parallel", "ddg"] as const,
		default: "auto",
		ui: {
			tab: "providers",
			group: "Services",
			label: "Web Search Provider",
			description: "Provider for web search tool (4-tier cascade: tavily → exa → parallel → ddg)",
			options: [
				{ value: "auto", label: "Auto (cascade)", description: "tavily → exa → parallel → ddg" },
				{ value: "tavily", label: "Tavily", description: "Requires TAVILY_API_KEY (1k req/mo free)" },
				{ value: "exa", label: "Exa", description: "Requires EXA_API_KEY" },
				{ value: "parallel", label: "Parallel", description: "Requires PARALLEL_API_KEY" },
				{ value: "ddg", label: "DuckDuckGo", description: "No API key required (fallback)" },
			],
		},
	},
	"providers.image": {
	...
	"providers.tinyModel": {
		...
		ui: {
			...
			group: "Tiny Model",
			label: "Tiny Model",
			description: "Session-title model: online aery/smol by default, or a local on-device model",
	...
	"dev.autoqaPush.endpoint": {
		type: "string",
		// Bundled QA collector — runs `/work/aery-www/autoqa` behind qa.aery.sh.
		// Override via `PI_AUTO_QA_PUSH_URL` or `dev.autoqaPush.endpoint`
		// in `config.yml` to point at a self-hosted instance.
		default: "https://qa.aery.sh/v1/grievances" as const,
		ui: {
			tab: "tools",
			group: "Developer",
			label: "Auto QA Push Endpoint",
			description: "Full URL that receives the JSON payload (default ships to https://qa.aery.sh/v1/grievances)",
		},
	},
