...
function loadModelsConfig(modelsPath: string): JsonObject {
	...
	try {
		...
		if (modelsPath.endsWith(".yml") || modelsPath.endsWith(".yaml")) {
			...
			parsed = YAML.parse(content) as JsonObject;
		} else {
			parsed = JSON.parse(stripJsonComments(content)) as JsonObject;
		}
		...
		throw new Error(`Failed to parse models file: ${error instanceof Error ? error.message : String(error)}`);
	}
}
