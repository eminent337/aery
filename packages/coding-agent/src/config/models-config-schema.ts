...
const ProviderConfigSchema = z.object({
	...
	/**
	 * Streaming transport override. When set to `"aery-native"`, aery dispatches
	 * every model under this provider via the auth-gateway's
	 * `POST /v1/aery/stream` endpoint instead of the per-provider SDK. The
	 * provider's `baseUrl` must point at a compatible `aery auth-gateway`
	 * and `apiKey` must carry the gateway bearer.
	 */
	transport: z.literal("aery-native").optional(),
});
