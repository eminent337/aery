...
import { fuzzyMatch } from "@aryee337/aery-tui";
...
function isSessionInheritedAgentPattern(value: string): boolean {
	return (
		value === DEFAULT_MODEL_ROLE || value === `${PREFIX_MODEL_ROLE}${DEFAULT_MODEL_ROLE}` || value === "aery/task"
	);
}
...
/**
 * Expand a role alias like "aery/smol" to the configured model string.
 */
...
export function resolveAgentModelPatterns(options: AgentModelPatternResolutionOptions): string[] {
	...
	if (configuredAgentPatterns.length > 0) {
		...
		if (singleAgentPattern === "aery/task") return configuredAgentPatterns;
	}
