/**
 * List and clean up agent-managed git worktrees under `~/.aery/wt`.
 */
...
export default class Worktree extends Command {
	static description = "List or clear agent-managed git worktrees (~/.aery/wt)";

	static aliases = ["wt"];

	static args = {
		// `list` (default) inspects the worktree dir; `clear` removes entries.
		// A positional action keeps `aery worktree` (the no-arg form) useful.
		action: Args.string({
	...
	static examples = [
		"aery worktree",
		"aery worktree list --json",
		"aery worktree clear",
		"aery worktree clear --dry-run",
		"aery worktree clear --all",
	];
