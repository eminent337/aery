/**
 * `aery install <target>` — top-level convenience over `aery plugin install` /
 * `aery plugin link`.
 *
 * The docs (aery.sh/docs/extension-authoring) advertise
 *
 *   aery install ./my-extension
 *
...
import { Args, Command, Flags } from "@aryee337/aery-utils/cli";
import { type PluginAction, type PluginCommandArgs, runPluginCommand } from "../cli/plugin-cli";
import { initTheme } from "../modes/theme/theme";

/**
 * Heuristic used to decide whether `aery install <target>` should `link` a
...
export default class Install extends Command {
	...
	async run(): Promise<void> {
		...
		if (targets.length === 0) {
			process.stderr.write("Usage: aery install <path | npm-spec | name@marketplace> [...]\n");
			process.exit(1);
		}
