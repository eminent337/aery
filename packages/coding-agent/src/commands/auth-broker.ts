/**
 * `aery auth-broker` — manage the aery credential vault.
 */
...
export default class AuthBroker extends Command {
	static description = "Manage the aery auth-broker (credential vault)";
	...
	static examples = [
		"# Boot the broker against the local SQLite store\n  aery auth-broker serve",
		"# Boot on a non-default port\n  aery auth-broker serve --bind=127.0.0.1:9000",
		"# Print the bearer token\n  aery auth-broker token",
		"# Rotate the bearer token\n  aery auth-broker token --regenerate",
		"# List supported OAuth providers\n  aery auth-broker list",
		"# Local login (run on the broker host)\n  aery auth-broker login anthropic",
		"# Interactive provider selection\n  aery auth-broker login",
		"# Remote login over SSH tunnel\n  aery auth-broker login anthropic --via=user@broker",
		"# Log out of a provider (interactive without provider arg)\n  aery auth-broker logout anthropic",
		"# Import a CLIProxyAPI auth dump\n  aery auth-broker import ~/.cliproxy/auth",
		"# Import a single CLIProxyAPI JSON, overriding the provider mapping\n  aery auth-broker import ~/.cliproxy/auth/claude-foo.json --provider anthropic",
		"# Preview a migration from local store + env vars to the configured broker\n  aery auth-broker migrate --from-local --include-env --dry-run",
		"# Apply the migration\n  aery auth-broker migrate --from-local --include-env",
		"# Health-check the configured remote broker\n  aery auth-broker status",
	];

	async run(): Promise<void> {
		const { args, flags } = await this.parse(AuthBroker);
		if (!args.action) {
			renderCommandHelp("aery", "auth-broker", AuthBroker);
			return;
		}
