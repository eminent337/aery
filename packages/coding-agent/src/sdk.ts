...
} from "@aryee337/aery-ai/providers/openai-codex-responses";
import {
	Agent,
	type AgentEvent,
	type AgentMessage,
...
export interface CreateAgentSessionOptions {
	...
	cwd?: string;
	/** Global config directory. Default: ~/.aery/agent */
	agentDir?: string;
	...
	/** Prompt templates. Default: discovered from cwd/.aery/prompts/ + agentDir/prompts/ */
...
export type { AgentSession } from "./session/agent-session";
 ...
 *
 * Broker mode: when `AERY_AUTH_BROKER_URL` is set, credentials are pulled from
 ...
 *
 * // With explicit model
 * import { getModel } from '@aryee337/aery-ai';
 * const { session } = await createAgentSession({
...
export async function createAgentSession(options: CreateAgentSessionOptions = {}): Promise<CreateAgentSessionResult> {
	...
	if (sessionManager.setOnSessionIndexed) {
		sessionManager.setOnSessionIndexed(sessionId => {
			eventBus.emit("SESSION_INDEXED", { sessionId });
		});
	}
	...
	try {
		...
		// Discover and load custom tools from .aery/tools/, .claude/tools/, etc.
		...
		inlineExtensions.push(createFermentExtension());
		inlineExtensions.push(createGuardsExtension());
