...
if (process.env.OTEL_EXPORTER_OTLP_ENDPOINT) {
	...
	// Cast to any to bypass strict type-checking on dynamic imports
	const provider = new (NodeTracerProvider as any)({
		resource: resources.resourceFromAttributes?.({
			"service.name": "aery-coding-agent",
		}),
	});
	provider.addSpanProcessor(
		new (SimpleSpanProcessor as any)(
	...
	const tracer = trace.getTracer("aery-coding-agent");
 ...
 * lightweight CLI runner from aery-utils.
 */
...
async function showHelp(config: CliConfig): Promise<void> {
	const { renderRootHelp } = await import("@aryee337/aery-utils/cli");
...
async function runSmokeTest(): Promise<void> {
	const { smokeTestSyncWorker } = await import("@aryee337/aery-stats");
