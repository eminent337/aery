...
import { CONFIG_DIR_NAME, getConfigAgentDirName, getProjectDir } from "@aryee337/aery-utils";
 ...
 * project with aery's own assets and was the source of issue #1423 (the host
 * project's `CHANGELOG.md` rendered as aery's startup changelog).
 */
...
/**
 * Path to aery's own `CHANGELOG.md`, or `undefined` when the package directory
 ...
 * User-level: ~/.aery/agent, ~/.claude, ~/.codex, ~/.gemini
 * Project-level: .aery, .claude, .codex, .gemini
 */
...
export interface ConfigDirEntry {
	path: string;
	source: string; // e.g., ".aery", ".claude"
	level: "user" | "project";
}

export interface GetConfigDirsOptions {
	/** Include user-level directories (~/.aery/agent/...). Default: true */
	user?: boolean;
	/** Include project-level directories (.aery/...). Default: true */
	project?: boolean;
 ...
 * // → [{ path: "~/.aery/agent/commands", source: ".aery", level: "user" }, ...]
 *
 * @example
 ...
 * Returns one entry per config base (.aery, .claude) - the nearest one found.
