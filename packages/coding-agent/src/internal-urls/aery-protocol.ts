/**
 * Protocol handler for aery:// URLs.
 *
 * Serves statically embedded documentation files bundled at build time.
 *
 * URL forms:
 * - aery:// - Lists all available documentation files
 * - aery://<file>.md - Reads a specific documentation file
 */
...
/**
 * Handler for aery:// URLs.
 *
 * Resolves documentation file names to their content, or lists available docs.
 */
export class AeryProtocolHandler implements ProtocolHandler {
	readonly scheme = "aery";
	...
	async #listDocs(url: InternalUrl): Promise<InternalResource> {
		...
		const listing = EMBEDDED_DOC_FILENAMES.map(f => `- [${f}](aery://${f})`).join("\n");
	...
	async #readDoc(filename: string, url: InternalUrl): Promise<InternalResource> {
		...
		if (path.isAbsolute(filename)) {
			throw new Error("Absolute paths are not allowed in aery:// URLs");
		}

		const normalized = path.posix.normalize(filename.replaceAll("\\", "/"));
		if (normalized === ".." || normalized.startsWith("../") || normalized.includes("/../")) {
			throw new Error("Path traversal (..) is not allowed in aery:// URLs");
		}
		...
		if (content === undefined) {
			...
			const suffix =
				suggestions.length > 0
					...
					: "\nUse aery:// to list available files.";
