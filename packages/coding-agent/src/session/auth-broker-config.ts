/**
 * Resolve auth-broker connection configuration for the local aery client.
 *
 * Precedence (highest first):
 *   1. `AERY_AUTH_BROKER_URL` / `AERY_AUTH_BROKER_TOKEN` env vars.
 *   2. `auth.broker.url` / `auth.broker.token` in `~/.aery/agent/config.yml`
 *      (hidden from the settings UI; `!command` resolution supported).
 *   3. Token file `~/.aery/auth-broker.token` (paired with URL from env or config).
 *
...
import { getAgentDir, getConfigRootDir, isEnoent, logger } from "@aryee337/aery-utils";
...
/** Path to the local bearer token file. Created on the broker host by `aery auth-broker token`. */
...
export async function resolveAuthBrokerConfig(): Promise<AuthBrokerClientConfig | null> {
	const envUrl = process.env.AERY_AUTH_BROKER_URL;
	const envToken = process.env.AERY_AUTH_BROKER_TOKEN;
	...
	if (!token) {
		throw new Error(
			`AERY_AUTH_BROKER_URL is set (${url}) but no bearer token is available. ` +
				`Set AERY_AUTH_BROKER_TOKEN, the \`auth.broker.token\` config entry, or place one at ${getAuthBrokerTokenFilePath()}.`,
		);
	}
	return { url, token };
}
