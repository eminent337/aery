...
/**
 * Coalesces synchronous writes into a deferred batch.
 */
class AsyncDrain<T> {
	#queue?: T[];
...
/** Get the busy timeout in milliseconds for SQLite operations. */
function getDbBusyTimeoutMs(): number {
	return 5000;
}
 ...
 * Delegates auth credential operations to AuthCredentialStore from @aryee337/aery-ai.
...
`);
		...
		try {
			for (const row of this.#listModelPerfStmt.all() as ModelPerfRow[]) {
				...
				stats.set(row.model_key, {
					...
					ttftMs: row.ttft_samples > 0 ? row.ttft_ms / row.ttft_samples : undefined,
				});
			}
		} catch (error) {
	 ...
	 * `omp stats` database (`~/.aery/stats.db`) into model_perf. Fire-and-forget:
