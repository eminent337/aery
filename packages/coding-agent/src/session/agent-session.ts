...
import {
	...
	type SummaryOptions,
	shouldCompact,
} from "@aryee337/aery-core/compaction";
import { type ProactivePruneConfig, shouldProactivePrune } from "@aryee337/aery-core/compaction/proactive-prune";
import { DEFAULT_PRUNE_CONFIG, pruneToolOutputs } from "@aryee337/aery-core/compaction/pruning";
import type { ProtectedToolMatcher } from "@aryee337/aery-core/compaction/tool-protection";
import { countTokens, MacOSPowerAssertion } from "@aryee337/aery-engine";
import {
	extractRetryHint,
	getAgentDbPath,
	getInstallId,
...
export type AgentSessionEvent =
	...
	| { type: "session_indexed"; sessionId: string };
...
function buildSessionMetadata(
	...
	if (provider === "anthropic") {
		...
		if (typeof accountUuid === "string" && accountUuid.length > 0) {
			...
			// aery's persistent install id as the root instead of deriving it from
			...
			userId.device_id = crypto
				.createHash("sha256")
				.update(`aery-claude-device-id-v1:${getInstallId()}`)
				.digest("hex");
		}
	}
...
export class AgentSession {
	...
	constructor(config: AgentSessionConfig) {
		...
		this.agent.setOnBeforeYield(() => {
			this.#flushPendingIrcAsides();
			return this.yieldQueue.flush("streaming");
		});
	...
	#buildAdvisorRuntime(seedToCurrent = false): boolean {
		...
		if (this.#agentId && this.#agentId !== "main" && !this.settings.get("advisor.subagents")) return false;
	 ...
	 * `device_id` is derived from the persistent aery install id. Resolving live
	...
	#isExplicitAutoApproveMode(): boolean {
		return this.#autoApprove || this.settings.get("tools.approvalMode") === "yolo";
	}
	 ...
	 * - Handles extension commands (registered via aery.registerCommand) immediately, even during streaming
		...
		try {
			...
			if (compactionPrep.kind === "fromHook") {
				...
				try {
					...
					// --- PLAN READ MATCHING AND COMPACTION PROTECTION BEHAVIOR ---
					for (const msg of preparation.messagesToSummarize) {
						...
						if (contentStr.includes("<DO_NOT_COMPACT_THIS>")) {
							const planMatch = contentStr.match(/<DO_NOT_COMPACT_THIS>([\s\S]*?)<\/DO_NOT_COMPACT_THIS>/);
							if (planMatch) {
								summary += `\n\n[DO NOT COMPACT THIS]\n${planMatch[1]}\n[/DO NOT COMPACT THIS]`;
							}
						}
	...
	async #runPrePromptCompactionIfNeeded(messages: AgentMessage[]): Promise<void> {
		...
		// Proactive pruning: cheap shake+prune before expensive LLM summarization.
		// Ported from Freebuff's context-pruner pattern — runs before every prompt
		// to keep context lean without full compaction.
		const proactiveConfig: ProactivePruneConfig = {
			contextWindow,
			thresholdPercent: 85,
			cacheExpiryMs: 5 * 60 * 1000, // Anthropic prompt cache TTL
		};
		if (shouldProactivePrune(contextTokens, undefined, proactiveConfig)) {
			logger.debug("Proactive pruning triggered before prompt", {
				contextTokens,
				contextWindow,
			});
			const shakeResult = await this.shake("elide", { config: DEFAULT_SHAKE_CONFIG });
			let reclaimedTokens = shakeResult.tokensFreed;
			const pruneResult = await this.#pruneToolOutputs();
			if (pruneResult) reclaimedTokens += pruneResult.tokensSaved;
			if (reclaimedTokens > 0) {
				logger.debug("Proactive pruning reclaimed tokens", { reclaimedTokens });
			}
		}
	...
	#isCompactionAuthFailure(error: unknown): boolean {
		...
		// aery-native gateway synthetic for "no credential configured" (issue #986).
	...
	#deobfuscatedProviderTextReadyForDelta(text: string): string {
		const deobfuscated = this.#obfuscator?.deobfuscate(text) ?? text;
	...
	async runEphemeralTurn(args: {
		...
		const replyText = this.#obfuscator?.deobfuscate(providerReplyText) ?? providerReplyText;
