...
import { FileType, glob } from "@aryee337/aery-engine";
import {
	CONFIG_DIR_NAME,
	getConfigDirName,
	getPluginsDir,
...
async function readExtensionModuleManifest(
	...
	const pkg = tryParseJson<{ aery?: ExtensionModuleManifest }>(content);
	const manifest = pkg?.aery ?? pkg?.aery;
 ...
 * 3. Subdirectory with package.json: `extensions/<ext>/package.json` with "aery"/"aery" field → load declared paths
 *
 ...
 *
 * Walk order:
 * 1. Walk up from `cwd` looking for the nearest directory containing `.aery/`.
 *    The first match returns `<dir>/.aery/plugins/installed_plugins.json`.
 * 2. If no `.aery/` is found, rescan from `cwd` upward looking for `.git`.
 *    The git root is used as an anchor: `<gitRoot>/.aery/plugins/installed_plugins.json`.
...
export async function resolveActiveProjectRegistryPath(cwd: string): Promise<string | null> {
	// Pass 1: walk up looking for an existing .aery/ directory (nearest wins).
	// Stop before os.homedir() — ~/.aery/ is the user-level config dir, not a project root.
...
/**
 * Like resolveActiveProjectRegistryPath, but falls back to `<cwd>/.aery/plugins/installed_plugins.json`
 * when no project anchor (.aery/ or .git/) is found.
 *
 * Use this when the caller accepts an explicit --scope project so that installing into a freshly
 * bootstrapped directory (no .aery/ or .git/ yet) works: writeInstalledPluginsRegistry auto-creates
 ...
 * Reads ~/.claude/plugins/installed_plugins.json and ~/.aery/plugins/installed_plugins.json,
...
export async function listClaudePluginRoots(
	...
	// ── AERY installed plugins registry ───────────────────────────────────────
	// AERY registry is authoritative: its entries replace Claude's entries for the same plugin ID.
	...
	const aeryRegistryPath = path.join(getPluginsDir(home), "installed_plugins.json");
	const aeryContent = await readFile(aeryRegistryPath);
	if (aeryContent) {
		const aeryRegistry = parseClaudePluginsRegistry(aeryContent);
		if (aeryRegistry) {
			for (const [pluginId, entries] of Object.entries(aeryRegistry.plugins)) {
				if (!Array.isArray(entries) || entries.length === 0) continue;
				...
				// AERY is authoritative: drop all Claude-sourced entries for this plugin ID
			...
			warnings.push(`Failed to parse AERY plugin registry: ${aeryRegistryPath}`);
		}
	}

	// ── Project-scoped AERY registry ────────────────────────────────────────
	// Loaded from the nearest .aery/plugins/installed_plugins.json relative to cwd.
 ...
 * These are prepended to the cache with highest precedence (before AERY/Claude entries).
