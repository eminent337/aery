 ...
 * `prompts/`, `.mcp.json`) are wired into discovery by `aery-plugins.ts`.
 *
 * CLI-provided paths are injected via {@link injectAeryExtensionCliRoots}
 * before discovery runs; settings paths are read lazily from
 * `<scope>/settings.json` in {@link listAeryExtensionRoots} to mirror what
...
import { isEnoent, logger, tryParseJson } from "@aryee337/aery-utils";
...
export interface AeryExtensionRoot {
 ...
 * the registered set; {@link clearAeryExtensionCliRoots} resets for tests.
 */
export function injectAeryExtensionCliRoots(paths: readonly string[], home: string, cwd: string): void {
...
export function clearAeryExtensionCliRoots(): void {
	injectedCliRoots = [];
}

/** Inspect currently-injected CLI roots (read-only). Exposed for diagnostics + tests. */
export function getInjectedAeryExtensionCliRoots(): readonly AeryExtensionRoot[] {
...
function scopeDirs(ctx: LoadContext): ScopeDirs {
	return {
		project: path.join(ctx.cwd, ".aery"),
		user: path.join(ctx.home, ".aery", "agent"),
	};
}
 ...
 * are dropped):
 *
 * 1. CLI roots injected via {@link injectAeryExtensionCliRoots}
 * 2. Project `<cwd>/.aery/settings.json#extensions`
 * 3. User `~/.aery/agent/settings.json#extensions`
 * 4. Enabled plugins installed under `<plugins>/node_modules/` (e.g. via
 *    `aery install <pkg>` / `aery plugin install` / `aery plugin link`)
 *
 ...
 */
export async function listAeryExtensionRoots(ctx: LoadContext): Promise<AeryExtensionRoot[]> {
 ...
 *
 * Marketplace and `aery plugin link` installs write to the plugin manager's
