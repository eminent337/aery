 ...
 * SSH JSON Provider
 *
 * Discovers SSH hosts from managed aery config paths and legacy root ssh.json files.
 * Priority: 5 (low, project/user config discovery)
 */
import * as path from "node:path";
import { getSSHConfigPath, tryParseJson } from "@aryee337/aery-utils";
...
registerProvider(sshCapability.id, {
	...
	description: "Load SSH hosts from managed aery paths and legacy ssh.json/.ssh.json files",
	priority: 5,
	load,
});
