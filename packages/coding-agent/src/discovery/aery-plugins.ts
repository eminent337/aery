/**
 * AERY extension-package sub-discovery provider.
 *
 ...
 * `rules/`, `prompts/`, and `.mcp.json` — are picked up by aery's standard
 * discovery surfaces. The native `aery` provider in `builtin.ts` only walks
 * `.aery/` and `~/.aery/agent/`, so without this provider those sub-trees are
 * silently ignored.
 *
 * Provider priority is set below the native `aery` provider (100) so an
 * extension package never shadows the user's own `.aery/` configuration on
 * dedup.
 *
 * @see ./aery-extension-roots.ts
 * @see ../../docs/extension-loading.md
 */
import * as path from "node:path";
import { logger, parseFrontmatter, tryParseJson } from "@aryee337/aery-utils";
...
import { type AeryExtensionRoot, listAeryExtensionRoots } from "./aery-extension-roots";
...
const DISPLAY_NAME = "AERY Extension Packages";
const DESCRIPTION =
...
async function loadSkills(ctx: LoadContext): Promise<LoadResult<Skill>> {
	const roots = await listAeryExtensionRoots(ctx);
...
async function loadHooks(ctx: LoadContext): Promise<LoadResult<Hook>> {
	const roots = await listAeryExtensionRoots(ctx);
	const tasks: Array<{ root: AeryExtensionRoot; hookType: "pre" | "post" }> = [];
...
async function loadMCPServers(ctx: LoadContext): Promise<LoadResult<MCPServer>> {
	...
	const tasks: Array<{ root: AeryExtensionRoot; mcpPath: string }> = [];
	for (const root of roots) {
	...
	for (let i = 0; i < tasks.length; i++) {
		...
		if (!parsed) {
			warnings.push(`[aery-plugins] Invalid JSON in ${mcpPath}`);
			logger.warn(`[aery-plugins] Invalid JSON in ${mcpPath}`);
			continue;
		}
		...
		for (const [serverName, serverCfg] of Object.entries(servers)) {
			...
			if (typeof cfg.command !== "string" && typeof cfg.url !== "string") {
				warnings.push(`[aery-plugins] Skipping MCP server "${serverName}" in ${mcpPath}: missing command or url`);
				continue;
			}
			items.push({
				name: serverName,
