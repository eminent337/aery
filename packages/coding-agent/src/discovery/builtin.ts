/**
 * Builtin Provider (.aery)
 *
 * Primary provider for AERY native configs. Supports all capabilities.
 */
...
const DISPLAY_NAME = "Aery";
const DESCRIPTION = "Native Aery configuration from ~/.aery and .aery/";
const PRIORITY = 100;
...
async function loadSkills(ctx: LoadContext): Promise<LoadResult<Skill>> {
	// Walk up from cwd finding .aery/skills/ in ancestors (closest first)
	...
	// User-level scan from ~/.aery/agent/skills/
...
async function loadRules(ctx: LoadContext): Promise<LoadResult<Rule>> {
	...
	// https://aery.dev/docs/context-files as the file that gets "re-injected near
	// the current turn so they keep hold across long conversations".
	// User scope:    ~/.aery/agent/RULES.md
	// Project scope: nearest .aery/RULES.md walking up from cwd to repoRoot
...
registerProvider<ContextFile>(contextFileCapability.id, {
	id: PROVIDER_ID,
	displayName: DISPLAY_NAME,
	description: "Load AGENTS.md from .aery/ directories",
	priority: PRIORITY,
