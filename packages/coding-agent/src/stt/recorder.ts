...
$r = Mci "open new type waveaudio alias aery_rec"
if ($r -ne 0) { exit 1 }

Mci "set aery_rec channels 1 samplespersec 16000 bitspersample 16"

$r = Mci "record aery_rec"
if ($r -ne 0) {
    Mci "close aery_rec"
    exit 1
}
...
async function startPowerShellRecording(outputPath: string): Promise<RecordingHandle> {
	...
	const scriptPath = path.join(os.tmpdir(), `aery-stt-record-${Snowflake.next()}.ps1`);
