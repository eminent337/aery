...
import { getWorktreeDir, hashPath, logger, Snowflake } from "@aryee337/aery-utils";
...
async function writeSyntheticTree(repoDir: string, baseTreeish: string, patches: readonly string[]): Promise<string> {
	const tempIndex = path.join(os.tmpdir(), `aery-task-index-${Snowflake.next()}`);
	try {
...
export async function commitToBranch(
	...
	const branchName = `aery/task/${taskId}`;
...
export async function mergeTaskBranches(
	...
	const didStash = await git.stash.push(repoRoot, "aery-task-merge");
