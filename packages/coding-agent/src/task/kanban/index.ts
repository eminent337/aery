...
export function isCommandAvailable(
	...
	const pathExtensions =
		platform === "win32"
			? (env.PATHEXT ?? ".EXE;.CMD;.BAT;.COM").split(";").filter(extension => extension.length > 0)
			: [];
...
export function resolveKanbanInstallCommand(
	...
	if (preferredInstaller) {
		...
		if (preferredCommand && isCommandAvailable(preferredCommand.command, env, platform)) {
			return {
				...preferredCommand,
...
async function ensureKanbanInstalled(command: string, options: LaunchKanbanOptions = {}): Promise<boolean> {
	if (isCommandAvailable(command)) {
		return true;
	}

	const writeErr = options.outputErr ?? console.error;
	const writeln = options.output ?? console.log;

	const installCommand = resolveKanbanInstallCommand(process.env, process.platform, options.preferredInstaller);
	if (!installCommand) {
		writeErr('kanban is not installed. Install it with "npm i -g kanban"');
		return false;
	}

	writeln(chalk.cyan("Installing kanban@latest…"));
	const installProcess = spawnKanbanInstallProcess(installCommand, {
		env: process.env,
		windowsHide: true,
	});
	const installExitCode = await waitForProcessExit(installProcess).catch((error: unknown) => {
		const message = error instanceof Error ? error.message : String(error);
		writeErr(`Failed to run ${installCommand.displayCommand}: ${message}`);
		return 1;
	});
	if (installExitCode !== 0) {
		writeErr(`Failed to install kanban. Try running: ${installCommand.displayCommand}`);
		return false;
	}

	if (!isCommandAvailable(command)) {
		writeErr(`Installed kanban, but ${command} was not found in PATH. Try opening a new terminal.`);
		return false;
	}
 ...
 */
export async function launchKanban(options: LaunchKanbanOptions = {}): Promise<number> {
