 ...
 *   - Bundled agents (shipped with aery-coding-agent)
 *   - ~/.aery/agent/agents/*.md (user-level)
 *   - .aery/agents/*.md (project-level)
 *
 * Supports:
...
import type { Usage } from "@aryee337/aery-ai";
import type { AgentTool, AgentToolResult, AgentToolUpdateCallback } from "@aryee337/aery-core";
import { $env, prompt, Snowflake } from "@aryee337/aery-utils";
...
export class TaskTool implements AgentTool<TaskToolSchemaInstance, TaskToolDetails, Theme> {
	...
	async #executeSync(
		...
		const tempArtifactsDir = artifactsDir ? null : path.join(os.tmpdir(), `aery-task-${Snowflake.next()}`);
					...
					return runSubprocessWithQa(
						{
							...
							parentEvalSessionId,
							mode: task.mode ?? (params as any).mode,
						},
						agents,
						task.assignment ?? "",
					);
