...
import { getWorktreeDir, hashPath, isEnoent, prompt, untilAborted } from "@aryee337/aery-utils";
...
async function resolvePrBranchPushTarget(
	...
	const headRef = await git.config.getBranch(repoRoot, localBranch, "aeryPrHeadRef", signal);
	if (!headRef) {
	...
	const prUrl = await git.config.getBranch(repoRoot, localBranch, "aeryPrUrl", signal);
	const maintainerCanModifyValue = await git.config.getBranch(
		repoRoot,
		localBranch,
		"aeryPrMaintainerCanModify",
		signal,
	);
	const isCrossRepositoryValue = await git.config.getBranch(repoRoot, localBranch, "aeryPrIsCrossRepository", signal);
...
async function checkoutPullRequest(
	...
	return git.withRepoLock(
		...
		async () => {
			...
			await git.config.setBranch(repoRoot, localBranch, "aeryPrHeadRef", headRefName, signal);
			await git.config.setBranch(repoRoot, localBranch, "aeryPrUrl", data.url ?? "", signal);
			await git.config.setBranch(
				repoRoot,
				localBranch,
				"aeryPrIsCrossRepository",
