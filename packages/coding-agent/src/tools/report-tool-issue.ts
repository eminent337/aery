 ...
 * bundled endpoint (`dev.autoqaPush.endpoint`, default `qa.aery.sh`). Each
...
import type { AgentTool } from "@aryee337/aery-core";
import { $env, $flag, getAgentDir, getInstallId, logger, VERSION } from "@aryee337/aery-utils";
 ...
 * `~/.aery/agent/autoqa.db`. Idempotently runs schema creation, the
 * `pushed`-column migration, and index setup so every consumer — tool
 * execute path, manual `aery grievances push`, future debug scripts —
 ...
 *
 * Exported because the `aery grievances` CLI handlers need the migrated
...
/**
 * Optional per-flush controls. Used by `aery grievances push` to surface
...
function resolvePushConfig(settings: Settings | undefined, bypassConsent: boolean): PushConfig | null {
	...
	// covers explicit user-driven pushes (`aery grievances push`) where the
...
async function performFlush(db: Database, config: PushConfig, options: FlushOptions = {}): Promise<FlushResult> {
	...
	for (;;) {
		...
		const body = JSON.stringify({
			agent: { name: "aery", version: VERSION },
...
export function createReportToolIssueTool(session: ToolSession, activeBuiltinNames: readonly string[] = []): AgentTool {
	...
	return {
		...
		async execute(_toolCallId, rawParams) {
			...
			// at ~/.aery/agent/autoqa.db regardless of consent — they always
			// own their local data and can inspect or wipe it via `aery grievances`.
			...
			try {
				...
				if (db) {
					...
					//      `aery grievances push` or a future consent change.
