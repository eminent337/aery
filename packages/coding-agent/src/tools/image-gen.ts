...
import { getAntigravityUserAgent, getEnvApiKey, type Model } from "@aryee337/aery-ai";
import {
	CODEX_BASE_URL,
	getCodexAccountId,
...
async function saveImageToTemp(image: InlineImageData): Promise<string> {
	...
	const filename = `aery-image-${Snowflake.next()}.${ext}`;
...
function buildOpenAIImageHeaders(model: Model, apiKey: string, sessionId: string | undefined): Headers {
	...
	if (model.api === "openai-codex-responses" || model.provider === "openai-codex") {
		...
		headers.set("User-Agent", `aery/${packageJson.version} (${os.platform()} ${os.release()}; ${os.arch()})`);
		if (sessionId) {
