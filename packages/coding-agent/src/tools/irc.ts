...
import { type Component, Text } from "@aryee337/aery-tui";
import { formatAge, formatDuration, prompt } from "@aryee337/aery-utils";
...
function renderSendResult(
	...
	if (receipts.length === 0) {
		...
		return [
			...
			result.isError ? String(text) : `  ${theme.fg("muted", replaceTabs(text))}`,
		];
	}
...
function renderWaitResult(
	...
	return [
		renderStatusLine({ title: `IRC ${theme.nav.back} ${waited.from}`, meta }, theme),
...
function renderInboxResult(
	...
	if (messages.length === 0) {
		return [renderStatusLine({ title: "IRC inbox", meta: ["empty"] }, theme)];
	}
	const meta = [`${messages.length} ${messages.length === 1 ? "message" : "messages"}`];
	if (args?.peek) meta.push("peek");
	const header = renderStatusLine({ title: "IRC inbox", meta }, theme);
...
function renderListResult(details: Partial<IrcDetails>, expanded: boolean, theme: Theme): string[] {
	...
	const header = renderStatusLine({ title: "IRC peers", meta }, theme);
