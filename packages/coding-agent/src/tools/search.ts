...
import { type GrepMatch, GrepOutputMode, type GrepResult, grep } from "@aryee337/aery-engine";
...
async function resolveArchiveSearchPaths(
	...
	for (let idx = 0; idx < paths.length; idx++) {
		...
		if (!tempDir) {
			tempDir = await mkdtemp(path.join(tmpdir(), "aery-search-archive-"));
		}
...
async function expandVirtualInternalResource(
	...
	if (AERY_ROOT_URL_RE.test(rawPath)) {
		const completions = await internalRouter.complete("aery", "");
		if (completions && completions.length > 0) {
			...
			for (const completion of completions) {
				...
				const docUrl = `aery://${completion.value}`;
