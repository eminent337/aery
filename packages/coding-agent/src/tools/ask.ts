...
import { type Component, Container, Markdown, renderInlineMarkdown, TERMINAL, Text } from "@aryee337/aery-tui";
import { prompt, untilAborted } from "@aryee337/aery-utils";
...
async function askSingleQuestion(
	...
	if (multi) {
		...
		const radioPrefix = `${theme.radio.unchecked} `;
		const optionsWithNavigation: ExtensionUISelectItem[] = [
			...displayOptions.map(opt => {
				if (typeof opt === "string") return `${radioPrefix}${opt}`;
				return { ...opt, label: `${radioPrefix}${opt.label}` };
			}),
			`${radioPrefix}${OTHER_OPTION}`,
		];
		...
		if (choice === undefined) {
			...
			const rawChoice = choice.startsWith(radioPrefix) ? choice.slice(radioPrefix.length) : choice;
			selectedOptions = [stripRecommendedSuffix(rawChoice)];
	...
	renderCall(args: AskRenderArgs, _options: RenderResultOptions, uiTheme: Theme): Component {
			...
			for (let i = 0; i < args.questions.length; i++) {
				...
				if (q.options?.length) {
					...
					for (let j = 0; j < q.options.length; j++) {
						...
						const icon = q.multi ? uiTheme.checkbox.unchecked : uiTheme.radio.unchecked;
						optText += `\n ${uiTheme.fg("dim", continuation)}   ${uiTheme.fg("dim", optBranch)} ${uiTheme.fg("dim", icon)} ${optLabel}`;
		...
		if (args.options?.length) {
			...
			for (let i = 0; i < args.options.length; i++) {
				...
				const optLabel = renderInlineMarkdown(opt.label, mdTheme, t => uiTheme.fg("muted", t));
				const icon = args.multi ? uiTheme.checkbox.unchecked : uiTheme.radio.unchecked;
				optText += `\n ${uiTheme.fg("dim", branch)} ${uiTheme.fg("dim", icon)} ${optLabel}`;
				if (opt.description?.trim()) {
	...
	renderResult(
		...
		if (details.results && details.results.length > 0) {
			...
			for (let i = 0; i < details.results.length; i++) {
				...
				for (let j = 0; j < r.selectedOptions.length; j++) {
					...
					const icon = r.multi ? uiTheme.checkbox.checked : uiTheme.radio.checked;
					answerLines.push(
						`${continuation}${uiTheme.fg("dim", optBranch)} ${uiTheme.fg("success", icon)} ${selectedLabel}`,
					);
				}
		...
		if (details.selectedOptions && details.selectedOptions.length > 0) {
			for (let i = 0; i < details.selectedOptions.length; i++) {
				...
				const icon = details.multi ? uiTheme.checkbox.checked : uiTheme.radio.checked;
				answerLines.push(` ${uiTheme.fg("dim", branch)} ${uiTheme.fg("success", icon)} ${selectedLabel}`);
			}
		}
		if (answerLines.length > 0) {
			container.addChild(new Text(answerLines.join("\n"), 0, 0));
