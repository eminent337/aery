 ...
 *
 * Daemons are detached processes that intentionally outlive the Aery
 * session. Because they survive the session, their bookkeeping can NOT
 * live in the session-scoped process registry — that is drained on
 * session shutdown. Instead each daemon gets a small JSON record on disk
 * under ~/.aery/agent/daemons/:
 *
...
export function daemonStateDir(): string {
	return join(homedir(), ".aery", "agent", "daemons");
}
 ...
 * separators or dots could escape the state dir. Empty string is INVALID
 * — callers wanting "no name" must pass `undefined`.
 */
...
/** True when `filePath` resolves to a location inside the daemon state dir. */
...
export function readDaemon(dir: string, id: string): DaemonRecord | undefined {
	const recordPath = daemonRecordPath(dir, id);
