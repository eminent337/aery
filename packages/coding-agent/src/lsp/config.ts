...
import { $which, isRecord, logger, pathIsWithin } from "@aryee337/aery-utils";
...
function getConfigSources(cwd: string): ConfigSource[] {
	...
	// Project config directories (.aery/, .aery/, .claude/)
	...
	// User config directories (~/.aery/agent/, ~/.aery/agent/, ~/.claude/)
 ...
 * 2. Project config dirs: .aery/lsp.*, .aery/lsp.*, .claude/lsp.* (+ hidden variants)
 * 3. User config dirs: ~/.aery/agent/lsp.*, ~/.aery/agent/lsp.*, ~/.claude/lsp.* (+ hidden variants)
