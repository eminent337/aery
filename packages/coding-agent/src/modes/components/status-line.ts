...
import { estimateTokens } from "@aryee337/aery-core/compaction";
import { type Component, truncateToWidth, visibleWidth } from "@aryee337/aery-tui";
import { formatCount, getProjectDir } from "@aryee337/aery-utils";
import { $ } from "bun";
...
export class StatusLineComponent implements Component {
	...
	#buildStatusLine(width: number): string {
		...
		const sepAnsi = theme.getFgAnsi("border");
		...
		// Clean flat separator (vertical bar) for minimalist arrangement
		const sep = "│";
		const leftSepWidth = 1;
		const rightSepWidth = 1;
		...
		const gapFill = `\x1b[39m${" ".repeat(gapWidth)}\x1b[39m`;
	...
	render(width: number): string[] {
		...
		// Aery Flat Style: Render main status line natively as a component below the editor
		lines.push(this.#buildStatusLine(width));
