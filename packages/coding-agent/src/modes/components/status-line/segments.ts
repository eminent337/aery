...
import { TERMINAL } from "@aryee337/aery-tui";
import {
	formatDuration,
	formatNumber,
	getProjectDir,
...
const aerySegment: StatusLineSegment = {
	id: "aery",
	render(_ctx) {
		const content = theme.icon.aery ? `${theme.icon.aery}` : "";
...
const memoryMeterSegment: StatusLineSegment = {
	id: "memory_meter",
	render(ctx) {
		const pct = ctx.contextPercent;
		const width = 10;
		const totalEighths = Math.round((Math.max(0, Math.min(100, pct)) / 100) * width * 8);
		...
		const color = getContextUsageThemeColor(getContextUsageLevel(pct, ctx.contextWindow));
		const text = `Mem [${bar}] ${pct.toFixed(1)}%`;
		const content = withIcon(theme.icon.context, theme.fg(color, text));
...
const workingSegment: StatusLineSegment = {
	...
	render(ctx) {
		...
		const now = Date.now();
		if (now - lastRenderMs >= 80) {
			workingFrameIdx = (workingFrameIdx + 1) % BRAILLE_FRAMES.length;
			lastRenderMs = now;
		}
		const msgIdx = Math.floor(Date.now() / 3000) % WORKING_MESSAGES.length;
		const content = theme.fg("accent", `${BRAILLE_FRAMES[workingFrameIdx]} ${WORKING_MESSAGES[msgIdx]}`);
		return { content, visible: true };
	},
};
...
export const SEGMENTS: Record<StatusLineSegmentId, StatusLineSegment> = {
	...
	usage: usageSegment,
	memory_meter: memoryMeterSegment,
