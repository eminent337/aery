...
} from "@aryee337/aery-engine";
import type { EditorTheme, MarkdownTheme, SelectListTheme, SymbolTheme } from "@aryee337/aery-tui";
import { adjustHsv, getCustomThemesDir, isEnoent, logger } from "@aryee337/aery-utils";
...
export class Theme {
	...
	get sep() {
		return {
			...
			aeryBarLeft: this.#symbols["sep.aeryBarLeft"],
			aeryBarRight: this.#symbols["sep.aeryBarRight"],
			aeryBarStart: this.#symbols["sep.aeryBarStart"],
		};
	}
	...
	get radio() {
		return {
			checked: this.#symbols["radio.checked"],
			unchecked: this.#symbols["radio.unchecked"],
		};
	}
...
function getLuminance(hex: string): number {
	if (typeof hex === "number") return 0; // fallback
	...
	const cR = r <= 0.03928 ? r / 12.92 : ((r + 0.055) / 1.055) ** 2.4;
	const cG = g <= 0.03928 ? g / 12.92 : ((g + 0.055) / 1.055) ** 2.4;
	const cB = b <= 0.03928 ? b / 12.92 : ((b + 0.055) / 1.055) ** 2.4;
...
function capLuminance(accentHex: string | number, bgHex: string | number): string | number {
	if (typeof accentHex === "number" || typeof bgHex === "number") return accentHex;
	const bgLum = getLuminance(bgHex);
	if (bgLum > 0.5) {
		// Light background: ensure accent is not too light
		let currentAccent = accentHex;
		let currentLum = getLuminance(currentAccent);
		let iterations = 0;
		while (currentLum > 0.2 && iterations < 5) {
			currentAccent = darkenHex(currentAccent);
			currentLum = getLuminance(currentAccent);
			iterations++;
		}
...
function createTheme(themeJson: ThemeJson, options: CreateThemeOptions = {}): Theme {
	...
	if (resolvedColors.accent && resolvedColors.statusLineBg) {
		resolvedColors.accent = capLuminance(resolvedColors.accent, resolvedColors.statusLineBg) as string;
	}
...
var onThemeChangeListeners: Set<() => void> = new Set();
var themeLoadRequestId: number = 0;

/** Notify all registered theme change listeners. */
function notifyThemeChange(): void {
	for (const listener of onThemeChangeListeners) {
		try {
			listener();
		} catch {
			/* listeners must never crash the theme system */
		}
	}
}
...
/**
 * Register a callback to be called whenever the active theme changes.
 * Mirrors OpenClaude's ThemeProvider pattern where every consumer subscribes
 * and is notified simultaneously on theme switches.
 *
 * Returns a cleanup function — call it to unregister the listener.
 */
export function onThemeChange(callback: () => void): () => void {
	onThemeChangeListeners.add(callback);
	return () => onThemeChangeListeners.delete(callback);
}

/**
 * Remove a previously registered theme change listener.
 * @deprecated Prefer using the cleanup function returned by `onThemeChange()`.
 */
export function offThemeChange(callback: () => void): void {
	onThemeChangeListeners.delete(callback);
}
...
export function getSymbolTheme(): SymbolTheme {
	...
	return {
		...
		aeryBarStart: theme.sep.aeryBarStart,
		aeryBarLeft: theme.sep.aeryBarLeft,
		aeryBarRight: theme.sep.aeryBarRight,
	};
}
...
export function getSettingsListTheme(): import("@aryee337/aery-tui").SettingsListTheme {
	return {
