...
import type { AutocompleteProvider, SlashCommand } from "@aryee337/aery-tui";
import { $env, sanitizeText } from "@aryee337/aery-utils";
...
export class InputController {
	...
	setupKeyHandlers(): void {
		...
		for (const key of this.ctx.keybindings.getKeys("app.studio.toggle")) {
			this.ctx.editor.setCustomKeyHandler(key, () => this.ctx.showStudio());
		}
	...
	async handleImagePaste(): Promise<boolean> {
		try {
			const MAX_IMAGES = 20; // Default image budget limit
			if ((this.ctx.pendingImages?.length ?? 0) >= MAX_IMAGES) {
				this.ctx.showStatus(`Cannot add image: budget limit of ${MAX_IMAGES} reached`);
				return false;
			}
