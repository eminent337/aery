import type { AssistantMessage, ImageContent } from "@aryee337/aery-ai";
...
export class EventController {
	...
	/** True while a thinking block is actively streaming — spinner shows "Thinking…" */
	#isThinking = false;
	...
	async #handleMessageUpdate(event: Extract<AgentSessionEvent, { type: "message_update" }>): Promise<void> {
		if (this.ctx.streamingComponent && event.message.role === "assistant") {
			...
			// Update the spinner label when the model enters/exits a thinking block.
			// This mirrors OpenClaude's Spinner.tsx `mode === 'thinking'` detection:
			// the thinking state is surfaced through the single working-message spinner
			// rather than a separate static "Thinking..." element in the chat.
			const ameType = event.assistantMessageEvent?.type;
			if (ameType === "thinking_start" || ameType === "thinking_delta") {
				if (!this.#isThinking) {
					this.#isThinking = true;
					this.ctx.setWorkingMessage(`Thinking…${interruptHint()}`);
				}
			} else if (ameType === "thinking_end" && this.#isThinking) {
				this.#isThinking = false;
				// Revert to the default working message (intent / "Working…")
