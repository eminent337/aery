...
import { logger, VERSION } from "@aryee337/aery-utils";
	...
	async initialize(params: InitializeRequest): Promise<InitializeResponse> {
		...
		const authMethods: AuthMethod[] = [
			{
				...
				description: "Authenticate via the provider keys/OAuth state already configured under ~/.aery.",
			},
		];
		if (params.clientCapabilities?.auth?.terminal === true) {
			authMethods.push({
				type: "terminal",
				id: "terminal",
				name: "Set up Aery in terminal",
				description: "Launch the aery TUI to add provider keys and select models.",
