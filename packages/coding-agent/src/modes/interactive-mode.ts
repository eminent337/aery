...
export class InteractiveMode implements InteractiveModeContext {
	...
	async init(options: InteractiveModeInitOptions = {}): Promise<void> {
		...
		await logger.time(
			"InteractiveMode.init:titleSystemPrompt",
			this.refreshTitleSystemPrompt.bind(this),
			getProjectDir(),
		);
		...
		this.ui.addChild(this.statusLine); // Renders flat status line natively below editor
	...
	updateEditorTopBorder(): void {
		const currentImages = this.pendingImages?.length ?? 0;
		if (currentImages > 0) {
			const MAX_IMAGES = 20; // Default budget limit for typical models
			const filled = Math.min(currentImages, MAX_IMAGES);
			const empty = Math.max(0, MAX_IMAGES - filled);
			const bar = "█".repeat(filled) + "░".repeat(empty);
			const text = ` Images [${bar}] ${currentImages}/${MAX_IMAGES} `;
			this.editor.setTopBorder({ content: text, width: text.length });
		} else {
	...
	async shutdown(): Promise<void> {
		...
		// hands Bash the cursor immediately after visible AERY content.
	...
	showSessionObserver(): void {
		...
		if (sessions.length === 0) {
			this.showStatus("No active sessions");
			return;
		}
	...
	showStudio(): void {
		this.#selectorController.showStudio();
	}

	showMediaStudio(): void {
		this.#selectorController.showMediaStudio();
	}
