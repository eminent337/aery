...
import { AERY_LOGO, gradientEscape, gradientLogo, type ShineConfig } from "../../components/welcome";
...
const LARGE_LOGO = AERY_LOGO.flatMap(line => {
	let wide = "";
...
/**
 * Animated setup splash, in the spirit of the aery landing page: the brand π
...
function renderCompactSplash(width: number, height: number, phase: number, shine: ShineConfig): string[] {
	const art = height >= 14 ? LARGE_LOGO : AERY_LOGO;
