import { type Component, matchesKey, padding, truncateToWidth, visibleWidth } from "@aryee337/aery-tui";
import { APP_NAME } from "@aryee337/aery-utils";
import { AERY_LOGO, gradientLogo } from "../components/welcome";
...
export class SetupWizardComponent implements Component {
	...
	#renderScene(width: number, height: number): string[] {
		...
		const logo = gradientLogo(AERY_LOGO, 0);
		const header = [
			"",
