 ...
 */
import type { ArgDescriptor, CliConfig, CommandCtor, FlagDescriptor } from "@aryee337/aery-utils/cli";
...
function bashValueBranch(bin: string, v: ValueSource): string {
	switch (v.kind) {
		...
		case "list":
			return `_aery_comma "${bashWords(v.values)}"; return 0`;
		case "models":
			return v.multiple
				? `_aery_comma "$(command ${bin} __complete models 2>/dev/null | cut -f1)"; return 0`
	...
	for (const c of spec.commands) {
		...
		const elseBranch = argWords
			...
			: fileArg
				...
				: ":";
		parts.push(`_aery_cmd_${bashFn(c.name)}() {
	case "$prev" in
...
function zshAction(v: ValueSource): string {
	switch (v.kind) {
		...
		case "list":
			return ":value:_aery_tools";
		case "models":
			return v.multiple ? ":models:_aery_models_list" : ":model:_aery_call models";
		case "sessions":
			return ":session:_aery_call sessions";
		case "file":
			return ":file:_files";
		case "dir":
			return ":dir:_files -/";
...
function generateZsh(spec: CompletionSpec): string {
	const { bin } = spec;
	// The `:value:_aery_tools` action references this helper; bake its candidates
...
${cmdRows}
	...
	const aliasArms = spec.commands
		.map(c => `\t\t\t${commandTokens(c).join("|")}) _aery_cmd_${bashFn(c.name)} ;;`)
		.join("\n");
	const rootSpecs = [
...
# Works both ways: autoloaded from $fpath (file named _aery) or eval'd from a
# startup file. When autoloaded, funcstack[1] is _aery and we invoke it; when
# sourced/eval'd we register it with compdef instead.
if [ "$funcstack[1]" = "_aery" ]; then
	_aery "$@"
else
	compdef _aery ${bin}
fi`);
...
function generateFish(spec: CompletionSpec): string {
	...
	lines.push(`function __fish_aery_no_subcommand`);
	...
	const rootCond = "__fish_aery_no_subcommand";
