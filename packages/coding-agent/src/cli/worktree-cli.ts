/**
 * CLI handler for `aery worktree` — list and clean up agent-managed worktrees.
 *
 * Layout under `~/.aery/wt/`:
 *
...
import { getWorktreesDir, isEnoent } from "@aryee337/aery-utils";
...
export interface WorktreeEntry {
	/** Absolute path to the worktree dir (or stray container) under `~/.aery/wt/`. */
	path: string;
	...
	branch?: string;
	/** When set, the entry is unhealthy and `aery worktree clear` will remove it. */
	orphanReason?: string;
}
...
async function scanWorktrees(): Promise<WorktreeEntry[]> {
	...
	for (const name of topLevel) {
		...
		// Legacy nesting: ~/.aery/wt/<encoded-project>/<branch-or-id>
		let children: string[];
		try {
