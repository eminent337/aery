/**
 * `aery auth-broker` command handlers.
 *
 * Sub-verbs:
...
import { $which, APP_NAME, getAgentDbPath, getConfigRootDir, isEnoent, logger, VERSION } from "@aryee337/aery-utils";
import { setTransports as setLoggerTransports } from "@aryee337/aery-utils/logger";
import { $ } from "bun";
...
async function runServe(flags: AuthBrokerCommandArgs["flags"]): Promise<void> {
	...
	// skip the rotating ~/.aery/logs/ file the TUI default would have used.
...
async function runLogin(flags: AuthBrokerCommandArgs["flags"]): Promise<void> {
	...
	if (!providerArg) {
		if (flags.via) {
			throw new Error(
				"Usage: aery auth-broker login <provider> --via=user@host (provider required for remote login)",
			);
		}
...
/**
 * Maps the `type` field of a CLIProxyAPI credential JSON to the aery provider id.
...
async function loadImportPlan(
	...
	for (const file of files) {
		...
		if (!provider) {
			skipped.push({
				file,
				reason: `cannot determine aery provider from type=${json.type ?? "?"} (pass --provider to override)`,
			});
			continue;
		}
...
async function runImport(flags: AuthBrokerCommandArgs["flags"]): Promise<void> {
	...
	if (!target) {
		throw new Error("Usage: aery auth-broker import <file|dir> [--provider=<id>] [--include-disabled] [--dry-run]");
	}
...
async function runMigrate(flags: AuthBrokerCommandArgs["flags"]): Promise<void> {
	...
	if (!brokerConfig) {
		throw new Error(
			"AERY_AUTH_BROKER_URL must be set (or `auth.broker.url` in config.yml). `migrate` uploads local credentials to a configured broker.",
		);
	}
	if (flags.fromLocal !== true) {
		throw new Error(
			"`aery auth-broker migrate` requires an explicit source. Pass `--from-local` to migrate from the local SQLite store and env vars.",
		);
	}
	...
	try {
		for (const row of localStore.listAuthCredentials()) {
			// Skip placeholder sentinels that aery-ai treats as "authenticated via
...
async function runStatus(flags: AuthBrokerCommandArgs["flags"]): Promise<void> {
	...
	if (!cfg) {
		const message = "No auth-broker configured (set AERY_AUTH_BROKER_URL to enable).";
