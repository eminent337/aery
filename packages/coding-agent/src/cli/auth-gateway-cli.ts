/**
 * `aery auth-gateway` command handlers.
 *
 ...
 * `AERY_AUTH_BROKER_URL` / `auth.broker.url` precedence used elsewhere).
 *
 * Sub-verbs:
...
import { getConfigRootDir, isEnoent, VERSION } from "@aryee337/aery-utils";
...
async function runServe(flags: AuthGatewayCommandArgs["flags"]): Promise<void> {
	...
	if (!brokerConfig) {
		throw new Error(
			"`aery auth-gateway serve` requires AERY_AUTH_BROKER_URL (or `auth.broker.url`/`auth.broker.token` in config.yml). The gateway is itself a broker client.",
		);
	}
	...
	// Build the model resolver + catalog from aery-ai's bundled metadata, scoped
	// to providers we hold credentials for. Format handlers ask `resolveModel`
	// to translate a client-requested `model` field into a aery-ai `Model<Api>`
...
async function runStatus(flags: AuthGatewayCommandArgs["flags"]): Promise<void> {
	...
	if (!brokerConfig) {
		...
		if (flags.json) {
			...
			process.stdout.write(`${chalk.yellow("No broker configured.")} Set AERY_AUTH_BROKER_URL.\n`);
			process.stdout.write(
	...
	try {
		...
		if (flags.json) {
			...
			if (!tokenPresent) {
				process.stdout.write(
					"Run `aery auth-gateway token` or `aery auth-gateway serve` to create a bearer token.\n",
				);
			}
		}
 ...
 * - `cursor-agent` and `aery-native` (gateway forwarding) have transport quirks
 ...
 * aery-native transport (would loop through the gateway), and placeholder /
...
function pickProbeCandidates(provider: string): Model<Api>[] {
	...
	const candidates = bundled.filter(model => {
		if (model.transport === "aery-native") return false;
...
/**
 * `aery auth-gateway check` — probe each broker-supplied credential and print
...
async function runCheck(flags: AuthGatewayCommandArgs["flags"]): Promise<void> {
	...
	if (!brokerConfig) {
		throw new Error(
			"`aery auth-gateway check` requires AERY_AUTH_BROKER_URL (or `auth.broker.url`/`auth.broker.token` in config.yml). It probes the same credentials the gateway would serve.",
		);
	}
