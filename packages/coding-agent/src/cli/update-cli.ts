 ...
 *
 * Handles `aery update` to check for and install updates.
 * Uses bun if available, otherwise downloads binary from GitHub releases.
 */
import * as fs from "node:fs";
import * as path from "node:path";
import { pipeline } from "node:stream/promises";
import { $which, APP_NAME, isEnoent, VERSION } from "@aryee337/aery-utils";
import { $ } from "bun";
import chalk from "chalk";
import { theme } from "../modes/theme/theme";

const REPO = "eminent337/aery";
const PACKAGE = "@aryee337/aery";
/**
...
function isPathInDirectory(filePath: string, directoryPath: string): boolean {
	...
	// PATH-resolved aery path can refer to the same directory through different
...
function resolveUpdateMethod(aeryPath: string, bunBinDir: string | undefined): "bun" | "binary" {
	if (!bunBinDir) return "binary";
	return isPathInDirectory(aeryPath, bunBinDir) ? "bun" : "binary";
}

export function resolveUpdateMethodForTest(aeryPath: string, bunBinDir: string | undefined): "bun" | "binary" {
	return resolveUpdateMethod(aeryPath, bunBinDir);
}
async function resolveUpdateTarget(): Promise<UpdateTarget> {
	const bunBinDir = await getBunGlobalBinDir();
	const aeryPath = resolveAeryPath();

	if (aeryPath) {
		const method = resolveUpdateMethod(aeryPath, bunBinDir);
		if (method === "bun") return { method };
		return { method, path: aeryPath };
	}
...
/**
 * Resolve the path that `aery` maps to in the user's PATH.
 */
function resolveAeryPath(): string | undefined {
	return $which(APP_NAME) ?? undefined;
}

/**
 * Run the resolved aery binary and check if it reports the expected version.
 */
async function verifyInstalledVersion(expectedVersion: string): Promise<InstalledVersionVerification> {
	const aeryPath = resolveAeryPath();
	if (!aeryPath) return { ok: false };
	try {
		const result = await $`${aeryPath} --version`.quiet().nothrow();
		if (result.exitCode !== 0) return { ok: false, path: aeryPath };
		const output = result.text().trim();
		// Output format: "aery/X.Y.Z"
		const match = output.match(/\/(\d+\.\d+\.\d+)/);
		const actual = match?.[1];
		return { ok: actual === expectedVersion, actual, path: aeryPath };
	} catch {
		return { ok: false, path: aeryPath };
	}
}
...
async function printVerification(expectedVersion: string): Promise<void> {
	...
	console.log(chalk.yellow(`You may need to reinstall: curl -fsSL https://aery.sh/install | sh`));
}
...
/**
 * Build the bun argv used to globally install a specific aery version.
 *
 ...
 *
 * Together these two flags make `aery update` produce exactly the registry
...
export async function runUpdateCommand(opts: { force: boolean; check: boolean }): Promise<void> {
	...
	// Choose update method based on the prioritized aery binary in PATH
	try {
