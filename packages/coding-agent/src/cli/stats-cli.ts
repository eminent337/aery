 ...
 *
 * Handles `aery stats` subcommand for viewing AI usage statistics.
 */

import { APP_NAME, formatDuration, formatNumber, formatPercent } from "@aryee337/aery-utils";
...
async function printStatsSummary(): Promise<void> {
	const { getDashboardStats } = await import("@aryee337/aery-stats");
	const stats = await getDashboardStats();
