...
import { registerAeryExtensions } from "./extension-bridge";
...
async function checkForNewVersion(currentVersion: string): Promise<string | undefined> {
	...
	try {
		const response = await fetch("https://registry.npmjs.org/@aryee337/aery/latest");
...
function discoverSystemPromptFile(): string | undefined {
	// Check project-local first (.aery/SYSTEM.md, .aery/SYSTEM.md legacy)
...
export async function runRootCommand(
	...
	// the `aery-plugins` discovery provider can surface their `skills/`, `hooks/`,
	...
	if (!parsedArgs.noExtensions) {
		...
		if (cliExtensions && cliExtensions.length > 0) {
			injectAeryExtensionCliRoots(cliExtensions, home, getProjectDir());
		}

		registerAeryExtensions(home, getProjectDir());
	}
	...
	if (mode === "acp") {
		...
		} else if (mode === "fas" || mode === "ferment") {
			const { runFermentMode } = await import("./modes/ferment-mode.js");
			const goal = initialArgs.messages[0] ?? parsedArgs.messages[0] ?? pipedInput ?? "";
			if (!goal) {
				process.stderr.write("Error: Ferment mode requires a goal.\n");
				process.exit(1);
			}
