import type { ImageContent, Message, Model, TextContent } from "@aryee337/aery-ai";
...
export interface HookContext {
	...
	cwd: string;
	/** Session manager (read-only) - use aery.sendMessage()/aery.appendEntry() for writes */
 ...
 * Hooks use aery.on() to subscribe to events and aery.sendMessage() to inject messages.
 */
export interface HookAPI {
	 ...
	 * aery.appendEntry("permissions", { level: "full", grantedAt: Date.now() });
	 *
	 * // On reload, reconstruct state from entries
	 * aery.on("session", async (event, ctx) => {
	...
	logger: typeof import("@aryee337/aery-utils").logger;
	...
	/** Injected aery-coding-agent exports */
	aery: typeof import("../..");
}
 ...
 */
export type HookFactory = (aery: HookAPI) => void;
