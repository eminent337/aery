...
import { getAgentDir, getProjectDir, isEnoent } from "@aryee337/aery-utils";
...
export async function installPlugin(packageName: string): Promise<InstalledPlugin> {
	...
	if (!(await pkgJson.exists())) {
		await pkgJson.write(JSON.stringify({ name: "aery-plugins", private: true, dependencies: {} }, null, 2));
	}
	...
	return {
		...
		manifest: pkg.aery || pkg.aery || { version: pkg.version },
