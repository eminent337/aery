...
import { getPluginsLockfile, getPluginsNodeModules, getPluginsPackageJson, isEnoent } from "@aryee337/aery-utils";
import { getConfigDirPaths } from "../../config";
import type { InstalledPlugin, PluginManifest, PluginRuntimeConfig, ProjectPluginOverrides } from "./types";
import { installAeryModuleResolver } from "./upstream-compat";
 ...
 *
 * `home` controls which `<plugins>/aery-plugins.lock.json` is read — pass it
...
/**
 * Load project-local plugin overrides (checks .aery and .aery directories).
 */
 ...
 * packages) and `<plugins>/aery-plugins.lock.json#plugins` (so locally
...
export async function getEnabledPlugins(cwd: string, opts: { home?: string } = {}): Promise<InstalledPlugin[]> {
	...
	for (const name of names) {
		...
		let pluginPkg: { version: string; aery?: PluginManifest };
		try {
		...
		const manifest: PluginManifest | undefined = pluginPkg.aery || pluginPkg.aery;
		if (!manifest) {
			// Not an aery plugin, skip
			continue;
		}
