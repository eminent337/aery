...
import { hasFsCode, isEacces, isEnoent, logger } from "@aryee337/aery-utils";
...
import { installAeryModuleResolver, loadAeryExtensionModule } from "../plugins/upstream-compat";
...
async function loadExtension(
	...
	try {
		const module = (await loadAeryExtensionModule(resolvedPath)) as LoadedExtensionModule;
...
async function readExtensionManifest(packageJsonPath: string): Promise<ExtensionManifest | null> {
	try {
		const pkg = (await Bun.file(packageJsonPath).json()) as { aery?: ExtensionManifest };
...
export async function discoverAndLoadExtensions(
	...
	// 1. Discover extension modules via capability API (native .aery/.aery only)
