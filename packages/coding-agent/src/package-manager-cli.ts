...
import { getLatestAeryRelease, isNewerPackageVersion } from "./utils/version-check.js";
...
Options:
  -l, --local    Install project-locally (.aery/settings.json)
...
Options:
  -l, --local    Remove from project settings (.aery/settings.json)
...
Short forms:
  ...
  ${APP_NAME} update aery           Update ${APP_NAME} only (self works as alias to aery)
`);
			return;
...
function parsePackageCommand(args: string[]): PackageCommandOptions | undefined {
	...
	if (command === "update") {
		if (extensionFlagSource) {
			...
			const sourceIsSelf = source === "self" || source === "aery";
			if (sourceIsSelf) {
...
async function getSelfUpdatePlan(force: boolean): Promise<SelfUpdatePlan> {
	...
	try {
		const latestRelease = await getLatestAeryRelease(VERSION);
