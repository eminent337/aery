...
- `resolveExtensionEntries(dir)` checks, in order:
  - `package.json` manifest (`aery` or `aery`) with `extensions[]` entries,
...
- Uses `Settings.getShellConfig()` and `Shell` from `@aryee337/aery-engine`.
...
- The `task.isolation.merge` setting controls how isolated changes are integrated back:
  ...
  - **branch**: each task commits to a temp branch (`aery/task/<id>`) via `commitToBranch(...)`, then `mergeTaskBranches(...)` cherry-picks them sequentially onto HEAD. If `git apply` fails inside `commitToBranch`, the error is non-fatal — the agent result is preserved with a `merge failed` status.
...
1. Normalize URL (`normalizeUrl`) and short-circuit `aery-internal://`.
...
- Build compiled binary artifact (`dist/aery`):
