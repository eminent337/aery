...
const outputPath = path.join(packageDir, "dist", "aery");
...
async function main(): Promise<void> {
	...
	try {
		await runCommand(["bun", "--cwd=../aery-engine", "run", "embed:native"]);
		try {
			...
			await runCommand(
				[
					...
					// Legacy aery-* extension compat entrypoints served by
					// `upstream-compat.ts`. These are reached via computed bunfs paths
					...
					// extra entrypoint (issue #1474), so legacy `aery-coding-agent` imports
					// resolve through `upstream-coding-agent-shim.ts` instead.
					"../agent/src/index.ts",
					"../aery-engine/native/index.js",
					"../tui/src/index.ts",
					"../utils/src/index.ts",
					"./src/extensibility/typebox.ts",
					"./src/extensibility/upstream-ai-shim.ts",
					"./src/extensibility/upstream-coding-agent-shim.ts",
					"--outfile",
					"dist/aery",
				],
				buildEnv,
			...
			await runCommand(["bun", "--cwd=../aery-engine", "run", "embed:native", "--reset"]);
		}
	} finally {
