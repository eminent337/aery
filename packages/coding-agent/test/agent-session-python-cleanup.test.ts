...
import * as pythonExecutor from "@aryee337/aery/eval/py/executor";
import type { PythonKernel as PythonKernelInstance } from "@aryee337/aery/eval/py/kernel";
import * as pythonKernel from "@aryee337/aery/eval/py/kernel";
import { AgentRegistry } from "@aryee337/aery/registry/agent-registry";
import { createAgentSession, type ExtensionFactory, type WorkspaceTree } from "@aryee337/aery/sdk";
...
const createTempProject = () => {
	const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), `aery-agent-session-python-cleanup-${Snowflake.next()}-`));
