...
// to scoop\persist\Oven-sh.Bun\.bun. `bun pm bin -g` and the aery path that
...
// does not follow filesystem links, so it misclassified Bun-installed aery
// as "binary" and tried to swap aery.exe in place – which fails on Windows
...
describe("issue-845: resolveUpdateMethod follows symlinks/junctions", () => {
	...
	let aeryPathViaLink: string;

	beforeAll(() => {
		tmpRoot = fs.mkdtempSync(path.join(os.tmpdir(), "aery-issue-845-"));
		realBinDir = path.join(tmpRoot, "real", "bin");
		fs.mkdirSync(realBinDir, { recursive: true });
		fs.writeFileSync(path.join(realBinDir, "aery"), "#!/bin/sh\n", { mode: 0o755 });

		linkedBinDir = path.join(tmpRoot, "link-bin");
		fs.symlinkSync(realBinDir, linkedBinDir, "dir");
		aeryPathViaLink = path.join(linkedBinDir, "aery");
	});
	...
	it("classifies aery reached through a symlinked bin dir as bun-managed", () => {
		// $which resolves through the symlink, `bun pm bin -g` returns the real path
		// (or vice versa). Either direction must be recognized.
		const method = resolveUpdateMethodForTest(aeryPathViaLink, realBinDir);
		expect(method).toBe("bun");
	});

	it("classifies aery at the real bin dir as bun-managed when bunBinDir is symlinked", () => {
		const aeryAtReal = path.join(realBinDir, "aery");
		const method = resolveUpdateMethodForTest(aeryAtReal, linkedBinDir);
		expect(method).toBe("bun");
	});
});
