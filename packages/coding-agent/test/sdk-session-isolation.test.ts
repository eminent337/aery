...
import { SecretObfuscator } from "@aryee337/aery/secrets";
import { SessionManager } from "@aryee337/aery/session/session-manager";
import { type AssistantMessage, getBundledModel } from "@aryee337/aery-ai";
import { getSessionsDir, Snowflake } from "@aryee337/aery-utils";
...
describe("createAgentSession session storage isolation", () => {
	...
	it("uses the provided agentDir for the default persistent session root", async () => {
		const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), `aery-sdk-session-isolation-${Snowflake.next()}-`));
	...
	it("wires the discovered TTSR manager into the created session", async () => {
		const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), `aery-sdk-ttsr-${Snowflake.next()}-`));
		tempDirs.push(tempDir);
		const cwd = path.join(tempDir, `project-${Snowflake.next()}`);
		const agentDir = path.join(tempDir, "agent");
	...
	it("shows redaction guidance only when secrets are actually loaded", async () => {
		await withClearedSecretEnv(async () => {
			...
			const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), `aery-sdk-secrets-${Snowflake.next()}-`));
			tempDirs.push(tempDir);
			...
			fs.mkdirSync(path.join(cwd, ".aery"), { recursive: true });
			fs.writeFileSync(
				path.join(cwd, ".aery", "secrets.yml"),
	...
	it("keeps restored assistant messages deobfuscated across reloads", async () => {
		await withClearedSecretEnv(async () => {
			const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), `aery-sdk-session-secrets-${Snowflake.next()}-`));
			tempDirs.push(tempDir);
			const cwd = path.join(tempDir, "project");
			const agentDir = path.join(tempDir, "agent");
			fs.mkdirSync(path.join(cwd, ".aery"), { recursive: true });
