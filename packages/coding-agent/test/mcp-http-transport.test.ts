...
describe("HTTP MCP transport", () => {
	...
	describe("resolveSSEConnectTimeoutMs", () => {
		const originalEnv = process.env.AERY_MCP_TIMEOUT_MS;
		...
		afterEach(() => {
			if (originalEnv === undefined) delete process.env.AERY_MCP_TIMEOUT_MS;
			else process.env.AERY_MCP_TIMEOUT_MS = originalEnv;
		});
		...
		it("returns 0 when AERY_MCP_TIMEOUT_MS disables the MCP timeout", () => {
