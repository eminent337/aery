...
import { HindsightApi } from "@aryee337/aery/hindsight/client";
import type { HindsightConfig } from "@aryee337/aery/hindsight/config";
import { HindsightSessionState } from "@aryee337/aery/hindsight/state";
import { mnemopiBackend } from "@aryee337/aery/mnemopi/backend";
import { loadMnemopiConfig, type MnemopiBackendConfig } from "@aryee337/aery/mnemopi/config";
import {
	getMnemopiScopedDbPaths,
	getMnemopiSessionState,
	MnemopiSessionState,
	setMnemopiSessionState,
} from "@aryee337/aery/mnemopi/state";
import type { ToolSession } from "@aryee337/aery/tools/index";
import { MemoryEditTool } from "@aryee337/aery/tools/memory-edit";
import { MemoryRecallTool } from "@aryee337/aery/tools/memory-recall";
import { MemoryReflectTool } from "@aryee337/aery/tools/memory-reflect";
import { MemoryRetainTool } from "@aryee337/aery/tools/memory-retain";
...
describe("retain.execute", () => {
	...
	it("flushes a multi-item tool call as a single retainBatch call with per-item context", async () => {
		...
		const client = new HindsightApi({ baseUrl: "http://localhost:8888" });
		const retainBatchSpy = vi.spyOn(HindsightApi.prototype, "retainBatch").mockResolvedValue({} as never);
		registerState(client, settings, { retainTags: ["project:aery"] });
...
describe("recall.execute", () => {
	...
	it("forwards recall tags + tagsMatch from session state when present", async () => {
		...
		registerState(client, settings, { recallTags: ["project:aery"], recallTagsMatch: "any" });
		...
		expect(recallSpy).toHaveBeenCalledWith(
			"test-bank",
			"anything",
			expect.objectContaining({ tags: ["project:aery"], tagsMatch: "any" }),
		);
	});
