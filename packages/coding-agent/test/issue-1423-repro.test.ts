...
/**
 * Regression: aery startup parsed the host project's `CHANGELOG.md` as if it
 * were aery's, then persisted `lastChangelogVersion` to the global config. Root
...
describe("issue #1423 — package-dir lookup must not fall back to cwd", () => {
	...
	beforeEach(() => {
		...
		projectDir = fs.mkdtempSync(path.join(os.tmpdir(), "aery-issue-1423-"));
		fs.writeFileSync(
			path.join(projectDir, "CHANGELOG.md"),
			"# Changelog\n\n## [99.0.0] - 2099-01-01\n\n### Added\n\n- Host project entry, NOT aery's.\n",
		);
	...
	it("walkUpForPackageDir returns undefined when no ancestor owns a package.json", () => {
		...
		const isolated = fs.mkdtempSync(path.join(os.tmpdir(), "aery-issue-1423-isolated-"));
	...
	it("getChangelogPath()'s real-tree result never points under host cwd", () => {
		...
		// to aery's own CHANGELOG.md. The host project's `cwd` MUST not bleed
	...
	it("host project's CHANGELOG.md is never parsed as aery's", async () => {
		const changelogPath = getChangelogPath();
	...
	it("PI_PACKAGE_DIR override stays put and does not fall back to cwd", () => {
		const bogusDir = fs.mkdtempSync(path.join(os.tmpdir(), "aery-issue-1423-override-"));
		try {
