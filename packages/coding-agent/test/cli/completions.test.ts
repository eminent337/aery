...
import type { CliConfig, CommandCtor } from "@aryee337/aery-utils/cli";
...
describe("generateCompletion — bash", () => {
	...
	it("registers the dispatcher and resolves alias arms to the canonical handler", () => {
		expect(out).toContain("complete -F _aery aery");
		expect(out).toContain("_aery_cmd_commit");
	...
	it("completes enum, dynamic, and comma-list flag values by previous flag", () => {
		...
		expect(out).toContain('--model)\n\t\t\tCOMPREPLY=( $(compgen -W "$(command aery __complete models -- "$cur"');
		expect(out).toContain("--resume|-r)");
		expect(out).toContain("command aery __complete sessions");
		// static comma list routes through the comma-aware helper
		expect(out).toContain('--tools)\n\t\t\t_aery_comma "read bash"');
	...
	it("completes a subcommand's positional enum and its own flags", () => {
		expect(out).toContain("_aery_cmd_worktree()");
...
describe("generateCompletion — zsh", () => {
	...
	it("emits the compdef header and dual-mode (autoload + eval) tail", () => {
		expect(out.startsWith("#compdef aery")).toBe(true);
		expect(out).toContain('if [ "$funcstack[1]" = "_aery" ]; then');
		expect(out).toContain("compdef _aery aery");
	});

	it("maps value sources to the right _arguments actions", () => {
		expect(out).toContain("'--model[Model to use]:model:_aery_call models'");
		expect(out).toContain("'--models[Model list]:models:_aery_models_list'");
		expect(out).toContain("'--thinking[Effort]:value:(low high)'");
		expect(out).toContain("'--tools[Tools]:value:_aery_tools'");
		expect(out).toContain("'(-r --resume)'{-r,--resume}'[Resume]:session:_aery_call sessions'");
		...
		expect(out).toContain("_aery_tools() { _values -s , 'tools' read bash }");
	});

	it("dispatches aliased subcommands and completes positional enums", () => {
		expect(out).toContain("worktree|wt) _aery_cmd_worktree ;;");
...
describe("generateCompletion — fish", () => {
	...
	it("declares the no-subcommand predicate over every command token", () => {
		expect(out).toContain("function __fish_aery_no_subcommand");
	...
	it("maps value sources to fish completion args", () => {
		expect(out).toContain("-l model -d 'Model to use' -x -a '(command aery __complete models -- (commandline -ct))'");
		expect(out).toContain("-l thinking -d 'Effort' -x -a 'low high'");
		expect(out).toContain("-l tools -d 'Tools' -x -a 'read bash'");
		expect(out).toContain("-s r -l resume -d 'Resume' -x -a '(command aery __complete sessions");
...
describe.skip("aery completions (integration / drift)", () => {
	it("emits a zsh script reflecting the live command + flag surface", async () => {
		...
		expect(stdout).toContain("_aery_cmd_commit");
		expect(stdout).toContain("'completions:");
		// zsh routes single-value dynamic flags through the _aery_call action, which
		// itself shells out to `aery __complete $kind`.
		expect(stdout).toContain("_aery_call models");
		expect(stdout).toContain("_aery_call sessions");
		expect(stdout).toContain("command aery __complete $kind");
		// Hidden/default commands must NOT surface as completable subcommands.
		expect(stdout).not.toContain("_aery_cmd_launch");
		expect(stdout).not.toContain("_aery_cmd___complete");
	});
});
