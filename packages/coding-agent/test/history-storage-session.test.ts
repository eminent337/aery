...
async function freshStorage(prefix = "aery-history-session-"): Promise<{ storage: HistoryStorage; dbPath: string }> {
...
describe("HistoryStorage session linkage", () => {
	...
	it("adds session_id to a pre-existing schema and leaves legacy rows unstamped", async () => {
		tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "aery-history-session-migrate-"));
		const dbPath = path.join(tempDir, "history.db");
