...
import { KeybindingsManager } from "@aryee337/aery/config/keybindings";
import { ExtensionList } from "@aryee337/aery/modes/components/extensions/extension-list";
import type { Extension } from "@aryee337/aery/modes/components/extensions/types";
import { HistorySearchComponent } from "@aryee337/aery/modes/components/history-search";
import { SessionSelectorComponent } from "@aryee337/aery/modes/components/session-selector";
import { TreeSelectorComponent } from "@aryee337/aery/modes/components/tree-selector";
import { UserMessageSelectorComponent } from "@aryee337/aery/modes/components/user-message-selector";
import { initTheme } from "@aryee337/aery/modes/theme/theme";
import { HistoryStorage } from "@aryee337/aery/session/history-storage";
import type { SessionInfo, SessionTreeNode } from "@aryee337/aery/session/session-manager";
import type { AgentMessage } from "@aryee337/aery-core";
import { setKeybindings } from "@aryee337/aery-tui";
...
async function createHistoryStorage(prompts: string[]): Promise<HistoryStorage> {
	const dir = await fs.mkdtemp(path.join(os.tmpdir(), "aery-history-nav-"));
	tempDirs.push(dir);
	HistoryStorage.resetInstance();
