/**
 * Regression test for #1496 (bug 2): `aery install ./my-extension` used to be
...
describe("looksLikeLocalPath", () => {
	...
	test("npm specs and marketplace refs are remote", () => {
		expect(looksLikeLocalPath("@aryee337/exa")).toBe(false);
	...
	test("bare names that exist as a local directory are treated as local", () => {
		const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "aery-install-test-"));
