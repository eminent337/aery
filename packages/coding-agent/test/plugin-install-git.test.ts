 ...
 *
 * Strategy: spy on the six `@aryee337/aery-utils` plugin-path getters so the
 ...
 * `piUtils.getInstallId`), so we know namespace spying on `aery-utils` exports
...
import { PluginManager } from "@aryee337/aery/extensibility/plugins/manager";
...
describe("PluginManager.install with git sources", () => {
	...
	beforeEach(async () => {
		tmpRoot = await fs.mkdtemp(path.join(os.tmpdir(), "aery-plugin-git-"));
		...
		vi.spyOn(piUtils, "getPluginsLockfile").mockReturnValue(path.join(tmpRoot, "aery-plugins.lock.json"));
	...
	test("installs from github: shorthand and resolves real package name from deps diff", async () => {
		...
		await Bun.write(
			pluginsPkgJson,
			JSON.stringify({ name: "aery-plugins", private: true, dependencies: {} }, null, 2),
		);
