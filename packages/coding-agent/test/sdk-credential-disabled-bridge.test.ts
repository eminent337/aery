...
import type { Extension, ExtensionError, ExtensionFactory } from "@aryee337/aery/extensibility/extensions";
import { ExtensionRunner } from "@aryee337/aery/extensibility/extensions";
import { ExtensionRuntime } from "@aryee337/aery/extensibility/extensions/loader";
import { createAgentSession } from "@aryee337/aery/sdk";
import { SessionManager } from "@aryee337/aery/session/session-manager";
import { AuthStorage, type CredentialDisabledEvent } from "@aryee337/aery-ai";
import * as oauthUtils from "@aryee337/aery-ai/utils/oauth";
...
describe("createAgentSession credential_disabled subscription", () => {
	...
	const makeDirs = (label: string): SessionDirs => {
		const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), `aery-credential-disabled-${label}-${Snowflake.next()}-`));
	...
	const makeRecordingExtension = () => {
		...
		const factory: ExtensionFactory = aery => {
			aery.on("credential_disabled", event => {
