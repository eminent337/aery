...
describe("issue #1215: legacy @eminent337/aery-ai imports survive getResolvedSpecifier failure", () => {
	let projectDir: TempDir;
	let extensionPath: string;

	beforeEach(() => {
		projectDir = TempDir.createSync("@issue-1215-");
		const pluginDir = path.join(projectDir.path(), "aery-schedule-like-plugin");
		extensionPath = path.join(pluginDir, "index.ts");
		fs.mkdirSync(pluginDir, { recursive: true });

		// Mirrors the import pattern used by aery-schedule-prompt@0.3.0, which was
		// the reporter's failing plugin. z is a runtime value re-exported from
		// @aryee337/aery-ai so using it forces the import to be resolved at load time.
		fs.writeFileSync(
			extensionPath,
			[
