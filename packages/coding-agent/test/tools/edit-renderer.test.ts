...
import { editToolRenderer } from "@aryee337/aery/edit/renderer";
import { ToolExecutionComponent } from "@aryee337/aery/modes/components/tool-execution";
import * as themeModule from "@aryee337/aery/modes/theme/theme";
import type { AgentTool } from "@aryee337/aery-core";
import { Text, type TUI } from "@aryee337/aery-tui";
import { InMemorySnapshotStore } from "@aryee337/hashline";
...
describe("editToolRenderer", () => {
	...
	it("shows hashline envelope target path while preview diff is not computable yet", async () => {
		...
		const rendered = Bun.stripANSI(component.render(160).join("\n"));
		expect(rendered).toContain("crates/aery-engine/src/shell.rs");
