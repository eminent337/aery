...
} from "@aryee337/aery/tools/gh";
import * as git from "@aryee337/aery/utils/git";
import { getAgentDir, hashPath, setAgentDir } from "@aryee337/aery-utils";
...
/**
 * Stub `os.homedir()` AND rebuild the cached `dirs` resolver in aery-utils so
 * `getWorktreesDir()` resolves under an isolated temp home instead of the
 * user's real `~/.aery/wt`. Returns the temp home and a cleanup hook.
 */
async function setupTempHome(): Promise<{ home: string; cleanup: () => Promise<void> }> {
	...
	setAgentDir(path.join(home, ".aery", "agent"));
	return {
		home,
		cleanup: async () => {
...
async function expectedWorktreePath(home: string, primaryRoot: string, localBranch: string): Promise<string> {
	...
	return fs.realpath(path.join(home, ".aery", "wt", segment));
}
...
describe("github tool", () => {
	...
	it("checks out multiple pull requests in a single call when pr is an array", async () => {
		...
		try {
			...
			expect(runGit(fixture.repoRoot, ["config", "--get", "branch.pr-100.aeryPrUrl"])).toBe(
				"https://github.com/owner/repo/pull/100",
			);
			expect(runGit(fixture.repoRoot, ["config", "--get", "branch.pr-200.aeryPrUrl"])).toBe(
