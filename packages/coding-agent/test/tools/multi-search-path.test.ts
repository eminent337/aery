...
import { resolveExplicitSearchPaths } from "@aryee337/aery/tools/path-utils";
...
describe.skipIf(isWindows)("search across unrelated filesystem trees", () => {
	...
	beforeEach(async () => {
		...
		dirA = await fs.mkdtemp(path.join("/tmp", "aery-search-multi-A-"));
		dirB = await fs.mkdtemp(path.join("/var/tmp", "aery-search-multi-B-"));
		cwd = await fs.mkdtemp(path.join(os.tmpdir(), "aery-search-multi-cwd-"));
