...
import { __resetAutoQaFlushStateForTests, flushGrievances } from "@aryee337/aery/tools/report-tool-issue";
...
describe("flushGrievances", () => {
	...
	it("posts pending rows with bearer header and marks them pushed=1 on 200", async () => {
		...
		expect(body.agent?.name).toBe("aery");
		...
		// stays so users can browse what they've shipped via `aery grievances`.
