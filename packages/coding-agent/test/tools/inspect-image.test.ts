...
import { InspectImageTool } from "@aryee337/aery/tools/inspect-image";
import { inspectImageToolRenderer } from "@aryee337/aery/tools/inspect-image-renderer";
import { toolRenderers } from "@aryee337/aery/tools/renderers";
import type { completeSimple, Model } from "@aryee337/aery-ai";
import { sanitizeText } from "@aryee337/aery-utils";
...
describe("InspectImageTool", () => {
	...
	beforeEach(() => {
		testDir = fs.mkdtempSync(path.join(os.tmpdir(), "aery-inspect-image-"));
	});
	...
	it("falls back to aery/default when vision role is unset", async () => {
		const imagePath = path.join(testDir, "screen.png");
		fs.writeFileSync(imagePath, Buffer.from(TINY_PNG_BASE64, "base64"));
