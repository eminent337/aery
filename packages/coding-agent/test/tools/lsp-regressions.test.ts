...
import { preloadPluginRoots } from "@aryee337/aery/discovery/helpers";
import { LspTool } from "@aryee337/aery/lsp";
import * as lspClient from "@aryee337/aery/lsp/client";
import * as lspConfig from "@aryee337/aery/lsp/config";
import { getServersForFile, loadConfig } from "@aryee337/aery/lsp/config";
import { applyWorkspaceEdit } from "@aryee337/aery/lsp/edits";
import { renderCall, renderResult } from "@aryee337/aery/lsp/render";
import type {
	CodeAction,
	CreateFile,
	DeleteFile,
...
import {
	...
	resolveSymbolColumn,
} from "@aryee337/aery/lsp/utils";
import { getThemeByName } from "@aryee337/aery/modes/theme/theme";
import type { ToolSession } from "@aryee337/aery/tools";
import { clampTimeout } from "@aryee337/aery/tools/tool-timeouts";
import type { RenderResultOptions } from "@aryee337/aery-core";
import * as piUtils from "@aryee337/aery-utils";
import { sanitizeText, TempDir } from "@aryee337/aery-utils";
...
describe("lsp regressions", () => {
	...
	it("sends the LSP exit notification after shutdown completes", async () => {
		const tempDir = TempDir.createSync("@aery-lsp-shutdown-");
		try {
...
`,
	...
	it("limits glob collection to avoid large diagnostic stalls", async () => {
		const tempDir = TempDir.createSync("@aery-lsp-glob-");
		try {
			await Promise.all([
	...
	it("treats existing bracket paths as literal diagnostic targets", async () => {
		const tempDir = TempDir.createSync("@aery-lsp-bracket-path-");
		try {
	...
	it("resolves the requested symbol occurrence on a line", async () => {
		const tempDir = TempDir.createSync("@aery-lsp-regression-");
		try {
	...
	it("throws when symbol does not exist on the target line", async () => {
		const tempDir = TempDir.createSync("@aery-lsp-missing-symbol-");
		try {
			const filePath = `${tempDir.path()}/symbol.ts`;
	...
	it("throws when occurrence is out of bounds", async () => {
		const tempDir = TempDir.createSync("@aery-lsp-occurrence-");
		try {
			const filePath = `${tempDir.path()}/symbol.ts`;
	...
	it("does not reuse stale file diagnostics after another URI publishes", async () => {
		const tempDir = TempDir.createSync("@aery-lsp-stale-diags-");
		try {
	...
	it("detects Windows local .exe LSP shims in node_modules/.bin", async () => {
		...
		const tempDir = TempDir.createSync("@aery-lsp-win32-bin-");
	...
	it("detects tlaplus files for LSP startup and language ids", async () => {
		const tempDir = TempDir.createSync("@aery-lsp-tlaplus-");
	...
	it("loads config-only marketplace LSP servers from Claude plugin cache", async () => {
		const tempDir = TempDir.createSync("@aery-lsp-marketplace-config-");
	...
	it("rename_file applies LSP willRenameFiles edits and renames the file", async () => {
		const tempDir = TempDir.createSync("@aery-lsp-rename-file-");
		try {
	...
	it("rename_file with apply:false previews edits without filesystem changes", async () => {
		const tempDir = TempDir.createSync("@aery-lsp-rename-file-preview-");
		try {
	...
	it("rename_file enumerates every file inside a directory rename", async () => {
		const tempDir = TempDir.createSync("@aery-lsp-rename-dir-");
		try {
	...
	it("request action sends raw LSP method with auto-built textDocument/position params", async () => {
		const tempDir = TempDir.createSync("@aery-lsp-request-");
		try {
	...
	it("request action forwards explicit JSON payload verbatim", async () => {
		const tempDir = TempDir.createSync("@aery-lsp-request-payload-");
		try {
			const server: ServerConfig = { command: "test-lsp", fileTypes: ["ts"], rootMarkers: [] };
			const client: LspClient = {
				name: "test-lsp",
	...
	it("capabilities action dumps server capabilities", async () => {
		const tempDir = TempDir.createSync("@aery-lsp-caps-");
		try {
			const server: ServerConfig = { command: "test-lsp", fileTypes: ["ts"], rootMarkers: [] };
			const client: LspClient = {
				name: "test-lsp",
	...
	it("flushes pending descendant text edits before a folder rename", async () => {
		const tempDir = TempDir.createSync("@aery-lsp-folder-rename-");
		try {
	...
	it("flushes pending edits queued against a rename target before performing the rename", async () => {
		...
		const tempDir = TempDir.createSync("@aery-lsp-rename-target-prefill-");
		try {
	...
	it("resolves $-prefixed identifiers past compound matches", async () => {
		...
		const tempDir = TempDir.createSync("@aery-lsp-dollar-identifier-");
		try {
	...
	it("applies a create op followed by a text edit for the same URI in declared order", async () => {
		...
		const tempDir = TempDir.createSync("@aery-lsp-create-then-edit-");
		try {
	...
	it("flushes pending descendant text edits before a folder delete", async () => {
		...
		const tempDir = TempDir.createSync("@aery-lsp-folder-delete-");
		try {
			const srcDir = path.join(tempDir.path(), "src");
			fs.mkdirSync(srcDir, { recursive: true });
			const childPath = path.join(srcDir, "a.ts");
