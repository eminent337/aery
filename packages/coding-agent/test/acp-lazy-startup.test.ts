...
describe("ACP lazy startup", () => {
	it("keeps ACP background jobs disabled by default and preserves explicit opt-ins", async () => {
		...
		const runAcpStartup = async (settings: Settings): Promise<ObservedBackgroundSettings> => {
			using tempDir = TempDir.createSync("@aery-acp-background-settings-");
		...
		try {
			...
			expect(initializeResponse).toEqual(
				expect.objectContaining({
					protocolVersion: 1,
					agentInfo: expect.objectContaining({ name: "aery" }),
				}),
			);
	...
	it("applies CLI runtime API keys after ACP lazy session creation resolves extension models", async () => {
		using tempDir = TempDir.createSync("@aery-acp-lazy-api-key-");
		const cwd = tempDir.path();
