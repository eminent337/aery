...
import type { AssistantThinkingRenderer } from "@aryee337/aery/extensibility/extensions";
import { AssistantMessageComponent } from "@aryee337/aery/modes/components/assistant-message";
import { clearMermaidCache } from "@aryee337/aery/modes/theme/mermaid-cache";
import { initTheme } from "@aryee337/aery/modes/theme/theme";
import type { AssistantMessage } from "@aryee337/aery-ai";
import { ImageProtocol, setTerminalImageProtocol, TERMINAL, Text } from "@aryee337/aery-tui";
...
describe("AssistantMessageComponent thinking renderers", () => {
	...
	it("does not invoke extension renderers when thinking is hidden", () => {
		...
		const rendered = Bun.stripANSI(component.render(120).join("\n"));
		// The thinking block is completely hidden. The UI relies on the EventController spinner to show 'Thinking...'
