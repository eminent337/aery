...
import { OmfgController } from "@aryee337/aery/modes/controllers/omfg-controller";
import { initTheme } from "@aryee337/aery/modes/theme/theme";
import type { InteractiveModeContext } from "@aryee337/aery/modes/types";
import type { AssistantMessage, Usage } from "@aryee337/aery-ai";
import type { AgentMessage } from "@aryee337/aery-core";
import { Container, type TUI } from "@aryee337/aery-tui";

const PROJECT_OPTION = "This project (.aery/rules)";
const GLOBAL_OPTION = "Global — all projects (~/.aery/agent/rules)";
...
describe("OmfgController", () => {
	it("saves a matching generated rule under project rules and registers it live", async () => {
		...
		const savedPath = path.join(harness.projectDir, ".aery", "rules", "ts-no-any.md");
	...
	it("reiterates when the first valid rule does not match history", async () => {
		...
		expect(await Bun.file(path.join(harness.projectDir, ".aery", "rules", "ts-no-any.md")).exists()).toBe(true);
	});
	...
	it("asks before saving when validation never confirms a match", async () => {
		...
		expect(await Bun.file(path.join(harness.projectDir, ".aery", "rules", "no-match.md")).exists()).toBe(false);
	});
	...
	it("lets the user amend from the save selector before writing the rule", async () => {
		...
		expect(await Bun.file(path.join(harness.projectDir, ".aery", "rules", "ts-any-broad.md")).exists()).toBe(false);
		expect(await Bun.file(path.join(harness.projectDir, ".aery", "rules", "ts-no-explicit-any.md")).exists()).toBe(
			true,
		);
	});
	...
	it("clears the panel and aborts the inner request on Escape", async () => {
		...
		expect(await Bun.file(path.join(harness.projectDir, ".aery", "rules", "ts-no-any.md")).exists()).toBe(false);
	});
});
