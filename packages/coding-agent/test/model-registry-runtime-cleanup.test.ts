...
import { type AssistantMessageEventStream, clearCustomApis, getCustomApi } from "@aryee337/aery-ai";
...
describe("ModelRegistry runtime source cleanup", () => {
	...
	beforeEach(async () => {
		tempDir = path.join(os.tmpdir(), `aery-test-model-registry-runtime-cleanup-${Snowflake.next()}`);
