...
import { installAeryModuleResolver, loadAeryExtensionModule } from "../../src/extensibility/plugins/upstream-compat";
...
async function writeFixtureExtension(source: string): Promise<string> {
	const dir = await fs.mkdtemp(path.join(os.tmpdir(), "aery-typebox-remap-"));
	tempRoots.push(dir);
...
describe("upstream-compat @sinclair/typebox remap", () => {
