...
import { __computeBunfsPackageRoot } from "../../src/extensibility/plugins/upstream-compat";

// Regression for issue #1514: legacy aery compat shim paths were built from a
...
describe("legacy aery compat bunfs root computation (issue #1514)", () => {
	it("appends packages to the Windows-native compiled bunfs root", () => {
		...
		expect(path.win32.join(root, "coding-agent", "src", "extensibility", "upstream-ai-shim.js")).toBe(
			"B:\\~BUN\\root\\packages\\coding-agent\\src\\extensibility\\upstream-ai-shim.js",
		);
	});
