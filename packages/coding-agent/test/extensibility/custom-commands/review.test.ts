...
describe("ReviewCommand", () => {
	...
	async function createTempDir(): Promise<string> {
		tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "aery-review-command-"));
		return tmpDir;
	}
	...
	async function createGitRepoWithUncommittedChange(): Promise<string> {
		...
		await $`git config user.name Aery Test`.cwd(dir).quiet();
		await $`git config user.email aery-test@example.com`.cwd(dir).quiet();
