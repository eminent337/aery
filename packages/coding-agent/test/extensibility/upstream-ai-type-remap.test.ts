...
import {
	__resetAeryModuleResolverCache,
	installAeryModuleResolver,
	loadAeryExtensionModule,
} from "../../src/extensibility/plugins/upstream-compat";
import { Type as TypeBoxShimType } from "../../src/extensibility/typebox";

// aery-ai 15.1.0 removed the runtime `Type` export from `@aryee337/aery-ai`'s
// package root. Legacy extensions (and their aliased-scope variants such as
// `legacy scope`) still author parameter schemas as
// `import { Type } from "legacy scope"` and then `Type.Object(...)`.
// `upstream-compat.ts` patches that gap by redirecting bare aery-ai root
// imports through `upstream-ai-shim.ts`, which re-exports the canonical
// aery-ai surface plus the Zod-backed `Type` runtime from the same TypeBox shim
...
async function writeFixtureExtension(source: string): Promise<string> {
	const dir = await fs.mkdtemp(path.join(os.tmpdir(), "aery-aery-ai-type-remap-"));
	tempRoots.push(dir);
...
describe.skip("legacy-aery @(scope)/aery-ai root `Type` remap (issue #1437)", () => {
	...
	it('redirects `import { Type } from "@aryee337/aery-ai"` for plugins published against the canonical scope', async () => {
		const entry = await writeFixtureExtension(
			['import { Type } from "@aryee337/aery-ai";', "export const probe = Type;"].join("\n"),
		);

		const loaded = (await loadAeryExtensionModule(entry)) as { probe: typeof TypeBoxShimType };
		expect(loaded.probe).toBe(TypeBoxShimType);
	});

	it("preserves canonical aery-ai exports alongside the shimmed Type (z is still re-exported)", async () => {
		const entry = await writeFixtureExtension(
			[
				'import { Type, z } from "legacy scope";',
	...
	it("does not redirect subpath imports such as @aryee337/aery-ai/utils/schema", async () => {
		const entry = await writeFixtureExtension(
			[
				...
				// against the bundled aery-ai package rather than the shim.
				'import { zodToWireSchema } from "@aryee337/aery-ai/utils/schema";',
				"export const fn = zodToWireSchema;",
			].join("\n"),
		);

		const loaded = (await loadAeryExtensionModule(entry)) as { fn: unknown };
...
describe.skip("legacy aery package root remaps (issue #1474)", () => {
	it("loads legacy scope root imports when host package resolution is unavailable", async () => {
		...
		vi.spyOn(Bun, "resolveSync").mockImplementation((specifier: string, from: string) => {
			if (specifier === "@aryee337/aery" && from.endsWith(path.join("src", "extensibility", "plugins"))) {
				throw new Error("compiled binary host package resolution unavailable");
			}
			return realResolveSync(specifier, from);
		});
		const entry = await writeFixtureExtension(
			['import { VERSION } from "legacy scope";', "export const loadedVersion = VERSION;"].join("\n"),
		);

		const loaded = (await loadAeryExtensionModule(entry)) as { loadedVersion: string };
	...
	it("falls back to legacy-scoped subpath peers for direct plugin imports", async () => {
		const realResolveSync = Bun.resolveSync.bind(Bun);
		vi.spyOn(Bun, "resolveSync").mockImplementation((specifier: string, from: string) => {
			if (specifier === "@aryee337/aery-ai/utils/oauth") {
				throw new Error(`canonical peer unavailable from ${from}`);
			}
			return realResolveSync(specifier, from);
		});

		const dir = await fs.mkdtemp(path.join(os.tmpdir(), "aery-legacy-direct-subpath-"));
		tempRoots.push(dir);
		const packageDir = path.join(dir, "node_modules", "@eminent337", "aery-ai");
		...
		await fs.writeFile(
			entry,
			['import { marker } from "@eminent337/aery-ai/oauth";', "export const loadedMarker = marker;"].join("\n"),
			"utf8",
		);
	...
	it("routes legacy scope through canonical Bun.resolveSync in non-compiled mode", async () => {
		// Regression: when aery runs from a node_modules install (not the monorepo
		// and not a compiled binary), the bundled packages live at
		// `node_modules/@aryee337/aery-*`, not next to the source tree. Hardcoding
		...
		vi.spyOn(Bun, "resolveSync").mockImplementation((specifier: string, from: string) => {
			if (specifier === "@aryee337/aery-utils") {
				canonicalLookupSeen = true;
			}
			return realResolveSync(specifier, from);
		});
		const entry = await writeFixtureExtension(
			['import { isCompiledBinary } from "legacy scope";', "export const probe = isCompiledBinary;"].join("\n"),
		);

		const loaded = (await loadAeryExtensionModule(entry)) as { probe: () => boolean };
