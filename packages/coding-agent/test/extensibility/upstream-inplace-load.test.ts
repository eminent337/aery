...
import { loadAeryExtensionModule } from "../../src/extensibility/plugins/upstream-compat";

// Issue #1674: legacy Aery extensions load browser-UI assets (HTML/CSS) at module
...
// public `loadAeryExtensionModule` entry point.
...
async function writePackage(files: Record<string, string>): Promise<string> {
	const dir = await fs.mkdtemp(path.join(os.tmpdir(), "aery-legacy-inplace-"));
	tempRoots.push(dir);
...
describe("legacy-aery in-place module loading (issue #1674)", () => {
	it("reads __dirname-relative HTML assets from the real extension directory", async () => {
		const dir = await writePackage({
			...
			"index.ts": [
				...
				"export default function (aery) { void aery; }",
			].join("\n"),
		});

		const mod = (await loadAeryExtensionModule(path.join(dir, "index.ts"))) as { dirName: string; html: string };
	...
	it("resolves a .css sibling of a relatively-imported submodule", async () => {
		const dir = await writePackage({
			...
			"index.ts": ['export { css } from "./sub/widget.ts";', "export default function (aery) { void aery; }"].join(
				"\n",
			),
		});

		const mod = (await loadAeryExtensionModule(path.join(dir, "index.ts"))) as { css: string };
	...
	it("loads the extension's own node_modules deps natively while remapping legacy aery imports", async () => {
		const dir = await writePackage({
			...
			"index.ts": [
				...
				// legacy scope alias with no real published package,
		...
		const mod = (await loadAeryExtensionModule(path.join(dir, "index.ts"))) as { depValue: string; hasZod: boolean };

		// CJS dep under node_modules keeps Bun's native resolution (it is excluded
		// from the rewrite onLoad), and the legacy aery import is remapped to the
	...
	it("rewrites legacy imports in ../src modules reached through relative imports", async () => {
		const dir = await writePackage({
			...
			"src/helper.ts": [
				'import { isCompiledBinary } from "legacy scope";',
		...
		const mod = (await loadAeryExtensionModule(path.join(dir, "dist", "extension.ts"))) as { ok: boolean };
	...
	it("does not rewrite sibling files outside the loaded extension's import graph", async () => {
		const dir = await writePackage({
			...
			"index.ts": ['export { local } from "./local.ts";', "export default function (aery) { void aery; }"].join(
				"\n",
			),
			"local.ts": 'export const local = "local-ok";',
			// Not imported by index.ts, so it must stay outside the rewrite scope.
			// Uses a non-standard import that should fail to resolve.
			"unrelated.ts": [
				'import { z } from "nonexistent-scope/nonexistent-pkg";',
				'export const hasZod = typeof z?.object === "function";',
			].join("\n"),
		});

		const entryMod = (await loadAeryExtensionModule(path.join(dir, "index.ts"))) as { local: string };
		expect(entryMod.local).toBe("local-ok");

		// Loading the un-imported sibling directly must NOT benefit from the
		// extension's rewrite hook; its non-standard import stays unresolved.
		const siblingUrl = `${url.pathToFileURL(await fs.realpath(path.join(dir, "unrelated.ts"))).href}?nonce=${Date.now()}`;
		await expect(import(siblingUrl)).rejects.toThrow();
	});
});
