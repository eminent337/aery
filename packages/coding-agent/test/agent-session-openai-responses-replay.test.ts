...
import { type SessionEntry, SessionManager, type SessionMessageEntry } from "@aryee337/aery/session/session-manager";
...
import type {
	...
	ToolResultMessage,
	Usage,
} from "@aryee337/aery-ai/types";
import { createOpenAIResponsesHistoryPayload } from "@aryee337/aery-ai/utils";
...
async function createSessionHarness(
	...
	const [{ createAgentSession }, { Settings }, { AuthStorage }] = await Promise.all([
		import("@aryee337/aery/sdk"),
		import("@aryee337/aery/config/settings"),
		import("@aryee337/aery/session/auth-storage"),
	]);
...
describe("AgentSession OpenAI Responses replay boundaries", () => {
	...
	it("sanitizes stale assistant replay metadata during startup resume while preserving user payloads", async () => {
		const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), `aery-issue-505-startup-${Snowflake.next()}-`));
	...
	it("sanitizes stale Responses-family assistant replay metadata for direct SessionManager.open consumers", async () => {
		const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), `aery-issue-505-open-${Snowflake.next()}-`));
		tempDirs.push(tempDir);
	...
	it("sanitizes stale assistant replay metadata when forking a persisted session", async () => {
		const sourceDir = fs.mkdtempSync(path.join(os.tmpdir(), `aery-issue-505-fork-source-${Snowflake.next()}-`));
		const forkDir = fs.mkdtempSync(path.join(os.tmpdir(), `aery-issue-505-fork-target-${Snowflake.next()}-`));
	...
	it("keeps same-file reload safe without resetting live provider state after startup sanitization", async () => {
		const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), `aery-issue-505-reload-${Snowflake.next()}-`));
		tempDirs.push(tempDir);
	...
	it("keeps provider session state when same-file reload only changes message metadata", async () => {
		const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), `aery-issue-505-reload-metadata-${Snowflake.next()}-`));
		tempDirs.push(tempDir);
	...
	it("captures session-manager state when custom message details are proxy-backed", async () => {
		const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), `aery-issue-505-capture-proxy-${Snowflake.next()}-`));
		tempDirs.push(tempDir);
		const sessionManager = SessionManager.create(tempDir, tempDir);
	...
	it("reloads when current session contains proxy-backed custom message details", async () => {
		const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), `aery-issue-505-reload-proxy-${Snowflake.next()}-`));
		tempDirs.push(tempDir);
		const sessionManager = SessionManager.create(tempDir, tempDir);
	...
	it("resets provider session state when same-file reload restores different messages under the same model", async () => {
		const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), `aery-issue-505-reload-content-${Snowflake.next()}-`));
		tempDirs.push(tempDir);
	...
	it("resets provider session state when same-file reload restores a different saved model", async () => {
		const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), `aery-issue-505-reload-model-${Snowflake.next()}-`));
		tempDirs.push(tempDir);
	...
	it("resets plain openai-responses provider state when same-file reload restores a different saved model", async () => {
		const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), `aery-issue-505-reload-openai-${Snowflake.next()}-`));
		tempDirs.push(tempDir);
	...
	it("switches sessions without requiring write access during load-time sanitization", async () => {
		const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), `aery-issue-505-switch-fail-${Snowflake.next()}-`));
		tempDirs.push(tempDir);
	...
	it("clears provider session state and sanitizes loaded assistant metadata when switching sessions", async () => {
		const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), `aery-issue-505-switch-${Snowflake.next()}-`));
		tempDirs.push(tempDir);
		const preservedUserPayload = createUserHistoryPayload();
	...
	it("does not reintroduce stale assistant replay metadata when navigating to another branch after load sanitization", async () => {
		const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), `aery-issue-505-tree-${Snowflake.next()}-`));
		tempDirs.push(tempDir);
	...
	it("resets provider session state when starting a brand-new session", async () => {
		const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), `aery-issue-505-new-${Snowflake.next()}-`));
		tempDirs.push(tempDir);
		const sessionManager = SessionManager.create(tempDir, tempDir);
		const { session, authStorage } = await createSessionHarness(tempDir, sessionManager);
		sessions.push(session);
