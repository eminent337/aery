...
import { ExtensionRunner } from "@aryee337/aery/extensibility/extensions/runner";
...
import { getProjectAgentDir, TempDir, withTimeout } from "@aryee337/aery-utils";

const runtimeSignalStoreKey = "__aeryRuntimeSignals";
...
describe("AgentSession auto-compaction queue resume", () => {
	...
	beforeEach(async () => {
		tempDir = TempDir.createSync("@aery-auto-compaction-queue-");
		vi.useFakeTimers();
		...
		fs.writeFileSync(
			...
			[
				...
				'\taery.on("session_before_compact", async (event) => {',
				"\t\treturn {",
				"\t\t\tcompaction: {",
				'\t\t\t\tsummary: "compacted",',
				...
				"\t\t};",
				"\t});",
				'\taery.on("auto_compaction_start", async (event) => {',
				...
				"\t});",
				'\taery.on("auto_compaction_end", async (event) => {',
				`\t\tconst signals = globalThis.${runtimeSignalStoreKey} ?? (globalThis.${runtimeSignalStoreKey} = []);`,
				'\t\tsignals.push("compaction:end:" + (event.aborted ? "aborted" : "ok"));',
				"\t});",
				'\taery.on("todo_reminder", async (event) => {',
				`\t\tconst signals = globalThis.${runtimeSignalStoreKey} ?? (globalThis.${runtimeSignalStoreKey} = []);`,
