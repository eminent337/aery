...
import {
	...
	parseDefaultBranch,
	parseGitHubRepo,
} from "@aryee337/aery/modes/components/status-line/git-utils";

describe("parseGitHubRepo", () => {
	test("parses HTTPS URL", () => {
		expect(parseGitHubRepo("https://github.com/eminent337/aery.git")).toBe("eminent337/aery");
	});

	test("parses HTTPS URL without .git suffix", () => {
		expect(parseGitHubRepo("https://github.com/eminent337/aery")).toBe("eminent337/aery");
	});

	test("parses SSH scp-style URL", () => {
		expect(parseGitHubRepo("git@github.com:loftiskg/aery.git")).toBe("loftiskg/aery");
	});

	test("parses SSH scp-style URL without .git suffix", () => {
		expect(parseGitHubRepo("git@github.com:loftiskg/aery")).toBe("loftiskg/aery");
	});
