...
import type { Skill } from "@aryee337/aery/sdk";
...
describe("createAgentSession skills option", () => {
	...
	beforeEach(() => {
		tempDir = path.join(os.tmpdir(), `aery-sdk-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
		// Create skill in .aery/skills/ for native project-level discovery
		skillsDir = path.join(tempDir, ".aery", "skills", "test-skill");
		fs.mkdirSync(skillsDir, { recursive: true });
		originalHome = process.env.HOME;
		tempHomeDir = fs.mkdtempSync(path.join(os.tmpdir(), "aery-sdk-home-"));
		process.env.HOME = tempHomeDir;
		const nativeUserSkillsDir = path.join(tempHomeDir, ".aery", "agent", "skills");
		fs.mkdirSync(nativeUserSkillsDir, { recursive: true });

		// Create a test skill in the aery skills directory
		fs.writeFileSync(
...
`,
	...
	it("should still discover project skills when user skills directory is missing", async () => {
		const userAgentDir = path.join(tempHomeDir, ".aery", "agent");
