...
async function makeTempDir(): Promise<string> {
	const dir = await fs.mkdtemp(path.join(os.tmpdir(), "aery-update-test-"));
	tempDirs.push(dir);
	return dir;
}
...
describe("update-cli install target detection", () => {
	it("uses bun update when prioritized aery is inside bun global bin", () => {
		const method = resolveUpdateMethodForTest("/Users/test/.bun/bin/aery", "/Users/test/.bun/bin");

		expect(method).toBe("bun");
	});

	it("uses binary update when prioritized aery is outside bun global bin", () => {
		const method = resolveUpdateMethodForTest("/Users/test/.local/bin/aery", "/Users/test/.bun/bin");

		expect(method).toBe("binary");
	});

	it("uses binary update when bun global bin cannot be resolved", () => {
		const method = resolveUpdateMethodForTest("/Users/test/.local/bin/aery", undefined);
...
describe("update-cli bun install command", () => {
	it("pins the official npm registry and bypasses the manifest cache so a stale mirror or snapshot cannot mask a freshly published version", () => {
		// Regression: aery queries https://registry.npmjs.org/<pkg>/latest directly.
		...
		//     the release.
		// See https://github.com/eminent337/aery/issues/1686.
		expect(buildBunInstallArgs("15.7.6")).toEqual([
			"install",
			"-g",
			"--no-cache",
			"--registry=https://registry.npmjs.org/",
			"@aryee337/aery@15.7.6",
		]);
	});
});
...
describe("update-cli binary replacement", () => {
	it("restores the previous binary when the replacement fails verification", async () => {
		...
		const targetPath = path.join(dir, "aery");
		...
		).rejects.toThrow("restored previous aery binary");
