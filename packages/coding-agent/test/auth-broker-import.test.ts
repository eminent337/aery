...
import { getAgentDbPath, setAgentDir } from "@aryee337/aery-utils";
...
describe("auth-broker import (CLIProxyAPI)", () => {
	...
	beforeEach(async () => {
		originalAgentDir = process.env.AERY_AGENT_DIR;
		agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "aery-import-agent-"));
		cliproxyDir = await fs.mkdtemp(path.join(os.tmpdir(), "aery-import-cliproxy-"));
		setAgentDir(agentDir);
	});

	afterEach(async () => {
		process.stdout.write = ORIGINAL_STDOUT_WRITE;
		if (originalAgentDir === undefined) delete process.env.AERY_AGENT_DIR;
		else process.env.AERY_AGENT_DIR = originalAgentDir;
	...
	test("imports a directory of CLIProxyAPI JSONs and maps types to aery providers", async () => {
...
describe("auth-broker import (broker-routed)", () => {
	...
	beforeEach(async () => {
		savedEnv.AERY_AUTH_BROKER_URL = process.env.AERY_AUTH_BROKER_URL;
		savedEnv.AERY_AUTH_BROKER_TOKEN = process.env.AERY_AUTH_BROKER_TOKEN;
		agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "aery-import-client-"));
		brokerAgentDir = await fs.mkdtemp(path.join(os.tmpdir(), "aery-import-broker-"));
		cliproxyDir = await fs.mkdtemp(path.join(os.tmpdir(), "aery-import-cliproxy-broker-"));
		setAgentDir(agentDir);
		...
		process.env.AERY_AUTH_BROKER_URL = handle.url;
		process.env.AERY_AUTH_BROKER_TOKEN = token;
	});
	...
	afterEach(async () => {
		...
		await fs.rm(cliproxyDir, { recursive: true, force: true });
		for (const key of ["AERY_AUTH_BROKER_URL", "AERY_AUTH_BROKER_TOKEN"] as const) {
