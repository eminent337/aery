...
describe("AgentSession concurrent prompt guard", () => {
	...
	beforeEach(() => {
		tempDir = path.join(os.tmpdir(), `aery-concurrent-test-${Snowflake.next()}`);
	...
	// already processing` from aery-rpc clients (robomp triage reminder path).
...
describe("AgentSession TTSR resume gate", () => {
	...
	beforeEach(() => {
		tempDir = path.join(os.tmpdir(), `aery-ttsr-gate-test-${Snowflake.next()}`);
		fs.mkdirSync(tempDir, { recursive: true });
	});
