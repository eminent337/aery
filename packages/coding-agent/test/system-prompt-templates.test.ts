...
import { buildSystemPrompt, buildSystemPromptToolMetadata } from "@aryee337/aery/system-prompt";
import { type AgentTool, INTENT_FIELD } from "@aryee337/aery-core";
...
async function withTempDir(run: (dir: string) => Promise<void>): Promise<void> {
	const dir = await fs.mkdtemp(path.join(os.tmpdir(), "aery-system-prompt-"));
	try {
		await run(dir);
	} finally {
