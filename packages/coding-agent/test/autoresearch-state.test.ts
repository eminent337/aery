...
function makeTempDir(): string {
	const dir = path.join(os.tmpdir(), `aery-autoresearch-test-${Snowflake.next()}`);
...
describe("autoresearch slash command", () => {
	...
	beforeEach(() => {
		dbOverride = path.join(os.tmpdir(), `aery-autoresearch-cmd-${Snowflake.next()}`);
...
describe("autoresearch tool-call hook", () => {
	...
	beforeEach(() => {
		dbOverride = path.join(os.tmpdir(), `aery-autoresearch-hook-${Snowflake.next()}`);
		fs.mkdirSync(dbOverride, { recursive: true });
		process.env.AERY_AUTORESEARCH_DB_DIR = dbOverride;
		cleanups.push(dbOverride);
	});
