...
import { RedisSessionStorage, type RedisSessionStorageClient } from "@aryee337/aery/session/redis-session-storage";
...
describe("RedisSessionStorage", () => {
	...
	it("mirrors writeText into Redis and exposes content via sync reads", async () => {
		...
		expect(redis.strings.get("aery:sessions:file:/sessions/p/a.jsonl")).toBe("line1\nline2\n");
	...
	it("writer.writeLineSync appends to Redis after drain", async () => {
		...
		await storage.drain();
		expect(redis.strings.get("aery:sessions:file:/sessions/p/session.jsonl")).toBe(
			'{"type":"session"}\n{"type":"message"}\n',
		);
	...
	it("flags='w' truncates both mirror and Redis", async () => {
		...
		expect(redis.strings.get("aery:sessions:file:/sessions/p/keep.jsonl")).toBe("fresh\n");
	});
	...
	it("deleteSessionWithArtifacts removes JSONL plus any sidecar keys", async () => {
		...
		expect(redis.strings.has("aery:sessions:file:/sessions/p/s1.jsonl")).toBe(false);
		expect(redis.strings.has("aery:sessions:file:/sessions/p/s1/draft.txt")).toBe(false);
		expect(redis.strings.has("aery:sessions:file:/sessions/p/other.jsonl")).toBe(true);
	});
	...
	it("rename moves content and meta atomically inside the mirror", async () => {
		...
		expect(redis.strings.get("aery:sessions:file:/sessions/p/renamed.jsonl")).toBe("payload\n");
		expect(redis.strings.has("aery:sessions:file:/sessions/p/orig.jsonl")).toBe(false);
	});
	...
	it("refresh() reloads the mirror from Redis after out-of-band writes", async () => {
		...
		redis.strings.set("aery:sessions:file:/peer/x.jsonl", "from peer\n");
		const peerHash = redis.hashes.get("aery:sessions:meta") ?? new Map<string, string>();
		peerHash.set("/peer/x.jsonl", String(Date.now() + 5_000));
		redis.hashes.set("aery:sessions:meta", peerHash);
	...
	it("custom prefix isolates keyspaces", async () => {
		...
		expect(redis.strings.has("aery:sessions:file:/sessions/x.jsonl")).toBe(false);
	});
