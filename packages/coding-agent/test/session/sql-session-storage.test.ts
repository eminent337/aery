...
import { SqlSessionStorage, type SqlSessionStorageClient } from "@aryee337/aery/session/sql-session-storage";
import { SQL } from "bun";
...
describe("SqlSessionStorage (SQLite backend)", () => {
	it("mirrors writeText into SQL and exposes content via sync reads", async () => {
		...
		const rows = (await client.unsafe(`SELECT path, content FROM aery_session_files WHERE path = ?`, [
			"/sessions/p/a.jsonl",
	...
	it("writer.writeLineSync appends to SQL after drain", async () => {
		...
		await storage.drain();
		const rows = (await client.unsafe(`SELECT content FROM aery_session_files WHERE path = ?`, [
	...
	it("drain() surfaces writer errors so background failures are observable", async () => {
		...
		await client.unsafe("DROP TABLE aery_session_files");
	...
	it("deleteSessionWithArtifacts removes JSONL plus any sidecar keys", async () => {
		...
		const remaining = (await client.unsafe(`SELECT path FROM aery_session_files ORDER BY path`)) as Array<{
			path: string;
		}>;
	...
	it("rename moves content and mtime atomically inside the mirror and the DB", async () => {
		...
		const rows = (await client.unsafe(`SELECT path, content FROM aery_session_files`)) as Array<{
			path: string;
			content: string;
		}>;
	...
	it("refresh() reloads the mirror from SQL after out-of-band writes", async () => {
		...
		await client.unsafe(`INSERT INTO aery_session_files (path, content, mtime_ms) VALUES (?, ?, ?)`, [
			"/peer/x.jsonl",
			"from peer\n",
			Date.now() + 5_000,
		]);
	...
	it("LIKE special chars in artifact paths don't blow up the prefix sweep", async () => {
		...
		const remaining = (await client.unsafe(`SELECT path FROM aery_session_files`)) as Array<{ path: string }>;
	...
	it("createTable: false skips the DDL (consumer manages migrations)", async () => {
		...
		await client.unsafe(
			`CREATE TABLE aery_session_files (path TEXT PRIMARY KEY, content TEXT NOT NULL, mtime_ms INTEGER NOT NULL)`,
		);
