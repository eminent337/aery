...
import { discoverAndLoadExtensions, loadExtensions } from "@aryee337/aery/extensibility/extensions/loader";
import { getProjectAgentDir, TempDir } from "@aryee337/aery-utils";
...
describe("extensions discovery", () => {
	...
	beforeEach(() => {
		tempDir = TempDir.createSync("@aery-ext-test-");
	...
	const extensionCode = `
		export default function(aery) {
			aery.registerCommand("test", { handler: async () => {} });
		}
	`;
	...
	it("discovers subdirectory with package.json aery field", async () => {
	...
	it("package.json with aery field takes precedence over index.ts", async () => {
		const subdir = path.join(extensionsDir, "my-package");
		fs.mkdirSync(subdir);
	...
	it("ignores package.json without aery field, falls back to index.ts", async () => {
		const subdir = path.join(extensionsDir, "my-package");
		fs.mkdirSync(subdir);
		fs.writeFileSync(path.join(subdir, "index.ts"), extensionCode);
		fs.writeFileSync(
	...
	it("handles mixed direct files and subdirectories", async () => {
		...
		fs.writeFileSync(path.join(subdir2, "package.json"), JSON.stringify({ aery: { extensions: ["./entry.ts"] } }));
	...
	it("registers message renderers", async () => {
		const extCode = `
			export default function(aery) {
				aery.registerMessageRenderer("my-custom-type", (message, options, theme) => {
	...
	it("reports error when extension has no default export", async () => {
		const extCode = `
			export function notDefault(aery) {
				aery.registerCommand("test", { handler: async () => {} });
			}
		`;
	...
	it("loads extension with event handlers", async () => {
		const extCode = `
			export default function(aery) {
				...
				aery.on("tool_call", async (event) => undefined);
