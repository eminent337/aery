...
import { Effort, getBundledModel, type Model, type Effort as ThinkingLevelType } from "@aryee337/aery-ai";
...
describe.skipIf(!HAS_ANTIGRAVITY_AUTH)("Compaction with thinking models (Antigravity)", () => {
	...
	beforeEach(() => {
		tempDir = path.join(os.tmpdir(), `aery-thinking-compaction-test-${Snowflake.next()}`);
...
describe.skipIf(!HAS_ANTHROPIC_AUTH)("Compaction with thinking models (Anthropic)", () => {
	...
	beforeEach(() => {
		tempDir = path.join(os.tmpdir(), `aery-thinking-compaction-anthropic-test-${Snowflake.next()}`);
		fs.mkdirSync(tempDir, { recursive: true });
	});
