 ...
 * verified at the unit level by `aery-engine::shell::tests::child_session_action`
 * (truth-table) and `embedded_external_command_runs_in_its_own_session` (real
 * brush spawn). This test pulls the fix end-to-end through the AERY coding
 * agent stack:
 *
 *   AgentSession.prompt
 *     → Agent.prompt (real)
 ...
 *         → BashTool.execute
 *           → executeBash
 *             → aery-engine `Shell.run` (real native binding)
 ...
 * SIGTTIN/SIGTTOU the AERY host process.
 */
...
import { BashTool, type ToolSession } from "@aryee337/aery/tools";
import { getBundledModel } from "@aryee337/aery-ai";
import { createMockModel, type MockResponse } from "@aryee337/aery-ai/providers/mock";
import { Agent, type AgentMessage, type AgentTool } from "@aryee337/aery-core";
...
describe("BashTool through AgentSession runs children in their own session (e2e)", () => {
	...
	beforeEach(async () => {
		...
		tempDir = path.join(os.tmpdir(), `aery-bash-detach-test-${Snowflake.next()}`);
