...
import { defineRpcClientTool, RpcClient } from "@aryee337/aery/modes";
import { RpcHostToolBridge } from "@aryee337/aery/modes/rpc/host-tools";
import type {
	RpcHostToolCallRequest,
	RpcHostToolCancelRequest,
	RpcHostToolUpdate,
} from "@aryee337/aery/modes/rpc/rpc-types";
...
describe("RpcClient custom tools", () => {
	it("registers host custom tools and serves tool calls over the RPC transport", async () => {
		const scriptPath = path.join(os.tmpdir(), `aery-rpc-host-tools-${Date.now()}.js`);
