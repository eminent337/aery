 ...
 *
 * The native `aery` discovery provider only walks `.aery/` and `~/.aery/agent/`.
 * Extension packages registered via `extensions:` in settings or
 * `--extension` on the CLI ship their own `skills/`, `hooks/`, `tools/`,
 * `commands/`, `rules/`, `prompts/`, and `.mcp.json`. The `aery-plugins`
 * provider (`src/discovery/aery-plugins.ts`) is what wires those sub-trees
...
import { getCapability } from "@aryee337/aery/capability";
import { clearCache } from "@aryee337/aery/capability/fs";
import { hookCapability } from "@aryee337/aery/capability/hook";
import { mcpCapability } from "@aryee337/aery/capability/mcp";
import { promptCapability } from "@aryee337/aery/capability/prompt";
import { ruleCapability } from "@aryee337/aery/capability/rule";
import { skillCapability } from "@aryee337/aery/capability/skill";
import { slashCommandCapability } from "@aryee337/aery/capability/slash-command";
import { toolCapability } from "@aryee337/aery/capability/tool";
import type { LoadContext, Provider } from "@aryee337/aery/capability/types";
// Register all discovery providers as a side effect.
import "@aryee337/aery/discovery";
import { clearAeryExtensionCliRoots, injectAeryExtensionCliRoots } from "@aryee337/aery/discovery/aery-extension-roots";

const PROVIDER_ID = "aery-plugins";
...
function buildExtensionPackage(packageDir: string): void {
	writeFile(
		...
		JSON.stringify({ name: path.basename(packageDir), aery: { extensions: ["./src/main.ts"] } }),
	);
...
beforeEach(() => {
	clearCache();
	clearAeryExtensionCliRoots();
	tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "aery-plugins-"));
...
test("project settings.json#extensions surfaces every sub-directory", async () => {
	writeFile(path.join(project, ".aery", "settings.json"), JSON.stringify({ extensions: [ext] }));
...
test("user settings.json#extensions also feeds sub-discovery", async () => {
	writeFile(path.join(home, ".aery", "agent", "settings.json"), JSON.stringify({ extensions: [ext] }));
...
test("`--extension` CLI injection is wired through the same provider", async () => {
	...
	injectAeryExtensionCliRoots([ext], home, project);
...
test("file-extension entrypoints contribute zero sub-surface (the file has no siblings to scan)", async () => {
	...
	writeFile(path.join(project, ".aery", "settings.json"), JSON.stringify({ extensions: [standaloneFile] }));
...
test("relative paths in settings resolve against the project cwd", async () => {
	...
	writeFile(path.join(project, ".aery", "settings.json"), JSON.stringify({ extensions: [`./${relative}`] }));
...
test("installed plugins under `<plugins>/node_modules/` are surfaced (e.g. via `aery plugin link`/`install`)", async () => {
	...
	const pluginsDir = path.join(home, ".aery", "plugins");
	...
	writeFile(
		...
		JSON.stringify({ name: "aery-plugins", dependencies: { "my-installed-ext": "1.0.0" } }),
	);
	// Plugin's own package.json must carry an `aery`/`aery` manifest for the
	// loader to recognise it; the buildExtensionPackage fixture already wrote
	// one with `aery.extensions`, which is sufficient.
...
test("disabled installed plugins do not contribute sub-discovery", async () => {
	...
	writeFile(
		path.join(pluginsDir, "package.json"),
		JSON.stringify({ name: "aery-plugins", dependencies: { "my-disabled-ext": "1.0.0" } }),
	);
	writeFile(
		path.join(pluginsDir, "aery-plugins.lock.json"),
...
test.skip("linked plugins (only in lockfile, not in package.json#dependencies) are surfaced", async () => {
	...
	// still find the package — otherwise the documented `aery install
