...
import { clearCache, readFile } from "@aryee337/aery/capability/fs";
import type { Rule } from "@aryee337/aery/capability/rule";
import type { LoadContext } from "@aryee337/aery/capability/types";
import { getProjectPathCandidates } from "@aryee337/aery/discovery/agents";
import {
...
describe("agents provider project-level discovery", () => {
	...
	beforeEach(() => {
		clearCache();
		tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "aery-agents-monorepo-"));
	...
	describe("skills", () => {
		...
		test("project walk-up skips home directory (no repo root)", async () => {
			// Regression for https://github.com/eminent337/aery/issues/1116:
