...
import type { LoadContext, LoadResult } from "@aryee337/aery/capability/types";
import { scanSkillsFromDir } from "@aryee337/aery/discovery/helpers";
...
describe("monorepo skill discovery", () => {
	...
	beforeEach(() => {
		clearCache();
		tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "aery-monorepo-skills-"));
	...
	test("finds skills in ancestor .aery/skills/ directories", async () => {
		// Root has a skill
		writeSkill(path.join(repoRoot, ".aery", "skills"), "root-skill", "From repo root");
		// Sub-project has a skill
		writeSkill(path.join(subProject, ".aery", "skills"), "local-skill", "From sub-project");
		...
		while (true) {
			const result = await scanSkillsFromDir(ctx, {
				dir: path.join(current, ".aery", "skills"),
				providerId: "native",
				level: "project",
			});
			results.push(result);
	...
	test("closest skill wins when same name exists at multiple levels", async () => {
		...
		writeSkill(path.join(repoRoot, ".aery", "skills"), "shared-skill", "Root version");
		writeSkill(path.join(subProject, ".aery", "skills"), "shared-skill", "Local version");
	...
	test("finds skills across multiple ancestor levels", async () => {
		...
		writeSkill(path.join(repoRoot, ".aery", "skills"), "root-skill", "Root");
		writeSkill(path.join(packagesDir, ".aery", "skills"), "packages-skill", "Packages");
		writeSkill(path.join(subProject, ".aery", "skills"), "app-skill", "App");
	...
	test("walk-up stops at repo root and does not find skills above it", async () => {
		...
		writeSkill(path.join(tempDir, ".aery", "skills"), "above-repo-skill", "Above repo");
		// Skill AT the repo root (should be found)
		writeSkill(path.join(repoRoot, ".aery", "skills"), "root-skill", "At repo root");
