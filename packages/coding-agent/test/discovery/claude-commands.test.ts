...
import { clearCache as clearFsCache } from "@aryee337/aery/capability/fs";
import { type SlashCommand, slashCommandCapability } from "@aryee337/aery/capability/slash-command";
import { resetSettingsForTest } from "@aryee337/aery/config/settings";
import { loadCapability } from "@aryee337/aery/discovery";
...
describe("Claude Code slash command discovery", () => {
	...
	beforeEach(async () => {
		...
		root = await fs.mkdtemp(path.join(os.tmpdir(), "aery-claude-commands-"));
