 ...
 * from both `~/.aery/agent/RULES.md` (user) and the nearest `.aery/RULES.md`
...
import { type Rule, ruleCapability } from "@aryee337/aery/capability/rule";
...
beforeEach(() => {
	clearCache();
	tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "aery-rules-md-"));
...
test("user ~/.aery/agent/RULES.md becomes an alwaysApply rule", async () => {
	writeFile(
		path.join(home, ".aery", "agent", "RULES.md"),
...
test("project .aery/RULES.md becomes an alwaysApply rule", async () => {
	writeFile(path.join(project, ".aery", "RULES.md"), "# Project rule\nAlways say hi.\n");
...
test("project RULES.md is found walking up from a sub-package cwd", async () => {
	...
	writeFile(path.join(project, ".aery", "RULES.md"), "# Repo-wide sticky rule\n");
	...
	expect(projectRule).toBeDefined();
	expect(projectRule?.alwaysApply).toBe(true);
	expect(projectRule?.path).toBe(path.join(project, ".aery", "RULES.md"));
});

test("alwaysApply is forced even when frontmatter says false", async () => {
	writeFile(path.join(home, ".aery", "agent", "RULES.md"), "---\nalwaysApply: false\n---\nStick around anyway.\n");
