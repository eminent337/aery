...
import { type ContextFile, contextFileCapability } from "@aryee337/aery/capability/context-file";
import { resetSettingsForTest, Settings } from "@aryee337/aery/config/settings";
import { initializeWithSettings, loadCapability } from "@aryee337/aery/discovery";
...
describe("disabledExtensions runtime filtering", () => {
	...
	beforeEach(async () => {
		...
		tempHomeDir = await fs.mkdtemp(path.join(os.tmpdir(), "aery-disabled-ext-home-"));
		process.env.HOME = tempHomeDir;
		vi.spyOn(os, "homedir").mockReturnValue(tempHomeDir);
		tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "aery-disabled-ext-"));
		await fs.mkdir(path.join(tempDir, ".aery"), { recursive: true });
		await fs.writeFile(path.join(tempDir, ".aery", "AGENTS.md"), "# project instructions\n");
