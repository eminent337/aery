...
import { getSSHConfigPath, TempDir } from "@aryee337/aery-utils";
...
describe("AgentSession SSH tool refresh", () => {
	...
	it("adds the ssh tool after a first host is written over a cached missing config", async () => {
		const tempDir = TempDir.createSync("@aery-ssh-refresh-");
	...
	it("reloads ssh from the session's current cwd after move", async () => {
		const oldProject = TempDir.createSync("@aery-ssh-refresh-old-");
		const newProject = TempDir.createSync("@aery-ssh-refresh-new-");
