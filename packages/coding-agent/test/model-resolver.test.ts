...
import {
	...
	resolveModelScope,
} from "@aryee337/aery/config/model-resolver";
import { Settings } from "@aryee337/aery/config/settings";
import { Effort, type Model } from "@aryee337/aery-ai";
...
describe("resolveModelRoleValue", () => {
	test("resolves aery/<role>:<thinking> by expanding role alias before parsing thinking", () => {
		const settings = {
		...
		const result = resolveModelRoleValue("aery/smol:high", allModels, { settings });
	...
	test("resolves aery/default through configured default role alias", () => {
		const settings = {
			getModelRole: (role: string) => (role === "default" ? "openrouter/qwen/qwen3-coder:exacto" : undefined),
		} as NonNullable<Parameters<typeof resolveModelRoleValue>[2]>["settings"];

		const result = resolveModelRoleValue("aery/default", allModels, { settings });
...
describe("resolveAgentModelPatterns", () => {
	test("falls back to the active session model when aery/task is unset", () => {
		const settings = Settings.isolated({
			modelRoles: { default: "anthropic/claude-sonnet-4-5" },
		});

		const result = resolveAgentModelPatterns({
			agentModel: "aery/task",
			settings,
	...
	test("expands aery/designer to priority defaults", () => {
		const settings = Settings.isolated({
			modelRoles: {
...
describe("expandRoleAlias", () => {
	test("expands aery/vision to configured vision role", () => {
		const settings = Settings.isolated();
		settings.setModelRole("vision", "openai/gpt-4o");

		expect(expandRoleAlias("aery/vision", settings)).toBe("openai/gpt-4o");
	});

	test("keeps aery/vision alias when vision role is unset", () => {
		const settings = Settings.isolated();
		settings.setModelRole("default", "anthropic/claude-sonnet-4-5");

		expect(expandRoleAlias("aery/vision", settings)).toBe("aery/vision");
	});
});
