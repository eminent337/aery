...
async function createHarness(): Promise<AgentHarness> {
	const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "aery-acp-test-"));
...
describe("ACP agent", () => {
	...
	it("accepts only ACP underscore-prefixed extension methods", async () => {
		...
		const result = await harness.agent.extMethod("_aery/sessions/listAll", { limit: 2 });

		expect(Array.isArray(result.sessions)).toBe(true);
		expect(typeof result.total).toBe("number");
		await expect(harness.agent.extMethod("aery/sessions/listAll", { limit: 2 })).rejects.toThrow(
