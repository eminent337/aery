...
import { type Skill as CapabilitySkill, skillCapability } from "@aryee337/aery/capability/skill";
import { getCapability } from "@aryee337/aery/discovery";
import { loadSkills, loadSkillsFromDir, type Skill } from "@aryee337/aery/extensibility/skills";
...
describe("skills", () => {
	...
	describe("loadSkills with options", () => {
		...
		it("should keep user Claude skills when project .claude/skills is missing", async () => {
			const tempHomeDir = await fs.mkdtemp(path.join(os.tmpdir(), "aery-claude-home-"));
			const tempProjectDir = await fs.mkdtemp(path.join(os.tmpdir(), "aery-claude-project-"));
		...
		it("should skip skills disabled via frontmatter", async () => {
			const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "aery-disabled-skill-"));
...
`,
		...
		it("should expand ~ in customDirectories", async () => {
			const tempHomeSkillsDir = await fs.mkdtemp(path.join(os.homedir(), ".aery-skills-test-"));
