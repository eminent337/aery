...
import { substitutePluginRoot } from "@aryee337/aery/discovery/substitute-plugin-root";
...
const AERY_VAR = "$" + "{AERY_PLUGIN_ROOT}";
...
describe("substitutePluginRoot", () => {
	...
	it("replaces AERY_PLUGIN_ROOT in strings", () => {
		expect(substitutePluginRoot(`${AERY_VAR}/bin/server`, ROOT)).toBe("/plugins/my-plugin/bin/server");
	});

	it("replaces both variables in same string", () => {
		expect(substitutePluginRoot(`${CLAUDE_VAR}:${AERY_VAR}`, ROOT)).toBe("/plugins/my-plugin:/plugins/my-plugin");
	});
