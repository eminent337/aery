/**
 * Discovery integration tests for AERY plugin registry reading.
 *
 * NOTE: listClaudePluginRoots() lives in discovery/helpers.ts which imports
 * @aryee337/aery-engine (native Rust addon via glob). We cannot call it here.
 *
 * Instead these tests validate the structural contract that listClaudePluginRoots
 * depends on:
 *   1. AERY registry lives at path.join(home, ".aery", "plugins", "installed_plugins.json")
 *      (matches getConfigDirName() == ".aery")
...
import type { InstalledPluginEntry } from "@aryee337/aery/extensibility/plugins/marketplace";
import {
	addInstalledPlugin,
	buildPluginId,
...
// Kept here to avoid importing helpers.ts (which pulls in @aryee337/aery-engine).
...
// Matches getConfigDirName() — single source of truth is in @aryee337/aery-utils,
// but we know the value is ".aery" and hardcoding it here keeps tests free of
// native-addon transitive imports.
const AERY_CONFIG_DIR = ".aery";
...
let tmpHome: string;
/** ~/.aery/plugins/installed_plugins.json inside tmpHome */
let aeryRegistryPath: string;

beforeEach(() => {
	tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), "aery-discovery-test-"));
	aeryRegistryPath = path.join(tmpHome, AERY_CONFIG_DIR, "plugins", "installed_plugins.json");
	fs.mkdirSync(path.dirname(aeryRegistryPath), { recursive: true });
});
...
describe("AERY registry path contract", () => {
	it("AERY registry lives at home/.aery/plugins/installed_plugins.json", () => {
		// This is the path that listClaudePluginRoots reads.
		// Any change to this path must be reflected in helpers.ts.
		const expected = path.join(tmpHome, ".aery", "plugins", "installed_plugins.json");
		expect(aeryRegistryPath).toBe(expected);
	});

	it("AERY config dir name is .aery", () => {
		...
		expect(AERY_CONFIG_DIR).toBe(".aery");
	});
});

// ── Format compatibility ───────────────────────────────────────────────────────

describe("AERY registry format compatibility with Claude parser", () => {
	it("empty registry written by writeInstalledPluginsRegistry passes validator", async () => {
		await writeInstalledPluginsRegistry(aeryRegistryPath, { version: 2, plugins: {} });

		const content = fs.readFileSync(aeryRegistryPath, "utf8");
	...
	it("registry with installed plugin passes validator", async () => {
		...
		let reg = await readInstalledPluginsRegistry(aeryRegistryPath);
		reg = addInstalledPlugin(reg, pluginId, entry);
		await writeInstalledPluginsRegistry(aeryRegistryPath, reg);
...
describe("AERY registry round-trip", () => {
	it("reads back what was written — single plugin", async () => {
		...
		const readBack = await readInstalledPluginsRegistry(aeryRegistryPath);
...
//
// listClaudePluginRoots must replace Claude entries with AERY entries when the same
...
describe("AERY precedence contract (registry structure)", () => {
	it("same plugin ID in both registries — AERY entry has required fields for deduplication", () => {
		// The replacement logic: roots.filter(r => r.id !== pluginId) keyed by id.
		// AERY entries must have installPath so they can be added to roots[].
		const id = buildPluginId("shared-plugin", "common-mkt");
		const aeryEntry = makeEntry("/aery/cached/path");

		// AERY registry entry has installPath (required by listClaudePluginRoots)
		expect(aeryEntry.installPath).toBeTruthy();
		expect(typeof aeryEntry.installPath).toBe("string");
