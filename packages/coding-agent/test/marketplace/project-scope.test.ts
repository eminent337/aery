 ...
 *
 * Note: helpers.ts imports @aryee337/aery-engine (Rust addon via glob).
...
describe("resolveActiveProjectRegistryPath", () => {
	...
	it("walk-up finds nearest .aery/ directory", async () => {
		// Layout: tmpDir/.aery/   +   tmpDir/sub/nested/  (cwd)
		// Resolver must climb from cwd → sub → tmpDir and find .aery/ there.
		fs.mkdirSync(path.join(tmpDir, ".aery"), { recursive: true });
		const cwd = path.join(tmpDir, "sub", "nested");
		fs.mkdirSync(cwd, { recursive: true });

		const result = await resolveActiveProjectRegistryPath(cwd);

		expect(result).toBe(path.join(tmpDir, ".aery", "plugins", "installed_plugins.json"));
	});

	it("walk-up stops at the nearest .aery/ — does not skip to a more distant one", async () => {
		// Layout: tmpDir/.aery/   +   tmpDir/sub/.aery/   +   tmpDir/sub/nested/  (cwd)
		// Resolver must stop at tmpDir/sub/.aery/, not climb further to tmpDir/.aery/.
		fs.mkdirSync(path.join(tmpDir, ".aery"), { recursive: true });
		fs.mkdirSync(path.join(tmpDir, "sub", ".aery"), { recursive: true });
		const cwd = path.join(tmpDir, "sub", "nested");
		fs.mkdirSync(cwd, { recursive: true });

		const result = await resolveActiveProjectRegistryPath(cwd);

		expect(result).toBe(path.join(tmpDir, "sub", ".aery", "plugins", "installed_plugins.json"));
	});

	it("falls back to .git root when no .aery/ exists", async () => {
		// Layout: tmpDir/.git/   +   tmpDir/sub/  (cwd)
		// No .aery/ anywhere → second pass finds .git/ at tmpDir.
	...
	it("returns null when neither .aery/ nor .git/ found anywhere in the tree", async () => {
		// Start at the filesystem root — guaranteed to have no .aery/ or .git/ ancestors.
	...
	it("does not treat ~/.git as a project root (pass-2 home-dir guard)", async () => {
		...
		// resolveActiveProjectRegistryPath must NOT return ~/.aery/.../installed_plugins.json.
		...
		try {
			// Start from a tmpDir that has no .aery/ or .git/ of its own.
			const result = await resolveActiveProjectRegistryPath(tmpDir);
			// Must not resolve to the home-dir AERY registry.
			const homeAeryPath = path.join(homeDir, ".aery", "plugins", "installed_plugins.json");
			expect(result).not.toBe(homeAeryPath);
		} finally {
...
describe("listClaudePluginRoots — project shadows user", () => {
	...
	/** Path where listClaudePluginRoots reads the user AERY registry. */
	...
	beforeEach(() => {
		tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), "aery-shadow-home-"));
		tmpProject = fs.mkdtempSync(path.join(os.tmpdir(), "aery-shadow-proj-"));

		// Create .aery/ in project so resolveActiveProjectRegistryPath finds it.
		fs.mkdirSync(path.join(tmpProject, ".aery", "plugins"), { recursive: true });

		userRegPath = path.join(tmpHome, ".aery", "plugins", "installed_plugins.json");
		fs.mkdirSync(path.dirname(userRegPath), { recursive: true });

		projectRegPath = path.join(tmpProject, ".aery", "plugins", "installed_plugins.json");
	});
