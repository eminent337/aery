...
import { MarketplaceManager, readInstalledPluginsRegistry } from "@aryee337/aery/extensibility/plugins/marketplace";
...
function createTestContext(): TestContext {
	const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "aery-mgr-test-"));
...
describe("MarketplaceManager", () => {
	...
	it("installPlugin scope:project when no projectInstalledRegistryPath → throws", async () => {
		const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "aery-mgr-noproj-"));
		try {
