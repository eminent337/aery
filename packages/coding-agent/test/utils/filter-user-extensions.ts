...
import { getAgentDir, getConfigRootDir, getPluginsDir } from "@aryee337/aery-utils";
...
//    or `../`-relative entry pointing somewhere else under `~/.aery/`), plus
//    the legacy non-XDG `~/.aery/plugins` tree on hosts without XDG dirs.
// 2. `getAgentDir()` (`~/.aery/agent` or `$PI_CODING_AGENT_DIR`)
...
// 3. `getPluginsDir()` (XDG-aware: `$XDG_DATA_HOME/aery/plugins` or legacy)
//    Handles installed plugin extensions that live outside `~/.aery` when
//    XDG_DATA_HOME resolves the plugins dir somewhere else.
//
// We deliberately do NOT use `pathIsWithin` from aery-utils here: that helper
// resolves symlinks via `fs.realpathSync` on both sides, so a contributor who
// symlinks `~/.aery/agent/extensions/foo.ts` to a dotfiles repo would see the
