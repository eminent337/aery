...
import { getConfigRootDir, getProjectDir, setAgentDir, setProjectDir } from "@aryee337/aery-utils";
...
describe("issue #956: interactive /mcp test", () => {
	...
	beforeEach(async () => {
		projectDir = await fs.mkdtemp(path.join(os.tmpdir(), "aery-issue-956-project-"));
		agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "aery-issue-956-agent-"));
