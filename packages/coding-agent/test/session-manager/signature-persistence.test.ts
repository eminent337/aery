...
import { SessionManager, type SessionMessageEntry } from "@aryee337/aery/session/session-manager";
import type { AssistantMessage } from "@aryee337/aery-ai";
import { getBlobsDir, TempDir } from "@aryee337/aery-utils";
...
describe("SessionManager signature persistence", () => {
	it("clears oversized signatures instead of truncating them", async () => {
		using tempDir = TempDir.createSync("@aery-session-signature-persistence-");
	...
	it("externalizes provider image data URLs and restores preserved history payloads across reload", async () => {
		using tempDir = TempDir.createSync("@aery-session-provider-image-persistence-");
		const session = SessionManager.create(tempDir.path(), tempDir.path());
	...
	it("rehydrates assistant replay metadata in memory without rewriting the session file", async () => {
		using tempDir = TempDir.createSync("@aery-session-rehydrate-persistence-");
		const session = SessionManager.create(tempDir.path(), tempDir.path());
