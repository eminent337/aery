...
import { getConfigRootDir, getSessionsDir, Snowflake, setAgentDir } from "@aryee337/aery-utils";
...
describe("SessionManager temp cwd session dirs", () => {
	...
	beforeEach(() => {
		testAgentDir = fs.mkdtempSync(path.join(os.tmpdir(), "aery-session-dir-test-"));
	...
	it("stores symlink-equivalent home cwd sessions under home-relative directories", () => {
		...
		const realProjectDir = fs.mkdtempSync(path.join(projectsRoot, "aery-session-home-"));
		const nestedDir = path.join(realProjectDir, "nested");
		const aliasRoot = fs.mkdtempSync(path.join(os.tmpdir(), "aery-session-home-alias-"));
...
describe("SessionManager legacy session migration persistence", () => {
	...
	beforeEach(() => {
		tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "aery-session-manager-legacy-"));
	});
