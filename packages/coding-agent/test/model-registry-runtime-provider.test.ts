...
import { ModelRegistry, type ProviderConfigInput } from "@aryee337/aery/config/model-registry";
import { AuthStorage } from "@aryee337/aery/session/auth-storage";
import { type AssistantMessageEventStream, clearCustomApis, Effort, getCustomApi } from "@aryee337/aery-ai";
import { getOAuthProviders, unregisterOAuthProviders } from "@aryee337/aery-ai/utils/oauth";
import type { OAuthCredentials } from "@aryee337/aery-ai/utils/oauth/types";
...
describe("ModelRegistry runtime provider registration", () => {
	...
	beforeEach(async () => {
		tempDir = path.join(os.tmpdir(), `aery-test-model-registry-runtime-${Snowflake.next()}`);
