...
import { buildSystemPrompt, loadProjectContextFiles, loadSystemPromptFiles } from "@aryee337/aery/system-prompt";
...
describe("SYSTEM.md prompt assembly", () => {
	...
	beforeEach(() => {
		tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "aery-system-prompt-"));
		tempHomeDir = fs.mkdtempSync(path.join(os.tmpdir(), "aery-system-home-"));
	...
	it("renders SYSTEM.md exactly once when it is used as the custom base prompt", async () => {
		...
		const systemDir = path.join(projectDir, ".aery");
	...
	it("prefers project SYSTEM.md over user SYSTEM.md", async () => {
		const projectDir = path.join(tempDir, "project");
		fs.mkdirSync(path.join(projectDir, ".aery"), { recursive: true });
		fs.mkdirSync(path.join(tempHomeDir, ".aery", "agent"), { recursive: true });
		fs.writeFileSync(path.join(tempHomeDir, ".aery", "agent", "SYSTEM.md"), "User SYSTEM prompt");
		fs.writeFileSync(path.join(projectDir, ".aery", "SYSTEM.md"), "Project SYSTEM prompt");
