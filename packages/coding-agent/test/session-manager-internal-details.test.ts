...
import { type SkillPromptDetails, stripInternalDetailsFields } from "@aryee337/aery/session/messages";
import { type CustomMessageEntry, SessionManager } from "@aryee337/aery/session/session-manager";
	...
	it("F1: strips __pendingDisplayTag from persisted details while preserving all other SkillPromptDetails fields", () => {
		...
		const id = session.appendCustomMessageEntry<SkillPromptDetails>(
			...
			true,
			{
				...
				args: "bar",
				lineCount: 10,
				__pendingDisplayTag: "aery-cmd-1-0",
			},
			"user",
		);
