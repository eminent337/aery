...
import { type CompactionEntry, type FileEntry, parseSessionEntries, type SessionMessageEntry } from "@aryee337/aery";
import { RpcClient } from "@aryee337/aery/modes/rpc/rpc-client";
import type { BashExecutionMessage } from "@aryee337/aery/session/messages";
import { type AssistantMessage, Effort, type TextContent } from "@aryee337/aery-ai";
import type { AgentEvent, AgentMessage } from "@aryee337/aery-core";
...
describe.skipIf(!e2eApiKey("ANTHROPIC_API_KEY"))("RPC mode", () => {
	...
	beforeEach(() => {
		sessionDir = path.join(os.tmpdir(), `aery-rpc-test-${Snowflake.next()}`);
