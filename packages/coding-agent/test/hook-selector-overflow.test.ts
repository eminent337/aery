...
import { HookSelectorComponent } from "@aryee337/aery/modes/components/hook-selector";
import { getThemeByName, setThemeInstance, theme } from "@aryee337/aery/modes/theme/theme";
...
describe("HookSelectorComponent", () => {
	...
	it("wraps outlined option text without omitting the tail", () => {
		const options = [
			"Option A: Move to AERY-native only by migrating reusable shared AI instructions into .aery/AGENTS.md, .aery/rules, .aery/skills, and .aery/agents while deliberately not creating a root .github directory.",
			"Option B: Keep dual support by migrating canonical instructions into .aery while also maintaining a root .github/copilot-instructions.md compatibility bridge for editors that do not understand AERY resources yet.",
		];
		const component = new HookSelectorComponent(
		...
		expect(normalizedPlain).toContain("do not understand AERY resources yet");
		for (const line of lines) {
			expect(visibleWidth(Bun.stripANSI(line))).toBeLessThanOrEqual(width);
		}
	});
		...
		const options = [
			{
				...
				description:
					"Authenticate via the provider keys and OAuth state already configured under ~/.aery without opening a new browser-based setup flow.",
			},
			{
				label: "Set up Aery in terminal",
				description:
