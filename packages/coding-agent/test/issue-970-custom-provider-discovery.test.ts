...
import type { ModelRegistry, ProviderDiscoveryState } from "@aryee337/aery/config/model-registry";
import { ModelRegistry as ModelRegistryImpl } from "@aryee337/aery/config/model-registry";
import { Settings } from "@aryee337/aery/config/settings";
import { ModelSelectorComponent } from "@aryee337/aery/modes/components/model-selector";
...
import type { TUI } from "@aryee337/aery-tui";
...
describe("issue #970 custom provider discovery", () => {
	...
	beforeEach(async () => {
		tempDir = path.join(os.tmpdir(), `aery-test-issue-970-${Snowflake.next()}`);
