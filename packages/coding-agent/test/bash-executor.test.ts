...
import { executeBash } from "@aryee337/aery/exec/bash-executor";
import { DEFAULT_MAX_BYTES } from "@aryee337/aery/session/streaming-output";
import * as shellSnapshot from "@aryee337/aery/utils/shell-snapshot";
import type { Shell } from "@aryee337/aery-engine";
import * as aeryNatives from "@aryee337/aery-engine";
...
function makeTempDir(): string {
	return fs.mkdtempSync(path.join(os.tmpdir(), "aery-bash-exec-"));
}
...
describe("executeBash", () => {
	...
	it("returns promptly and quarantines the session key when native abort cleanup stalls", async () => {
		...
		const originalRun = aeryNatives.Shell.prototype.run;
		let runCalls = 0;
		const dispatched = Promise.withResolvers<void>();
		vi.spyOn(aeryNatives.Shell.prototype, "run").mockImplementation(function (this: Shell, options, onChunk) {
			runCalls++;
			if (runCalls === 1) {
		...
		const abortSpy = vi.spyOn(aeryNatives.Shell.prototype, "abort").mockResolvedValue();
	...
	it("restores persistent sessions after native abort cleanup settles", async () => {
		...
		const dispatched = Promise.withResolvers<void>();
		vi.spyOn(aeryNatives.Shell.prototype, "run").mockImplementation((_options, onChunk) => {
			onChunk?.(null, "started\n");
			dispatched.resolve();
			return nativeResult.promise;
		});
		vi.spyOn(aeryNatives.Shell.prototype, "abort").mockResolvedValue();
