 ...
 */
import type { HookAPI, HookContext } from "@aryee337/aery";
...
async function checkDirtyRepo(
	...
	const { stdout, code } = await aery.exec("git", ["status", "--porcelain"]);
...
export default function (aery: HookAPI) {
	aery.on("session_before_switch", async (event, ctx) => {
		...
		return checkDirtyRepo(aery, ctx, action);
	});

	aery.on("session_before_branch", async (_event, ctx) => {
		return checkDirtyRepo(aery, ctx, "branch");
	});
}
