...
Example hooks for aery-coding-agent.

## Usage

```bash
# Load a hook with --hook flag
aery --hook examples/hooks/permission-gate.ts

# Or copy to hooks directory for auto-discovery
cp permission-gate.ts ~/.aery/agent/hooks/
```
...
```typescript
import type { HookAPI } from "@aryee337/aery/hooks";
