 ...
 */
import type { HookAPI, SessionBeforeSwitchEvent, SessionMessageEntry } from "@aryee337/aery";

export default function (aery: HookAPI) {
	aery.on("session_before_switch", async (event: SessionBeforeSwitchEvent, ctx) => {
		if (!ctx.hasUI) return;
