...
export default function (aery: HookAPI) {
	aery.on("session_shutdown", async (_event, ctx) => {
		...
		const { stdout: status, code } = await aery.exec("git", ["status", "--porcelain"]);
		...
		// Stage and commit
		await aery.exec("git", ["add", "-A"]);
		const { code: commitCode } = await aery.exec("git", ["commit", "-m", commitMessage]);
