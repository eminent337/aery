 ...
 * - ~/.aery/agent/extensions/ (legacy: ~/.aery/agent/extensions/)
 * - <cwd>/.aery/extensions/ (legacy: <cwd>/.aery/extensions/)
 ...
 */
import { createAgentSession, SessionManager } from "@aryee337/aery";

// Extensions are loaded from disk, not passed inline to createAgentSession.
// Use the discovery mechanism:
//   1. Place extension files in ~/.aery/agent/extensions/ or .aery/extensions/
//   2. Add paths to settings.json: { "extensions": ["./my-extension.ts"] }
//   3. Use --extension flag: aery --extension ./my-extension.ts
...
export default function (aery: ExtensionAPI) {
	...
	// Register a command
	aery.registerCommand("mycommand", {
