 ...
 * `BlobStore` keep writing to `~/.aery/agent/...`. Reach for an object store
 * (S3, R2, GCS) if you need those off-host too.
 */

import { createAgentSession, RedisSessionStorage, SessionManager } from "@aryee337/aery";
...
const storage = await RedisSessionStorage.create({
	client: redis,
	prefix: "aery:sessions:", // optional, this is the default
});
