...
import { Key } from "@aryee337/aery-tui";
...
export default function planModeExtension(aery: ExtensionAPI) {
	...
	async function togglePlanMode(ctx: ExtensionContext) {
		...
		if (planModeEnabled) {
			await aery.setActiveTools(PLAN_MODE_TOOLS);
			ctx.ui.notify(`Plan mode enabled. Tools: ${PLAN_MODE_TOOLS.join(", ")}`);
		} else {
			await aery.setActiveTools(NORMAL_MODE_TOOLS);
	...
	aery.registerShortcut(Key.shift("p"), {
