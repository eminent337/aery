...
aery update                   # update pi and all non-pinned packages
aery update --extensions      # update all non-pinned packages only
aery update --self            # update pi only
aery update --self --force    # reinstall pi even if current
aery update npm:@foo/bar      # update one package
aery update --extension npm:@foo/bar
```
...
- Versioned specs are pinned and skipped by package updates (`aery update`, `aery update --extensions`).
...
- Refs pin the package and skip package updates (`aery update`, `aery update --extensions`).
