...
- Migrated the web UI's TypeBox-based tool definitions and runtime dependency from `@sinclair/typebox` 0.34.x to `typebox` 1.x. Install and import from `typebox` instead of `@sinclair/typebox` when embedding or extending `@eminent337/pi-web-ui` with shared TypeBox schemas ([#3112](https://github.com/eminent337/aery/issues/3112))

### Fixed

- Render SVG artifact previews through a blob-backed image instead of injecting untrusted SVG markup into the page DOM ([#3552](https://github.com/eminent337/aery/issues/3552))
...
- Exported `CustomProviderDialog` from `@eminent337/aery-web-ui` ([#2267](https://github.com/eminent337/aery/issues/2267))
...
- Build `@eminent337/aery-web-ui` with `tsc` instead of `tsgo` so Lit decorator-based state updates rerender correctly.
...
- **Agent class moved to `@eminent337/aery-core`**: The `Agent` class, `AgentState`, and related types are no longer exported from this package. Import them from `@eminent337/aery-core` instead.
...
- **`AppMessage` renamed to `AgentMessage`**: Now imported from `@eminent337/aery-core`. Custom message types use declaration merging on `CustomAgentMessages` interface.
...
- **`CustomMessages` interface removed**: Use declaration merging on `CustomAgentMessages` from `@eminent337/aery-core` instead.

- **`agent.appendMessage()` removed**: Use `agent.queueMessage()` instead.

- **Agent event types changed**: `AgentInterface` now handles new event types from `@eminent337/aery-core`: `message_start`, `message_end`, `message_update`, `turn_start`, `turn_end`, `agent_start`, `agent_end`.
...
- `Agent` class (moved to `@eminent337/aery-core`)
...
**Before (0.30.x):**
```typescript
import { Agent, ProviderTransport, type AppMessage } from '@eminent337/aery-web-ui';
...
import { defaultConvertToLlm } from '@eminent337/aery-web-ui';
...
declare module "@eminent337/aery-web-ui" {
