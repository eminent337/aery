...
import { getFastembedCacheDir } from "@aryee337/aery-utils";
import "./setup";
	...
	it("calls an OpenAI-compatible custom embeddings endpoint without requiring an API key", async () => {
		...
		const server = Bun.serve({
			...
			fetch: async request => {
				...
				expect(request.headers.get("user-agent")).toBe(`Aery/${packageJson.version}`);
				expect(request.headers.get("http-referer")).toBe("https://aery.dev/");
				expect(request.headers.get("x-openrouter-title")).toBe("Aery");
