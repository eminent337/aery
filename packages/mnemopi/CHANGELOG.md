...
- Fixed compiled `aery` binaries losing local Mnemopi embeddings by keeping `fastembed` and `onnxruntime-node` reachable to Bun's static compiler while preserving lazy runtime loading.
...
- Published `@aryee337/aery-mnemopi` to npm: the local SQLite memory engine is now built, checked, tested, and released through the monorepo CI pipeline alongside the other workspace packages.
- Exported the diagnostic inspector as the `@aryee337/aery-mnemopi/diagnose` subpath for coding-agent memory maintenance commands.
