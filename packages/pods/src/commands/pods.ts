...
export const setupPod = async (
	...
	const vllmApiKey = process.env.AERY_API_KEY;
	...
	if (!vllmApiKey) {
		console.error(chalk.red("ERROR: AERY_API_KEY environment variable is required"));
		console.error("Set an API key: export AERY_API_KEY=your_api_key_here");
		process.exit(1);
	}
	...
	console.log(chalk.cyan(`  aery start <model> --name <name>`));
};
