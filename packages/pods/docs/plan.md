...
### Pods
```bash
aery pods setup dc1 "ssh root@1.2.3.4" --mount "mount -t nfs..."  # Setup pod (requires HF_TOKEN, AERY_API_KEY env vars)
...
- `AERY_API_KEY`: API key for securing vLLM endpoints
...
4. Builds vLLM command with appropriate args and AERY_API_KEY
