...
beforeAll(() => {
	tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "aery-logger-error-"));
	restoredEnv = { ...process.env };
	// `getLogsDir()` honors AERY_AGENT_DIR / HOME for its base; pin to our tmp.
	process.env.AERY_AGENT_DIR = tempDir;
...
async function waitForLogEntry(targetMessage: string): Promise<Record<string, unknown>> {
	for (let i = 0; i < 40; i++) {
		const files = fs.readdirSync(tempDir).filter(f => f.startsWith("aery.") && f.endsWith(".log"));
