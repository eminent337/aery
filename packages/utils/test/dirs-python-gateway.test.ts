...
describe("python gateway directory", () => {
	...
	beforeEach(async () => {
		...
		tempRoot = path.join(os.tmpdir(), "aery-utils-python-gateway", Snowflake.next());
	...
	it("uses XDG state for the default agent profile", async () => {
		...
		process.env.PI_CONFIG_DIR = `.aery-test-${Snowflake.next()}`;
		process.env.XDG_STATE_HOME = path.join(tempRoot, "state");
		await fs.mkdir(path.join(process.env.XDG_STATE_HOME, "aery"), { recursive: true });

		const defaultAgentDir = path.join(os.homedir(), getConfigDirName(), "agent");
		setAgentDir(defaultAgentDir);

		expect(getPythonGatewayDir()).toBe(path.join(process.env.XDG_STATE_HOME, "aery", "python-gateway"));
	});
