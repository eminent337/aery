...
function writeTempEnv(content: string): string {
	const dir = fs.mkdtempSync(path.join(os.tmpdir(), "aery-utils-env-"));
	tempDirs.push(dir);
...
describe("parseEnvFile", () => {
	...
	it("mirrors valid AERY_ variables to PI_ variables", () => {
		const filePath = writeTempEnv("AERY_FEATURE=enabled\nAERY_BAD=before\0after\n");

		expect(parseEnvFile(filePath)).toEqual({
			AERY_FEATURE: "enabled",
			PI_FEATURE: "enabled",
		});
	});
});
