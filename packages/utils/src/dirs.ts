/**
 * Centralized path helpers for aery config directories.
 *
 * Uses PI_CONFIG_DIR (default ".aery") for the config root and
 * PI_CODING_AGENT_DIR to override the agent directory.
 *
 * On Linux, if XDG_DATA_HOME / XDG_STATE_HOME / XDG_CACHE_HOME environment
 * variables are set, paths are redirected to XDG-compliant locations under
 * $XDG_*_HOME/aery/. This requires running `aery config migrate` first to
 * move data to the new locations. No filesystem existence checks are performed
 * — if the env var is set, aery trusts that the migration has been done.
 */
...
/** App name (e.g. "aery") */
export const APP_NAME: string = "aery";

/** Config directory name (e.g. ".aery") */
export const CONFIG_DIR_NAME: string = ".aery";
...
/** Get the config directory name relative to home (e.g. ".aery" or PI_CONFIG_DIR override). */
export function getConfigDirName(): string {
	return process.env.PI_CONFIG_DIR || CONFIG_DIR_NAME;
}

/** Get the config agent directory name relative to home (e.g. ".aery/agent" or PI_CONFIG_DIR + "/agent"). */
...
/**
 * Resolves and caches all aery directory paths. On Linux, when XDG environment
 * variables are set, paths are redirected under $XDG_*_HOME/aery/. A new
...
class DirResolver {
	...
	// With XDG on Linux, they point to $XDG_*_HOME/aery/.
	...
	constructor(agentDirOverride?: string) {
		...
		// XDG flattens the agent/ prefix: ~/.aery/agent/sessions → $XDG_DATA_HOME/aery/sessions
		this.#agentDirs = {
...
/** Get the config root directory (~/.aery). */
...
/** Get the agent config directory (~/.aery/agent). */
export function getAgentDir(): string {
	return dirs.agentDir;
}

/** Get the project-local config directory (.aery). */
export function getProjectAgentDir(cwd: string = getProjectDir()): string {
	return path.join(cwd, CONFIG_DIR_NAME);
}

// =============================================================================
// Config-root subdirectories (~/.aery/*)
// =============================================================================

/** Get the reports directory (~/.aery/reports). */
export function getReportsDir(): string {
	return dirs.rootSubdir("reports", "state");
}

/** Get the logs directory (~/.aery/logs). */
export function getLogsDir(): string {
	return dirs.rootSubdir("logs", "state");
}

/** Get the path to a dated log file (~/.aery/logs/aery.YYYY-MM-DD.log). */
export function getLogPath(date = new Date()): string {
	return path.join(getLogsDir(), `${APP_NAME}.${date.toISOString().slice(0, 10)}.log`);
}

/**
 * Get the plugins directory (~/.aery/plugins or its XDG equivalent).
 *
 ...
 * (e.g. PR-checkout and task-isolation worktree dirs under `~/.aery/wt/`).
...
/** Get the path to a single worktree directory (~/.aery/wt/<segment>). */
export function getWorktreeDir(segment: string): string {
	return path.join(getWorktreesDir(), segment);
}

/** Get the GPU cache path (~/.aery/gpu_cache.json). */
export function getGpuCachePath(): string {
	return dirs.rootSubdir("gpu_cache.json", "cache");
}

/**
 * Get the GitHub view cache database path (~/.aery/cache/github-cache.db).
 * Honors the `AERY_GITHUB_CACHE_DB` env var when set so tests can isolate the
 * cache file without touching the rest of the config root.
 */
export function getGithubCacheDbPath(): string {
	const override = process.env.AERY_GITHUB_CACHE_DB;
	if (override) return override;
	return dirs.rootSubdir(path.join("cache", "github-cache.db"), "cache");
}

/** Get the local FastEmbed model cache directory (~/.aery/cache/fastembed). */
export function getFastembedCacheDir(): string {
	return dirs.rootSubdir(path.join("cache", "fastembed"), "cache");
}

/** Get the natives directory (~/.aery/natives). */
export function getNativesDir(): string {
	return dirs.rootSubdir("natives", "cache");
}

/** Get the stats database path (~/.aery/stats.db). */
export function getStatsDbPath(): string {
	return dirs.rootSubdir("stats.db", "data");
}

/** Get the autoresearch state directory (~/.aery/autoresearch). */
export function getAutoresearchDir(): string {
	return dirs.rootSubdir("autoresearch", "state");
}

/** Get the per-project autoresearch state directory (~/.aery/autoresearch/<encoded-project>). */
export function getAutoresearchProjectDir(encodedProject: string): string {
	return path.join(getAutoresearchDir(), encodedProject);
}

/** Get the per-project autoresearch SQLite database path (~/.aery/autoresearch/<encoded-project>.db). */
export function getAutoresearchDbPath(encodedProject: string): string {
	return path.join(getAutoresearchDir(), `${encodedProject}.db`);
}

/** Get the per-run artifact directory (~/.aery/autoresearch/<encoded-project>/runs/<runId>). */
...
// =============================================================================
// Agent subdirectories (~/.aery/agent/*)
// =============================================================================
...
/** Get the tiny title model cache directory (~/.aery/agent/cache/tiny-models). */
...
/**
 * Persistent per-install UUID stored at `~/.aery/install-id`.
 *
 ...
 * `~/.aery/agent/` does not regenerate it. Server-side dedup for grievance
