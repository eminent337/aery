 ...
 *     → anthropic-messages parser → aery Context (cacheRetention derived)
 *     → aery-ai openai-codex-responses provider
 ...
 * directly — caching only works if aery-ai.s codex transport reaches Codex
 ...
 * (override via `AERY_E2E_GATEWAY_URL`) AND the bearer token file exists at
 * `~/.aery/auth-gateway.token`.
 *
...
const MODEL = Bun.env.AERY_E2E_CODEX_MODEL ?? "gpt-5.3-codex";
...
the aery auth-gateway's cross-protocol prompt-caching pipeline. The request
...
describe.skipIf(!gateway.ok)("auth-gateway: anthropic-messages → openai-codex caching e2e", () => {
	...
	it("caches the system prefix across a cross-protocol translate (messages→codex)", async () => {
		...
		//   - the anthropic-messages encoder forgot to map aery-ai.s
