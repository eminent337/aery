...
} from "@aryee337/aery-ai/providers/anthropic";
import { getEnvApiKey } from "@aryee337/aery-ai/stream";
import type { Context, Model, TJsonSchema, TokenTaskBudget, Tool } from "@aryee337/aery-ai/types";
...
describe("Anthropic request fingerprint alignment", () => {
	...
	it("loads Foundry mTLS and CA material from file paths", async () => {
		const tmpDir = path.join(os.tmpdir(), `aery-ai-foundry-${Date.now()}-${Math.random().toString(16).slice(2)}`);
