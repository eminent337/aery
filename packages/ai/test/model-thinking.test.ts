...
} from "@aryee337/aery-ai/model-thinking";
import type { Api, Model, Provider } from "@aryee337/aery-ai/types";
...
describe("model thinking metadata", () => {
	...
	it("encodes anthropic transport mode in metadata", () => {
		...
		// Opus 4.6 has no real xhigh level — aery-ai aliases XHigh to Anthropic's "max".
		expect(mapEffortToAnthropicAdaptiveEffort(opus46, Effort.XHigh)).toBe("max");
		// Opus 4.7+ on the Messages API exposes the full five-tier scale, so aery-ai
