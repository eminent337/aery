 ...
 *     → openai-responses parser → aery Context
 *     → aery-ai anthropic provider (auto cache_control via cacheRetention)
 ...
 * with the gateway live (`aery auth-gateway serve` or pm2).
 */
...
const MODEL = Bun.env.AERY_E2E_ANTHROPIC_MODEL ?? "claude-sonnet-4-5";
