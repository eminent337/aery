...
import { enrichModelThinking } from "@aryee337/aery-ai/model-thinking";
import {
...
import type { Context, Model, ProviderSessionState } from "@aryee337/aery-ai/types";
import { getAgentDir, setAgentDir, TempDir } from "@aryee337/aery-utils";
...
describe("openai-codex streaming", () => {
	it("normalizes Codex response endpoint base URLs", async () => {
		const tempDir = TempDir.createSync("@aery-codex-stream-");
	...
	it("streams SSE responses into AssistantMessageEventStream", async () => {
		...
		const fetchMock = vi.fn(async (input: string | URL, init?: RequestInit) => {
			...
			if (url === "https://chatgpt.com/backend-api/codex/responses") {
				...
				expect(headers?.get("originator")).toBe("aery");
	...
	it("uses low Codex text verbosity by default while preserving explicit overrides", async () => {
		const tempDir = TempDir.createSync("@aery-codex-verbosity-");
		setAgentDir(tempDir.path());
		const payload = Buffer.from(
			JSON.stringify({ "https://api.openai.com/auth": { chatgpt_account_id: "acc_test" } }),
			"utf8",
