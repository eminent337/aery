...
import { streamAeryNative } from "../src/providers/aery-native-client";
...
describe("streamAeryNative request shape", () => {
	it("POSTs `{modelId, context, options, stream:true}` to `<baseUrl>/v1/aery/stream`", async () => {
		...
		const stream = streamAeryNative(fakeModel(), baseContext, {
			apiKey: "gw-bearer",
			fetch: fetchImpl,
			temperature: 0.7,
		});
		...
		expect(captured.url).toBe("http://llm-gateway.internal:4000/v1/aery/stream");
	...
	it("normalizes trailing slashes on `baseUrl` so the endpoint never double-slashes", async () => {
		...
		await streamAeryNative(fakeModel({ baseUrl: "http://llm-gateway.internal:4000///" }), baseContext, {
			apiKey: "k",
			fetch: fetchImpl,
		}).result();
		expect(captured.url).toBe("http://llm-gateway.internal:4000/v1/aery/stream");
	...
	it("forwards `model.headers` and lets a caller-supplied Authorization win", async () => {
		...
		await streamAeryNative(
			fakeModel({ headers: { "x-aery-slot": "robomp-1", Authorization: "Bearer model-wins" } }),
			baseContext,
			{ apiKey: "options-loses", fetch: fetchImpl },
		).result();

		const headers = captured.init?.headers as Record<string, string>;
		expect(headers["x-aery-slot"]).toBe("robomp-1");
	...
	it("throws synchronously when `baseUrl` is missing", async () => {
		...
		const stream = streamAeryNative(broken, baseContext, { apiKey: "k" });
		await expect(stream.result()).rejects.toThrow(/baseUrl/);
	});
});

describe("streamAeryNative event flow", () => {
	it("pushes parsed events verbatim and resolves `.result()` on terminal `done`", async () => {
		...
		const stream = streamAeryNative(fakeModel(), baseContext, { apiKey: "k", fetch: fetchImpl });
	...
	it("forwards the caller's AbortSignal to the underlying fetch", async () => {
		...
		const controller = new AbortController();
		await streamAeryNative(fakeModel(), baseContext, {
			apiKey: "k",
			fetch: fetchImpl,
			signal: controller.signal,
		}).result();
