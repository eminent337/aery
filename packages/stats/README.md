...
- **Session log parsing**: Reads JSONL session logs from `~/.aery/agent/sessions/`
...
```typescript
import { getDashboardStats, syncAllSessions } from "@aryee337/aery-stats";
...
- **Session logs**: `~/.aery/agent/sessions/` (JSONL files)
- **Stats database**: `~/.aery/stats.db` (SQLite)
