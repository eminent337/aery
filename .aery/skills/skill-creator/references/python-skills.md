...
A Python-backed skill is a regular markdown skill that also ships a Python package. Aery installs the package editable into the kernel venv (`~/.prime/agent/kernel-venv` by default, `PRIME_AGENT_KERNEL_VENV` to override) and exposes it in the persistent IPython kernel, so the agent can call it directly instead of shelling out.
...
- `rlm` and `tyro` are already present in the kernel venv. Do **not** declare `prime-agent-runtime` as a dependency: it is bundled with Aery, not published on PyPI, so declaring it breaks installs outside the kernel venv. The CLI entry point only works where the runtime is installed, i.e. inside the kernel venv.
...
- If the user sets `PRIME_AGENT_KERNEL_PYTHON`, Aery installs nothing — skills whose imports are missing there are disabled with a warning.
