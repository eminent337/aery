...
```bash
gh_PR=<NUMBER>
# pr_checkout creates ~/.aery/wt/<encoded-repo>/pr-<N>/ and configures push remote
```
...
```bash
MAIN="<absolute path to main worktree, e.g. ~/Projects/aery>"
WT="$(pwd)"
...
# binaries — the rest of packages/aery-engine/native/ is tracked by git, so
...
Worktree: ~/.aery/wt/.../pr-<N>   (or: not checked out)
...
- **MUST** symlink `target`, `node_modules`, and the native `*.node` binaries before any build/test runs in the worktree. **MUST NOT** symlink the whole `packages/aery-engine/native/` directory that would shadow tracked PR changes.
