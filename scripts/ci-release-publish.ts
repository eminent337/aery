 ...
 *
 * The default mode publishes public JS packages and the `@aryee337/aery-engine`
...
} from "../packages/aery-engine/scripts/gen-npm-packages.ts";
...
export const packages: PublishPackage[] = [
	...
	{ dir: "packages/aery-engine", kind: "native" },
	...
	{ dir: "packages/aery-sdk", kind: "typescript" },
	{ dir: "packages/aery-extensions", kind: "typescript" },
...
function buildNativeOptionalDependencies(version: string): JsonObject {
	...
	for (const target of LEAF_TARGETS) {
		optionalDependencies[`@aryee337/aery-engine-${target.tag}`] = version;
	}
...
async function packAndPublish(dir: string, name: string): Promise<void> {
	...
	const packDir = await fs.mkdtemp(path.join(os.tmpdir(), "aery-pack-"));
	try {
