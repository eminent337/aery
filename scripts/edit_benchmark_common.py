...
sys.path.insert(0, str(REPO_ROOT / "python/aery-rpc/src"))

from aery_rpc import MessageEndEvent, MessageStartEvent, MessageUpdateEvent, RpcClient, ToolExecutionStartEvent  # noqa: E402
...
def resolve_repo_aery_bin() -> str | None:
...
def resolve_aery_bin(raw: str | None) -> str:
    if raw:
        return raw
    repo_bin = resolve_repo_aery_bin()
    if repo_bin:
        return repo_bin
    found = shutil.which("aery")
    if not found:
        raise SystemExit("Could not find `aery` on PATH and could not resolve the repo CLI. Set --aery-bin or AERY_BIN.")
    return found
...
async def run_all(spec: BenchmarkSpec, args: argparse.Namespace) -> dict[str, dict[str, Any]]:
    aery_bin = resolve_aery_bin(args.aery_bin)
...
def parse_args(description: str) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--aery-bin",
        default=os.environ.get("AERY_BIN"),
        help="Executable to launch. Defaults to the repo checkout CLI, then falls back to `aery` on PATH.",
    )
    parser.add_argument(
