...
(`~/.aery/agent/sessions/`). SQLite-backed; data is synced once into the same
`~/.aery/stats.db` that `packages/stats` uses, then queried by short Python
scripts.
...
scripts/session-stats/
  sync.py          # walks ~/.aery/agent/sessions/ and populates ss_* tables
  analyze.py       # tools | edits | followups subcommands over the synced db
  audit.ts         # LLM-assisted token-usage audit (no sync needed)
  audit-praeryt.md  # system praeryt for the audit classifier
```

## One-time prep

```sh
aeryp install tiktoken
```
...
|`ss_edit_sections`|per `¶PATH` section in an edit; precaeryuted `longest_repeat_*`, `dup_anchors`. Legacy `§PATH` sections from pre-2026-05 sessions still parse.|
...
bun run stats:audit -- --since 3d --folder Projects-aery
...
reads of the same file, the largest single tool results, caeryactions, and
edit-failure churn.

The LLM phase (default `anthroaeryc/claude-sonnet-4-6` via `@aery/aery-ai`,
credentials resolved through aery's auth storage — stored key, OAuth, or env
var) classifies the costliest sessions: multi-toaeryc sessions that should have
...
Verdicts are cached in `~/.aery/stats-audit-cache.json` (keyed by session id +
digest hash + model + praeryt hash, so any change to the transcript, digest
format, or `audit-praeryt.md` invalidates the entry automatically). Re-runs
...
recaeryute on every run (token counts, hashline parse output, success flags),
