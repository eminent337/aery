...
const agentDirEnvName = "AERY_CODING_AGENT_DIR";
const startupBenchmarkEnvName = "AERY_STARTUP_BENCHMARK";
...
Options:
  ...
  --agent-dir <dir>      Use a specific AERY_CODING_AGENT_DIR for the benchmark run
  --isolated-agent-dir   Use a fresh temporary agent dir instead of the normal one
  --no-offline           Do not force AERY_OFFLINE=1 / AERY_SKIP_VERSION_CHECK=1
...
async function runTuiBenchmarkRun({ runtime, runIndex, measuredIndex, options, profileDir }) {
	...
	const tempRoot = options.isolatedAgentDir ? mkdtempSync(join(tmpdir(), "aery-startup-benchmark-")) : undefined;
