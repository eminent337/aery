...
const targets: BinaryTarget[] = [
	{
		...
		outfile: "packages/coding-agent/binaries/aery-darwin-arm64",
	},
	{
		id: "darwin-x64",
		platform: "darwin",
		...
		outfile: "packages/coding-agent/binaries/aery-darwin-x64",
	},
	{
		id: "linux-x64",
		platform: "linux",
		...
		outfile: "packages/coding-agent/binaries/aery-linux-x64",
	},
	{
		id: "linux-arm64",
		platform: "linux",
		...
		outfile: "packages/coding-agent/binaries/aery-linux-arm64",
	},
	{
		id: "win32-x64",
		platform: "win32",
		...
		outfile: "packages/coding-agent/binaries/aery-windows-x64.exe",
	},
];
...
async function embedNative(target: BinaryTarget): Promise<void> {
	if (isDryRun) {
		console.log(`DRY RUN bun --cwd=packages/aery-engine run embed:native [${target.platform}/${target.arch}]`);
		return;
	}

	await runCommand(["bun", "--cwd=packages/aery-engine", "run", "embed:native"], repoRoot, {
		...Bun.env,
...
async function resetArtifacts(): Promise<void> {
	if (isDryRun) {
		console.log("DRY RUN bun --cwd=packages/aery-engine run embed:native --reset");
		console.log("DRY RUN bun --cwd=packages/stats scripts/generate-client-bundle.ts --reset");
		return;
	}
	await runCommand(["bun", "--cwd=packages/aery-engine", "run", "embed:native", "--reset"], repoRoot);
