...
import { $ } from "bun";
import { LEAF_TARGETS } from "../packages/aery-engine/scripts/gen-npm-packages.ts";
import { packages } from "./ci-release-publish.ts";

const repoRoot = path.join(import.meta.dir, "..");
const MIN_NPM = "11.16.0";
const DEFAULT_WORKFLOW = "ci.yml";
const FALLBACK_REPO = "eminent337/aery";
	...
	for (const pkg of packages) {
		...
		if (pkg.kind === "native") {
			for (const target of LEAF_TARGETS) {
				const leaf = `@aryee337/aery-engine-${target.tag}`;
				if (!seen.has(leaf)) {
					seen.add(leaf);
					names.push(leaf);
				}
...
function nativeLeafName(tag: string): string {
	return `@aryee337/aery-engine-${tag}`;
}
...
function placeholderManifest(name: string, target: NativeLeafTarget, repo: string): PlaceholderManifest {
	return {
		...
		description: `Placeholder for the ${target.tag} native addon of @aryee337/aery-engine. The real binary is published during release.`,
		license: "MIT",
		os: [target.os],
		cpu: [target.cpu],
		repository: {
			type: "git",
			url: repoGitUrl(repo),
			directory: "packages/aery-engine",
		},
		engines: {
			bun: ">=1.3.14",
		},
...
function placeholderReadme(name: string, target: NativeLeafTarget): string {
	return [
		`# ${name}`,
		"",
		`Placeholder package reserving the npm name for the \`${target.tag}\` native addon of \`@aryee337/aery-engine\`.`,
		"",
...
async function publishNativeLeafPlaceholder(name: string, target: NativeLeafTarget, repo: string): Promise<boolean> {
	const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "aery-native-placeholder-"));
	try {
