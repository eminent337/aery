#!/usr/bin/env bun
/**
 * Release script for aery-monorepo
 *
 * Usage:
...
async function cmdRelease(version: string, skipWatch: boolean = false): Promise<void> {
	...
	await $`sed -i -E 's/"version": "[^"]+"/"version": "${version}"/' ${publicPkgPaths}`;
	...
	// Update @aryee337/* catalog entries in root package.json
	...
	console.log("  Updated root catalog @aryee337/* entries");

	// 3. Update Rust workspace version
	console.log(`Updating Rust workspace version to ${version}…`);
	await $`sed -i -E 's/^version = "[^"]+"/version = "${version}"/' Cargo.toml`;
	...
	// 3b. Rename the aery-engine version sentinel so any `.node` left on disk from
	...
	// `js_name = "__aeryNativesV…"` literal. `gen-enums.ts` regenerates the matching
	// entries in `packages/aery-engine/native/{index.d.ts,index.js}` on the next napi
	...
	console.log(`Bumping aery-engine version sentinel to v${version}…`);
	const sentinelJsId = version.replace(/[^A-Za-z0-9]/g, "_");
	const sentinelName = `__aeryNativesV${sentinelJsId}`;
	const sentinelFiles = [
	...
	await $`sed -i -E 's/__aeryNativesV[A-Za-z0-9_]+/${sentinelName}/g' ${sentinelFiles}`;
	const libRs = await Bun.file("crates/aery-engine/src/lib.rs").text();
	if (!libRs.includes(`js_name = "${sentinelName}"`)) {
		console.error(
			`Error: aery-engine version sentinel did not move to ${sentinelName} in crates/aery-engine/src/lib.rs. ` +
				"The `__aeryNativesV…` literal may have been removed or renamed; restore it before releasing.",
		);
		process.exit(1);
	}
	...
	await git(["add", "."]);
	await git(["commit", "-m", `chore: bump version to ${version}`]).nothrow();
	console.log();
