...
from aery_rpc import (  # noqa: E402
    AgentEndEvent,
    AutoRetryEndEvent,
    AutoRetryStartEvent,
    ExtensionUiRequest,
...
def resolve_aery_bin(raw: str | None) -> str:
    ...
    if not found:
        raise SystemExit("Could not find `aery` on PATH. Set --aery-bin or AERY_BIN.")
    return found
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run OpenRouter fixture evaluations through aery RPC mode."
    )
    parser.add_argument("--aery-bin", default=os.environ.get("AERY_BIN"))
...
async def run_all(args: argparse.Namespace) -> int:
    ...
    results_dir = (
        ...
        if args.results_dir
        else tmp_root / f"aery-fixture-runs-{timestamp}"
    )
