...
| `packages/aery-engine`      | Bindings for native text/image/grep operations       |
| `packages/stats`        | Local observability dashboard (`aery stats`)          |
| `packages/utils`        | Shared utilities (logger, streams, temp files)       |
| `crates/aery-engine`     | Rust crate for performance-critical text/grep ops    |
...
- **Worker scripts**: spawn workers with the dev/compile-safe hybrid pattern. `with { type: "file" }` only copies the entry as a raw asset and does **not** bundle its imports — workers crashed silently in compiled binaries on every prior incarnation of that pattern (issues #1011, #1027). Use this shape instead:
  ...
  Validate any new worker with the dedicated smoke probe: `aery --smoke-test` spawns the stats sync worker, pings it, and exits — it's wired into `ci:test:smoke` and `scripts/install-tests/run-ci.sh` so binary, source-link, and tarball installs all exercise it. Add a sibling smoke if the new worker is on a different module graph.
...
| Binary lookup   | `$which("git")` from `@aryee337/aery-utils` | `spawnSync(["which", "git"])`   |
...
- `if (await file.exists()) { await file.json() }` → two syscalls plus race. Use try-catch with `isEnoent`:
  ```typescript
  import { isEnoent } from "@aryee337/aery-utils";
  try {
...
Logs go to `~/.aery/logs/aery.YYYY-MM-DD.log` with automatic rotation.
...
**Rules:**
- **Tabs → spaces** via `replaceTabs()` (from `@aryee337/aery-tui` or `../tools/render-utils`).
...
**Attribution:**
- Internal (from issues): `Fixed foo bar ([#123](https://github.com/eminent337/aery/issues/123))`.
- External contributions: `Added feature X ([#456](https://github.com/eminent337/aery/pull/456) by [@username](https://github.com/username))`.
