...
Self-hosted GitHub triage bot. Drives [`aery --mode rpc`](https://github.com/eminent337/aery)
...
- `question` → one comment, suffixed with a 👎-to-keep-open prompt; if the
  issue author doesn't react 👎 within `ROBAERY_QUESTION_AUTOCLOSE_HOURS`
...
Follow-up issue comments and PR review comments resume the same aery session
...
- **robaery** — FastAPI + sqlite event queue + `WorkerPool` running `aery` in
...
- **gh-proxy** — sibling on an `internal: true` network. Holds `GITHUB_TOKEN`,
  verifies HMAC-signed requests from robaery, executes REST + `git push`.
...
→ `worker.run_task` spawns `aery --mode rpc` with `cwd=worktree`,
persistent `session_dir`, model randomly drawn from `ROBAERY_MODEL` (CSV).

The agent uses aery's built-in tools (`read`/`edit`/`bash`/`lsp`, scoped to
...
`~/.aery/agent/models.container.yml` points at (mounted into the container as `models.yml`; kept under a separate filename on the host so the host aery doesn't route through the gateway). roboomp lives inside the aery
monorepo at `python/robaery/`; both the docker build context and the
`/work/aery` bind mount default to the parent monorepo (`../..`). Override
`PI_ROOT` only if you want a different aery checkout backing the build
and runtime.

Bot account needs **Write** on every repo in `ROBAERY_REPO_ALLOWLIST`. A
...
cp .env.example .env
$EDITOR .env
openssl rand -hex 32              # ROBAERY_GH_PROXY_HMAC_KEY
openssl rand -hex 32              # GITHUB_WEBHOOK_SECRET

bun run aery:image                  # build aery/aery:dev (one-time / on aery change)
bun run robaery:build && bun run robaery:up
...
comment out `ROBAERY_GH_PROXY_URL` / `ROBAERY_GH_PROXY_HMAC_KEY` and set
...
runtime layer; editing aery source rebuilds `aery/aery:dev`, which
roboomp's `Dockerfile.robaery` extends via `FROM ${PI_BASE}`.
...
The container entrypoint is `python -m robaery serve`. Other commands run
inside the running container:

```bash
docker compose exec robaery robaery triage  owner/repo#123   # synthesize an issues.opened and wait
docker compose exec robaery robaery replay  <delivery_id>    # re-enqueue a stored event and wait
docker compose exec robaery robaery status                   # dump issues table
docker compose exec robaery robaery cleanup owner/repo#123   # force workspace removal, state=abandoned
```

`bun run robaery:…` shortcuts in the root `package.json` cover the common
lifecycle commands (`robaery:dev`, `robaery:build`, `robaery:up`, `robaery:down`,
`robaery:logs`, `robaery:restart`, `robaery:reset`).

## Tests

```bash
pytest -x tests/                              # unit suite, no network
ROBAERY_INTEGRATION=1 pytest -x tests/test_worker_smoke.py
```

The integration test spawns a real `aery --mode rpc` against an
`httpx.MockTransport` GitHub and a local bare repo, so it needs `aery` on
...
- gh-proxy has no host port. The `robaery_internal` network is
...
- Agent subprocess env is scrubbed of `GITHUB_TOKEN` /
  `ROBAERY_GH_PROXY_HMAC_KEY` / friends via `worker._SCRUBBED_ENV_KEYS`.
...
- Pre-push gates (`gh_push_branch`): branch matches the workspace
  ...
  `origin/<default>..HEAD` carries `ROBAERY_GIT_AUTHOR_NAME` +
  `ROBAERY_GIT_AUTHOR_EMAIL`.
...
- **Crash recovery.** On startup, `db.reset_stuck_running()` flips
  ...
  `ROBAERY_SHUTDOWN_DRAIN_TIMEOUT_SECONDS` (25s) +
  `ROBAERY_SHUTDOWN_KILL_TIMEOUT_SECONDS` (5s); compose
  `stop_grace_period: 30s` covers both.
- **Logs.** Structured JSON on stdout, rotated to
  `/data/logs/robaery.log.jsonl`.
...
| Container exits with `PI_ROOT … missing` | `/work/aery` mount empty inside the container; on the host either run `docker compose` from `python/robaery/` so `PI_ROOT` defaults to `../..`, or export `PI_ROOT` to a valid aery checkout. |
| `git push: Authentication required` | Bot PAT lacks push, or `ROBAERY_BOT_LOGIN` ≠ PAT's account. |
| `refusing to push: commit author identity mismatch` | Some commit not authored as `ROBAERY_GIT_AUTHOR_*`. The error lists the offending shas; `git commit --amend --reset-author --no-edit`. |
...
| `Failed to load aery_engine` | Wrong arch / missing native. `bun run aery:image` then `bun run robaery:build`. |
| `No API key found for <provider>` | `~/.aery/agent/models.container.yml` mount missing or provider id mismatch with `ROBAERY_MODEL`. |
...
src/
  ...
  worker.py          synchronous aery RPC driver, prompt assembly, env scrubbing
...
tests/               pytest unit suite + one ROBAERY_INTEGRATION=1 smoke test
