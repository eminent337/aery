"""Unit tests for `robaery.natives_cache`.
...
from robaery.natives_cache import (
    CACHE_KEY_PATHS,
    NativesCache,
    _atomic_link,
    compute_key,
...
def _seed_repo(root: Path, *, with_all_inputs: bool = True) -> Path:
    ...
    if with_all_inputs:
        ...
        (natives / "package.json").write_text('{"name":"@aery/pi-natives"}\n')
...
def _populate_built_artifacts(repo_dir: Path, *, body: bytes = b"\x7fELF...native") -> Path:
    """Fill `packages/aery-engine/native/` with a complete built-artifact set."""
...
def test_compute_key_changes_when_each_input_changes(tmp_path: Path) -> None:
    ...
    mutations: dict[str, tuple[str, str]] = {
        ...
        "packages/aery-engine": ("packages/aery-engine/scripts/build-native.ts", "// edited\n"),
    }
