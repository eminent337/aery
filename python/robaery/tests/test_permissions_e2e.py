...
from robaery.github_backend import GitHubBackend
from robaery.github_client import IssueInfo, RepoInfo
from robaery.natives_cache import NativesCache
from robaery.natives_cache import compute_key as natives_compute_key
from robaery.sandbox import LocalGitTransport, SandboxManager, Workspace

pytestmark = pytest.mark.skipif(
    os.environ.get("ROBAERY_PERMISSION_E2E") != "1",
    reason="set ROBAERY_PERMISSION_E2E=1 to run slot-permission e2e tests",
)

_SLOT_ONE = 2001
_SLOT_TWO = 2002
_SHARED_OMP_GID = 2000
_AUTHOR_NAME = "robaery-bot"
_AUTHOR_EMAIL = "robaery-bot@example.invalid"
_REPO = "octo/permission-e2e"


def _require_linux_root_toolchain() -> None:
    if platform.system() != "Linux" or os.geteuid() != 0:
        pytest.skip("slot permission e2e tests require Linux root so subprocesses can drop to aery-N UIDs")
...
def slot_tmp_path() -> Iterator[Path]:
    root = Path(tempfile.mkdtemp(prefix="robaery-permission-e2e-", dir="/tmp"))
    root.chmod(0o755)
    try:
        yield root
    finally:
...
def test_slot_workspace_runs_bun_biome_cargo_and_git_after_root_reentry(
    ...
    stale_bun_cache = first.root / ".aery-xdg" / "cache" / "bun-install" / "root-owned-stale"
...
def _prepare_shared_natives_cache(slot_tmp_path: Path) -> NativesCache:
    """Provision `/data/cache/pi-natives` shape (root:aery, setgid 2770)."""
...
def _stage_built_natives(bindings: host_tools.ToolBindings, *, body: str = "ELFx") -> None:
    """Mirror what a napi build would leave in `packages/aery-engine/native/`.
    ...
    """
    _write_as_slot(bindings, "packages/aery-engine/native/pi_natives.linux-arm64.node", body)
    _write_as_slot(bindings, "packages/aery-engine/native/index.d.ts", "export const X: number;\n")
    _write_as_slot(bindings, "packages/aery-engine/native/index.js", "export const X = 1;\n")
    _write_as_slot(
        bindings,
...
def test_natives_cache_shares_artifacts_across_slot_workspaces(
    ...
    1. A capture from a slot-owned workspace lands in the shared cache with
       group `aery` setgid inheritance so any other slot can read it.
    ...
    # Cache root is setgid `aery`; new files inherit gid `aery` so any slot
    # with `extra_groups=[aery]` can read them.
    ...
    # Slot 2 must be able to read the populated artifacts (group aery + 0660
    # via setgid inheritance from the cache root).
    _run_ok(bindings2, ["test", "-r", "packages/aery-engine/native/pi_natives.linux-arm64.node"])
    _run_ok(bindings2, ["test", "-r", "packages/aery-engine/native/index.d.ts"])
    ...
    _run_ok(
        ...
        [
            ...
            "packages/aery-engine/native/pi_natives.linux-arm64.node",
        ],
    )
