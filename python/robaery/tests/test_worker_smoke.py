...
Runs only when ROBAERY_INTEGRATION=1 and `aery` is available on PATH (or via
ROBAERY_COMMAND). Spins up:

- a local bare git repo with a trivial failing test,
- a fake GitHub API via httpx.MockTransport that records comments + PRs,
- a real `aery --mode rpc` subprocess driven by `worker.run_task`.
...
INTEGRATION = os.environ.get("ROBAERY_INTEGRATION") == "1"

pytestmark = pytest.mark.skipif(
    not INTEGRATION,
    reason="ROBAERY_INTEGRATION=1 required to run the aery-backed smoke test",
)
...
def test_triage_end_to_end(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    from robaery.sandbox import LocalGitTransport, SandboxManager
    from robaery.tasks import triage_issue

    bare = _seed_failing_repo(tmp_path)

    monkeypatch.setenv("ROBAERY_GH_PROXY_URL", "http://gh-proxy.invalid:8081")
    monkeypatch.setenv("ROBAERY_GH_PROXY_HMAC_KEY", "test-hmac-key-aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa")
    monkeypatch.setenv("GITHUB_TOKEN", "")
    monkeypatch.setenv("GITHUB_WEBHOOK_SECRET", "secret")
    monkeypatch.setenv("ROBAERY_BOT_LOGIN", "robaery-bot")
    monkeypatch.setenv("ROBAERY_REPO_ALLOWLIST", "octo/widget")
    monkeypatch.setenv("ROBAERY_WORKSPACE_ROOT", str(tmp_path / "workspaces"))
    monkeypatch.setenv("ROBAERY_SQLITE_PATH", str(tmp_path / "robaery.sqlite"))
    monkeypatch.setenv("ROBAERY_LOG_DIR", str(tmp_path / "logs"))
    monkeypatch.setenv("ROBAERY_TASK_TIMEOUT_SECONDS", "300")
