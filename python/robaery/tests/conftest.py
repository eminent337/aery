...
from robaery.dashboard import reset_index_cache, static_dir
from robaery.db import Database, close_database

# Minimum HTML the dashboard handler needs to render: `<title>` plus a script
# block carrying the `__ROBAERY_CONFIG__` sentinel. The real Vite-built bundle
# adds JS/CSS asset links; tests only care about the rendering contract.
_PLACEHOLDER_INDEX_HTML = (
    "<!doctype html>\n"
    '<html lang="en">\n'
    '  <head><meta charset="utf-8"><title>robaery</title></head>\n'
    "  <body>\n"
    '    <div id="app"></div>\n'
    '    <script id="robaery-config" type="application/json">__ROBAERY_CONFIG__</script>\n'
    "  </body>\n"
    "</html>\n"
)
...
def _baseline_env(tmp_path: Path) -> dict[str, str]:
    return {
        ...
        "ROBAERY_GH_PROXY_URL": "http://gh-proxy.invalid:8081",
        "ROBAERY_GH_PROXY_HMAC_KEY": "test-hmac-key-aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa",
        "GITHUB_WEBHOOK_SECRET": "test-webhook-secret",
        "ROBAERY_BOT_LOGIN": "robaery-bot",
        "ROBAERY_GIT_AUTHOR_NAME": "robaery-bot",
        "ROBAERY_GIT_AUTHOR_EMAIL": "robaery-bot@example.invalid",
        "ROBAERY_REPO_ALLOWLIST": "octo/widget",
        "ROBAERY_MODEL": "anthropic/claude-sonnet-4-5",
        "ROBAERY_THINKING": "high",
        "ROBAERY_WORKSPACE_ROOT": str(tmp_path / "workspaces"),
        "ROBAERY_SQLITE_PATH": str(tmp_path / "robaery.sqlite"),
        "ROBAERY_LOG_DIR": str(tmp_path / "logs"),
        ...
        # cache flip `ROBAERY_NATIVES_CACHE_ENABLED=true` explicitly.
        "ROBAERY_NATIVES_CACHE_ROOT": str(tmp_path / "natives-cache"),
        "ROBAERY_NATIVES_CACHE_ENABLED": "false",
    }
...
def env(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> dict[str, str]:
    ...
    monkeypatch.delenv("ROBAERY_PROVIDER", raising=False)
    monkeypatch.setenv("ROBAERY_REPLAY_TOKEN", "")
...
def proxy_env(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> dict[str, str]:
    ...
    baseline.pop("ROBAERY_GH_PROXY_URL", None)
    baseline.pop("ROBAERY_GH_PROXY_HMAC_KEY", None)
    ...
    monkeypatch.setenv("ROBAERY_GH_PROXY_URL", "")
    monkeypatch.setenv("ROBAERY_GH_PROXY_HMAC_KEY", "")
    monkeypatch.delenv("ROBAERY_PROVIDER", raising=False)
    monkeypatch.setenv("ROBAERY_REPLAY_TOKEN", "")
    reset_settings_cache()
    yield baseline
