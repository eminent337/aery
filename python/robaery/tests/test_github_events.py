...
from robaery.github_events import (
    extract_mention,
    ...
    decision = route(
        "issue_comment",
        {
            ...
            "comment": {
                ...
                "body": "@robaery-bot please re-review",
            },
            "issue": {"number": 9, "user": {"login": "contributor"}, "pull_request": {"url": "x"}},
            "repository": {"full_name": "octo/widget"},
        },
...
def test_extract_mention_returns_body_minus_mention() -> None:
    assert extract_mention("hey @robaery-bot please look", "robaery-bot") == "hey please look"
    assert extract_mention("@robaery-bot do X", "robaery-bot") == "do X"


def test_extract_mention_returns_none_without_mention() -> None:
    assert extract_mention("hello there", "robaery-bot") is None
    assert extract_mention(None, "robaery-bot") is None
    assert extract_mention("", "robaery-bot") is None


def test_extract_mention_is_case_insensitive() -> None:
    assert extract_mention("yo @ROBAERY-BOT", "robaery-bot") == "yo"


def test_extract_mention_respects_hyphen_word_boundary() -> None:
    # @robaery-bot-helper must NOT match @robaery-bot.
    assert extract_mention("@robaery-bot-helper hi", "robaery-bot") is None


def test_extract_mention_handles_multiple_occurrences() -> None:
    assert extract_mention("@robaery-bot one, then @robaery-bot two", "robaery-bot") == "one, then two"
    ...
    decision = route(
        "issue_comment",
        {
            ...
            "comment": {
                ...
                "body": "@robaery-bot go ahead with the plan",
            },
            "issue": {"number": 9},
            "repository": {"full_name": "octo/widget"},
        },
    ...
    decision = route(
        "issue_comment",
        {
            ...
            "comment": {
                "user": {"login": "can1357"},
                "author_association": "OWNER",
                "body": "@robaery-bot change the indentation in foo.py",
            },
    ...
    decision = route(
        "pull_request_review_comment",
        {
            ...
            "comment": {
                "user": {"login": "can1357"},
                "author_association": "OWNER",
                "body": "@robaery-bot use a generator here",
            },
    ...
    decision = route(
        "issue_comment",
        {
            ...
            "comment": {
                "user": {"login": "can1357"},
                "author_association": "OWNER",
                "body": "@robaery-bot /model gpt /thinking low\nrefactor X",
            },
            "issue": {"number": 9},
            "repository": {"full_name": "octo/widget"},
        },
