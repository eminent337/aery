...
from robaery import host_tools, persona, pragmas
from robaery.cancellation import register_cancel_hook, unregister_cancel_hook
from robaery.config import Settings
from robaery.db import Database, issue_key
from robaery.git_ops import DirtyState, inspect_dirty_state
...
from robaery.host_tools import AbortController, ToolBindings, _git_identity_env
from robaery.natives_cache import NativesCache
from robaery.natives_cache import compute_key as natives_compute_key
from robaery.sandbox import GitTransport, Workspace, _prepare_slot_runtime_env, _safe_directory_env
...
_SCRUBBED_ENV_KEYS: tuple[str, ...] = (
    ...
    "ROBAERY_REPLAY_TOKEN",
    "ROBAERY_GH_PROXY_HMAC_KEY",
)
...
def _stage_agent_home() -> None:
    ...
    for rel in (Path(".agent"), Path(".aery/agent")):
...
def _build_extra_env(settings: Settings) -> dict[str, str]:
    """Build the env overlay passed to the aery subprocess.
...
def _has_prior_session(session_dir: Path) -> bool:
    """Return True iff `session_dir` already contains an aery JSONL transcript.
...
def _run_rpc_blocking(
    ...
    *,
    ...
    with RpcClient(
        ...
        extra_groups=["aery"] if inputs.slot_uid is not None else None,
    ) as client:
        # Arm cancellation: from this point the API can kill the aery subprocess
        ...
        try:
            ...
            if hard_timeout_fired.is_set():
                raise TimeoutError("aery task exceeded hard timeout")
            log.info(
                "rpc_done",
                extra={
                    "issue": bindings.issue_key,
