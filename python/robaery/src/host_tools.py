...
from aery_rpc import HostTool, HostToolContext, RpcCommandError, host_tool

from robaery import persona
from robaery.config import Settings
from robaery.db import Database, issue_key
from robaery.git_ops import GitCommandError, HeadDriftError
from robaery.github_backend import GitHubBackend
from robaery.github_client import GitHubError, IssueInfo, PullRequestFileInfo, RepoInfo
...
class AbortController:
    ...
    teardown of the aery subprocess. The worker pre-populates `stop` with a
...
class ToolBindings:
    ...
    # Set by the worker before launching aery. Carries the abort-task signal
...
def _build_abort_task(bindings: ToolBindings) -> HostTool[Any, Any]:
    def execute(args: dict[str, Any], _ctx: HostToolContext[Any]) -> str:
        ...
        # races against the imminent aery teardown.
