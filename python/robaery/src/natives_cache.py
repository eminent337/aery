"""Content-addressed cache of pre-built ``packages/aery-engine/native/`` artifacts.
...
2. On workspace populate: hardlinks cached files into the worktree's
   ``packages/aery-engine/native/`` (a noop on cache miss).
...
Ownership: cache root is provisioned ``root:aery 02770`` by ``entrypoint.sh``
so slot subprocesses (group ``aery``) can capture under setgid inheritance.
...
CACHE_KEY_PATHS: tuple[str, ...] = (
    ...
    "Cargo.toml",
    "rust-toolchain.toml",
    "packages/aery-engine",
)

# Files in ``packages/aery-engine/native/`` that ARE pure functions of the
...
def _git_safe_directory_env(repo_dir: Path) -> dict[str, str]:
    ...
    ``robaery.sandbox._safe_directory_env`` but kept local to avoid a circular
        ...
        *,
        ...
        with _flock(self.lockfile(repo)):
            ...
            try:
                ...
                # the setgid `aery` model that lets other slots read it.
                # A copy creates a fresh inode owned by the orchestrator
                # (root) and inherits gid `aery` from the setgid 2770
                # cache root.
