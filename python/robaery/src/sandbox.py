...
PAT injection via `--config-env` (see `robaery.git_ops`); the `ProxyGitTransport`
in `robaery.proxy_client` forwards the same set of operations over HMAC RPC.
...
1. **Workspace tree** (`/data/workspaces/<key>/`, including `repo/`,
   `.aery-session/`, `context/`, `artifacts/`, `.aery-tmp/`, `.aery-xdg`):
   single-owner. Owned by the active slot UID/GID (`aery-N`) when slot
...
2. **Clone pool** (`/data/workspaces/_pool/<owner>__<repo>/`): genuinely
   multi-slot. Owned by `root:aery` (gid 2000) with setgid `02770`; cross-slot
...
3. **Language tool caches** (`/data/cache/{cargo,cargo-target,rustup,bun-cache}`):
   multi-slot. Owned by `root:aery` with setgid `02770`; provisioned by
   `entrypoint.sh`.
...
from robaery.natives_cache import CacheHit, NativesCache
...
class GitTransport(Protocol):
    ...
    - `robaery.proxy_client.ProxyGitTransport`: forwards over HMAC RPC.
    """
...
def _prepare_slot_tmpdir(workspace: Workspace, slot_uid: int | None) -> Path:
    ...
    - replaces any non-directory at ``.aery-tmp`` (symlink-protection: a user
    ...
    tmpdir = workspace.root / ".aery-tmp"
    try:
        st = tmpdir.lstat()
...
def _prepare_slot_runtime_env(workspace: Workspace, slot_uid: int | None) -> dict[str, str]:
    ...
    and are group-shared via ``aery``. Bun's install cache is explicitly
    ...
    xdg_root = workspace.root / ".aery-xdg"
    ...
    for base in (xdg_data, xdg_state, xdg_cache):
        ...
        (base / "aery").mkdir(parents=True, exist_ok=True)
...
def _provision_runtime_dirs(ws_root: Path) -> None:
    ...
    picks up ``.aery-tmp`` and the per-workspace XDG tree. Without this,
    ...
    Symlink-safe on ``.aery-tmp`` (replaces a planted non-directory in place).
    """
    tmpdir = ws_root / ".aery-tmp"
    try:
        st = tmpdir.lstat()
    except FileNotFoundError:
        pass
...
def _share_git_metadata_with_slots(repo_dir: Path, slot_uid: int | None) -> None:
    ...
    `aery-N` user, so the pool-side worktree gitdir, refs, reflogs, and object
    directories must stay writable through the shared `aery` group.
    """
    if not _slot_permissions_active(slot_uid):
        return
...
class SandboxManager:
    ...
    def ensure_workspace(
        ...
        *,
        ...
        session_dir = ws_root / ".aery-session"
        ...
        # keeps its `root:aery` ownership (the slot reads through group `aery`);
    ...
    def _populate_natives_cache(self, workspace: Workspace, *, slot_uid: int | None = None) -> None:
        ...
        Post-populate, the populated `packages/aery-engine/native/` directory
        ...
        `.node` files are LEFT at `root:aery` ownership — chowning them
        would chown the cache file too (shared inode), breaking the
        cross-slot sharing model. The slot reads them via group `aery`.
        """
    ...
    def _chown_natives_for_slot(native_dir: Path, hit: CacheHit, *, slot_uid: int) -> None:
        ...
        hardlinks back into the root:aery cache and the slot reads them via
        group `aery`. Everything else (the directory itself, copied
