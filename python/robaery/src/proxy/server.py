...
Robaery calls every endpoint with HMAC headers (see `robaery.proxy_hmac`).
Authenticated requests dispatch to a single `GitHubClient` instance holding
the PAT, or to `robaery.git_ops` for git transport. The PAT never leaves
this process.

Endpoint payloads are deliberately typed (no generic GitHub passthrough):
each one names exactly one operation robaery performs.
"""
...
from robaery.github_client import GitHubClient, GitHubError
from robaery.proxy_hmac import HEADER_SIGNATURE, HEADER_TIMESTAMP, verify
from robaery.sandbox import _safe_directory_env, _slot_subprocess_kwargs
from robaery.sandbox import workspace_key as compute_workspace_key
...
def _resolve_hmac_key(cfg: Settings) -> bytes:
    if cfg.gh_proxy_hmac_key is None:
        raise HTTPException(500, "gh-proxy: ROBAERY_GH_PROXY_HMAC_KEY not configured")
...
def create_proxy_app(settings: Settings) -> FastAPI:
    ...
    app = FastAPI(title="robaery-gh-proxy", version="0.1.0", lifespan=lifespan)
    ...
    # ---- git transport ----
    #
    # The underlying `robaery.git_ops` primitives are blocking `subprocess.run`
