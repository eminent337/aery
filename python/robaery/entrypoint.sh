...
#
# Used by both the orchestrator (CMD: `python -m robaery serve`) and the
# sibling gh-proxy (compose command: `python -m robaery.proxy serve`). The
# proxy role does NOT need a $PI_ROOT aery checkout — it never runs aery.
set -euo pipefail

# Shared git metadata under /data/workspaces/_pool is intentionally group
# writable by the `aery` group so interrupted work can resume on a different
...
# as $@ here (after tini --), so $1=python, $2=-m, $3=robaery.proxy is the
# canonical shape; we also accept a single concatenated arg for safety.
is_proxy_role=0
if [ "${1:-}" = "python" ] && [ "${2:-}" = "-m" ] && [[ "${3:-}" == robaery.proxy* ]]; then
    is_proxy_role=1
elif [[ "${1:-}" == *"robaery.proxy"* ]]; then
    is_proxy_role=1
fi

/usr/sbin/groupadd -f -g 2000 aery
max_slots="${ROBAERY_MAX_CONCURRENCY:-8}"
for i in $(seq 1 "$max_slots"); do
    user="aery-$i"
    slot_group="aery-$i"
    slot_id=$((2000 + i))
    /usr/sbin/groupadd -f -g "$slot_id" "$slot_group"
    id -u "$user" >/dev/null 2>&1 || /usr/sbin/useradd -u "$slot_id" -g "$slot_group" -G aery -M -N -s /usr/sbin/nologin "$user"
    /usr/sbin/usermod -g "$slot_group" -a -G aery "$user"
done
...
if [ ! -d "$PI_ROOT/packages/coding-agent" ]; then
    echo "roboomp: PI_ROOT=$PI_ROOT does not look like a aery checkout (no packages/coding-agent/)" >&2
    exit 1
fi
...
mkdir -p /data/cache/cargo /data/cache/cargo-target /data/cache/rustup /data/cache/aery-engine
chown -R root:aery /data/cache /data/workspaces/_pool
...
rm -rf /srv/agent-home/.agent /srv/agent-home/.aery/agent
mkdir -p /srv/agent-home/.agent /srv/agent-home/.aery/agent
if [ -e /srv/agent-home-stage/.agent ]; then
    cp -a /srv/agent-home-stage/.agent/. /srv/agent-home/.agent/
fi
if [ -e /srv/agent-home-stage/.aery/agent ]; then
    cp -a /srv/agent-home-stage/.aery/agent/. /srv/agent-home/.aery/agent/
fi
...
touch /data/robaery.sqlite
chown root:root /data/robaery.sqlite
chmod 0600 /data/robaery.sqlite
for db_file in /data/robaery.sqlite-wal /data/robaery.sqlite-shm; do
    if [ -e "$db_file" ]; then
