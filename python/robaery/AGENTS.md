...
`roboomp` is a self-hosted GitHub triage-and-fix bot that drives [`aery --mode rpc`](https://github.com/eminent337/aery) as a subprocess. On every issue opened in an allowlisted repository it classifies the issue, applies labels, then branches into one of: reproduce → fix → PR (`bug` / `documentation`), single-comment answer (`question`), single thoughtful comment (`enhancement` / `proposal`), or brief comment (`invalid` / `duplicate`). Follow-up comments and PR review comments resume the same aery session so the agent keeps its prior reasoning. If the orchestrator restarts mid-task, the dispatcher resumes the same session via `aery --continue` from the per-issue `session_dir`, so an interrupted task re-enters its prior reasoning instead of restarting from scratch. The orchestrator runs as a single FastAPI process inside Docker with SQLite-backed durable event state.

## Architecture & Data Flow

Webhook → durable queue → async dispatcher → per-issue git worktree → aery RPC subprocess + host tools.
...
4. `queue.WorkerPool._dispatch_loop` atomically claims `state='queued'` rows under `BEGIN IMMEDIATE`, guarded by an in-process `_inflight` set keyed by `(owner, repo, number)` to serialize per-issue work. Cap: `ROBAERY_MAX_CONCURRENCY` (default 8).
...
6. `tasks.*` dispatchers build `TaskInputs` and call `worker.run_task()` which spawns `aery --mode rpc` with `cwd=worktree`, persistent `session_dir`, and a randomly-picked model from `ROBAERY_MODEL` (CSV pool). When `<session_dir>/*.jsonl` already exists the worker passes `--continue`, so both follow-up events and crash-restarted events resume the same session.
7. Inside the subprocess the agent uses **built-in aery tools** (read/edit/write/bash/lsp, scoped to the worktree) and **host tools** from `host_tools.py` (the only surface allowed to mutate GitHub or write audit rows).
...
- `tests/` — pytest suite. `test_worker_smoke.py` is gated on `ROBAERY_INTEGRATION=1`.
- `data/` — runtime state (sqlite + WAL, `workspaces/`, `logs/`). Never committed.
- `/Dockerfile` (aery root) — produces `aery/aery:dev` (aery runtime image: python + bun + rustup + aery-engine + omp_rpc + `/usr/local/bin/aery` shim + the full aery source under `/aery`). Stages: `natives-builder` → `wheel-builder` → `aery-base` → `aery-runtime` (default). Built via `bun run aery:image`. Robomp's image extends `aery-base` via `FROM ${PI_BASE}` in `/Dockerfile.robaery`.

## Development Commands

Task runner is `bun` against the **monorepo root** `package.json`. roboomp itself no longer ships a `package.json`; every recipe lives at the root under the `robaery:*` namespace. Local venv (no docker): `bun run robaery:install` runs `pip install -e 'python/robaery[dev]'`. From there:

```
bun run test:py                   # pytest -x python/aery-rpc/tests python/robaery/tests
bun run robaery:test:integration   # ROBAERY_INTEGRATION=1, requires aery on PATH
bun run robaery:serve              # python -m robaery serve on the host
```

Docker inner loop:

```
bun run aery:image                  # build aery/aery:dev (one-time / on aery change)
bun run aery:run                    # docker run -it aery/aery:dev (smoke-test the shim)
bun run robaery:build              # aery:image (if aery changed) + docker compose build
bun run robaery:dev                # build + up -d + follow logs
bun run robaery:up / robaery:down / robaery:restart / robaery:logs
bun run robaery:rebuild            # docker compose build --no-cache
bun run robaery:reset              # `down -v` + drop the aery image
```

Frontend (Vite + SolidJS, in `web/` — still a bun workspace):

```
bun run robaery:web:dev            # vite dev server with proxy to :8080
bun run robaery:web:build          # produce src/static/ bundle
bun --cwd=python/robaery/web run typecheck   # tsc --noEmit
```

In-container CLI (`robaery` console script → `robaery.cli:main`): no root aliases — invoke directly:

```
docker compose --project-directory python/robaery exec robaery robaery triage owner/repo#N
docker compose --project-directory python/robaery exec robaery robaery replay <delivery_id>
docker compose --project-directory python/robaery exec robaery robaery status
docker compose --project-directory python/robaery exec robaery robaery cleanup owner/repo#N
```

HTTP / sqlite / webhook inspection is unaliased — use `curl http://localhost:${ROBAERY_BIND_PORT:-8080}/{healthz,readyz,events,issues}` and `docker compose --project-directory python/robaery exec robaery sqlite3 /data/robaery.sqlite` directly.
...
- **Async style**: FastAPI handlers and `queue.WorkerPool` are async. `worker.run_task` is **synchronous** and runs in a worker thread because `aery-rpc` is blocking — keep it that way; don't try to async it. CLI commands wrap with `asyncio.run`.
- **Config**: `pydantic-settings` `Settings` in `config.py` with `ROBAERY_*` env prefix (e.g. `ROBAERY_MAX_CONCURRENCY`, `ROBAERY_REPO_ALLOWLIST`). Access only via `get_settings()` (`@cache` singleton). Tests must call `reset_settings_cache()` after mutating env.
...
- `src/worker.py` — synchronous aery RPC driver, prompt assembly via `persona`.
...
- `/Dockerfile.robaery` (aery root) — robaery's image. `FROM ${PI_BASE}` (default `aery/aery:dev`), adds the SolidJS dashboard bundle, the robaery Python package, and the `robaery-entrypoint` shim. Tini entrypoint, exposes `8080`, `VOLUME /data`. The toolchain (python + bun + rustup + aery-engine + omp_rpc + `aery` shim) comes from `aery-base` — no duplication in this file.
- `docker-compose.yml` — `build.args.PI_BASE`, mounts `$PI_ROOT:/work/aery:ro`, `./data:/data`, `~/.aery/agent/models.container.yml:ro` (mapped to `models.yml` inside the container — kept separate from the host's `~/.aery/agent/models.yml` so the host aery doesn't pick up gateway routing intended only for the container), `extra_hosts: llm-gateway.internal:host-gateway`.
...
- **Container runtime**: Docker Compose v2. The image embeds Bun 1.3.14 + a rustup launcher and exposes `aery` via a `/usr/local/bin/aery` shim; `ROBAERY_COMMAND=aery` should not need changing.
- **Required env** (set in `.env`, see `.env.example`): `GITHUB_WEBHOOK_SECRET`, `ROBAERY_BOT_LOGIN`, `ROBAERY_GIT_AUTHOR_NAME`, `ROBAERY_GIT_AUTHOR_EMAIL`, `ROBAERY_REPO_ALLOWLIST`, plus model knobs (`ROBAERY_MODEL`, `ROBAERY_THINKING`, optional `ROBAERY_PROVIDER`) and rate-limit / concurrency / timeout overrides. **GitHub auth is mode-exclusive**: either set `ROBAERY_GH_PROXY_URL` + `ROBAERY_GH_PROXY_HMAC_KEY` (gh-proxy mode; PAT lives only in the sidecar container — the bundled compose default), or set `GITHUB_TOKEN` directly (single-process PAT mode). `Settings._validate_proxy_or_pat` rejects a `.env` that sets both.
- **PI_ROOT resolution**: roboomp lives inside the aery monorepo at `python/robaery/`. `bun run aery:image` builds the parent monorepo (`../..`) as its docker build context to produce `aery/aery:dev`; `docker-compose.yml` extends that image via `PI_BASE` and mounts the same parent path read-only at `/work/aery` for the orchestrator to see live source. Override `PI_ROOT` only when pointing the build/mount at a different aery checkout. Inside the container the path is always `/work/aery`. Build invalidation stays bounded: Python-only edits in roboomp never trigger a natives recompile.
...
- **Fixtures** (`tests/conftest.py`):
  - `env` — `monkeypatch`-sets all required `ROBAERY_*` env vars and calls `reset_settings_cache()` before/after.
...
- **Integration**: `tests/test_worker_smoke.py` is gated by `ROBAERY_INTEGRATION=1` (uses `pytestmark.skipif`) and needs `aery` on `PATH`. Don't enable it in default `bun run test:py`.
