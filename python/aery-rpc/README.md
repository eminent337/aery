...
Typed Python bindings for the `aery --mode rpc` protocol used by the coding agent.
...
- typed startup options for common `aery --mode rpc` flags such as thinking level,
...
```python
from aery_rpc import RpcClient
...
```python
from aery_rpc import MessageUpdateEvent, RpcClient
...
from aery_rpc import RpcClient, host_tool
...
```python
from aery_rpc import RpcClient, host_uri
...
```python
from aery_rpc import assistant_text, assistant_text_with_thinking
